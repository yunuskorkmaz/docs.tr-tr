...
Windows Presentation Foundation (WPF) içindeki denetimler <xref:System.Windows.Controls.ControlTemplate> , bu denetimin görsel ağacını içeren bir öğesine sahiptir. Bir denetimin yapısını ve görünümünü, bu denetimin öğesini değiştirerek değiştirebilirsiniz <xref:System.Windows.Controls.ControlTemplate> . Bir denetimin görsel ağacının yalnızca bir kısmını değiştirmek mümkün değildir; bir denetimin görsel ağacını değiştirmek için <xref:System.Windows.Controls.Control.Template%2A> denetimin özelliğini yeni ve tamamlanmış olarak ayarlamanız gerekir <xref:System.Windows.Controls.ControlTemplate> .  
