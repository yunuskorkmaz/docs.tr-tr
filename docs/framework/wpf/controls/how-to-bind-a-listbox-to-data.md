...
Bir uygulama geliştiricisi oluşturabilirsiniz <xref:System.Windows.Controls.ListBox> her içeriğini belirtmeden denetimleri <xref:System.Windows.Controls.ListBoxItem> ayrı olarak. Veri bağlama, tek tek öğelere veri bağlamak için kullanabilirsiniz.  
  
 Aşağıdaki örnekte nasıl oluşturulacağını gösterir bir <xref:System.Windows.Controls.ListBox> , doldurur <xref:System.Windows.Controls.ListBoxItem> öğeleri adlı bir veri kaynağına veri bağlama tarafından *renkleri*. Bu durumda kullanmak gerekli değildir <xref:System.Windows.Controls.ListBoxItem> her öğenin içeriğini belirtmek için etiketler.  
