...
GDI + sağlar `Bitmap` ızgara görüntüleri ile çalışmak için sınıf ve `Metafile` vektör görüntüleri ile çalışmak için sınıf. `Bitmap` Ve `Metafile` sınıflarının her ikisi de devralmanız `Image` sınıfı.  
