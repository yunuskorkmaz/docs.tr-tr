...
> [!NOTE]
> Windows Form Tasarımcısı'nda Visual Studio shadows üretkenliği en üst düzeye çıkarmak için <xref:System.Windows.Forms.Control.AutoSize%2A> özelliği <xref:System.Windows.Forms.Form> sınıfı. Tasarım zamanında, gibi ancak form davranışını <xref:System.Windows.Forms.Control.AutoSize%2A> özelliği `false`gerçek ayarına bakılmaksızın. Hiçbir özel Konaklama çalışma zamanında yapılmış ve <xref:System.Windows.Forms.Control.AutoSize%2A> özelliği uygulanan özellik ayarı tarafından belirtildiği gibi.
