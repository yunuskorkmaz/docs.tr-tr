...
Bu tabloda, 4.5. *x* .NET Framework 4.5 ve onun nokta sürümleri için 4.5.1 ve 4.5.2, 4.6. *x* .NET Framework 4.6 ve 4.6.1 ve 4.6.2, onun nokta sürümleri başvuruyor 4.7. *x* 4.7.1 ve 4.7.2, onun nokta sürümleri .NET Framework 4.7 ile ifade eder ve 4.8 için .NET Framework 4.8 ifade eder.
...
|.NET Framework 4.5 *.x*/4.6 *.x*/4.7 *.x*/4,8 (*dil*) .NET Framework 4.5 gerektirir *.x*/4.6 *.x*/4.7 *.x*  /4,8. Lütfen .NET Framework 4.5 yükleme *.x*/4.6 *.x*/4.7 *.x*İndirme Merkezi'nden /4.8 ve kurulumu yeniden çalıştırın.|Dil paketini yüklemeden önce belirtilen .NET Framework sürüm İngilizce sürümünü yüklemeniz gerekir. Daha fazla bilgi için üzerinde bölümüne bakın. [dil paketlerini yüklemek için](../../../docs/framework/install/guide-for-developers.md#to-install-language-packs) Yükleme Kılavuzu.|  
|.NET Framework 4.5 yüklenemiyor *.x*/4.6 *.x*/4.7 *.x*  /4,8. Bilgisayarınızdaki diğer uygulamalar bu programla uyumlu değil.<br /><br /> -veya-<br /><br /> Bilgisayarınızdaki diğer uygulamalar bu programla uyumlu değil.|Bu ileti büyük olasılıkla .NET Framework'ün önizleme veya RC sürümünün yüklenmiş olmasından kaynaklanmıştır. Önizlemeyi veya RC sürümünü kaldırın ve kurulumu yeniden çalıştırın.|  
|.NET framework 4.5 *.x*/4.6 *.x*/4.7 *.x*  /4.8, bu paket kullanılarak kaldırılamıyor. .NET Framework 4.5 kaldırmak için *.x*/4.6 *.x*/4.7 *.x*/bilgisayarınızdan 4.8 Git **Denetim Masası**, seçin  **Programlar ve Özellikler**, seçin **yüklü güncelleştirmeleri görüntüle**, (KB2828152) Microsoft Windows Güncelleştirmesi'ni seçin ve ardından **kaldırma**.|Önizleme veya RC sürümlerini .NET Framework'ün kaldırın, yüklemekte olduğunuz paket değil.<br /><br /> Önizlemeyi veya RC kaldırın. Denetim Masası'ndan yayın.|  
...
|.NET framework 4.5 *.x*/4.6 *.x*/4.7 *.x*  /bileşen deposu bozuk olduğundan 4.8 yüklenmedi.|Bkz: [DISM ya da sistem güncelleştirme hazırlığı aracını kullanarak Windows güncelleştirme düzeltme hataları](https://support.microsoft.com/en-us/kb/947821) daha fazla bilgi için.|  
...
|.NET Framework 4.5 *.x*/4.6 *.x*/4.7 *.x*  /4.8 zaten bu işletim sisteminin bir parçası. .NET Framework 4.5 yüklemek gerekmez *.x*/4.6 *.x*/4.7 *.x*/4.8 yeniden dağıtılabilir.|Eylem yok.<br /><br /> Bir sistemde hangi .NET Framework sürümlerinin yüklü olduğunu saptamak için bkz: [nasıl yapılır: Hangi .NET Framework sürümlerinin yüklü olduğunu belirleme](../../../docs/framework/migration-guide/how-to-determine-which-versions-are-installed.md). Bkz: [sistem gereksinimleri](../../../docs/framework/get-started/system-requirements.md) desteklenen işletim sistemleri için.|  
