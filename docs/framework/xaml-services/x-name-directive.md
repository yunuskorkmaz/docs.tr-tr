...
## <a name="wpf-usage-notes"></a>WPF kullanım notları  
 ...
 Kullanan bir WPF uygulaması için [!INCLUDE[TLA#tla_visualb](../../../includes/tlasharptla-visualb-md.md)] hedefler ve XAML dosyaları içerir `Page` yapı eylemi, ayrı başvuru özelliği ekler derleme sırasında oluşturulan `WithEvents` sahip tüm öğeleri anahtar sözcüğü bir `x:Name`, Destek `Handles` olay işleyici temsilcileri sözdizimi. Bu özellik her zaman herkes tarafından kullanılabilir. Daha fazla bilgi için bkz: [Visual Basic ve WPF olay işleme](../../../docs/framework/wpf/advanced/visual-basic-and-wpf-event-handling.md).  
