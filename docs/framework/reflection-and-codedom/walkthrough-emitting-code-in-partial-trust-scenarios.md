...
1. (RMA) <xref:System.Security.Permissions.ReflectionPermission> bayrağıyla yeni bir nesne oluşturun ve izni izin kümesine eklemek için <xref:System.Security.PermissionSet.SetPermission%2A?displayProperty=nameWithType> yöntemini kullanın. <xref:System.Security.Permissions.ReflectionPermissionFlag.RestrictedMemberAccess>
...
- Alternatif olarak, <xref:System.Reflection.Emit.DynamicMethod.%23ctor%28System.String%2CSystem.Type%2CSystem.Type%5B%5D%2CSystem.Boolean%29> oluşturucuyu kullanarak ve `true` `restrictedSkipVisibility` parametresi için belirterek, JIT görünürlük denetimlerini atlayabilme özelliğine sahip anonim olarak barındırılan bir dinamik yöntem oluşturun.
...
- `AccessPrivateMethod` Metodun ikinci aşırı yüklemesi yürütülür ve daha sonra JIT görünürlük denetimleri atlanır. Bu kez, `internal` `FirstChar` <xref:System.String> sınıfının özelliğine erişmeye çalıştığı için dinamik yöntem derlendiğinde başarısız olur. <xref:System.String> Sınıfını içeren derleme tam olarak güvenilirdir. Bu nedenle, kodu yayan derlemeden daha yüksek bir güven düzeyindedir.

Bu karşılaştırma, kısmen <xref:System.Security.Permissions.ReflectionPermissionFlag.RestrictedMemberAccess?displayProperty=nameWithType> güvenilen kodun, güvenilen kodun güvenliği tehlikeye atmadan diğer kısmen güvenilen kod için görünürlük denetimlerini atlamasını sağlar.
