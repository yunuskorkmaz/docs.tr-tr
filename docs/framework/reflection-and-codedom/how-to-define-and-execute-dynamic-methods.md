...
3. Oluşturun <xref:System.Reflection.Emit.DynamicMethod>. Bu örnekte, yöntemin adı yoktur. Dönüş değerinin türü ( `int` `Integer` Visual Basic) olarak belirtilir. Yöntemi, `Example` sınıfının özel ve korunan üyelerine erişebilir.  
...
5. <xref:System.Reflection.Emit.DynamicMethod.CreateDelegate%28System.Type%2CSystem.Object%29> Yöntem aşırı yüklemesini çağırarak Dynamic metodunu temsil eden temsilcinin bir örneğini (adım 1 ' de belirtilen) oluşturun. Temsilcinin oluşturulması yöntemi tamamlar ve yöntemi değiştirme girişimleri (örneğin, daha fazla MSIL ekleme) yok sayılır.  
    ...
    > Hedef türün diğer örneklerine <xref:System.Reflection.Emit.DynamicMethod.CreateDelegate%2A> bağlantılı temsilciler oluşturmak için yöntemi birden çok kez çağırabilirsiniz.  
     ...
     Yöntemin ilk `OneParameter` parametresi her zaman örneğini aldığından, temsilci kullanılır `Example`. Temsilci çağrıldığında yalnızca ikinci parametre gereklidir.  
...
## <a name="example"></a>Örnek  
 ...
 İkinci dinamik yöntemin türü `Example` ve türü `int` (`Integer` Visual Basic) olmak üzere iki parametresi vardır. Dinamik yöntem oluşturulduğunda, türünde `Example` `int`bir bağımsız değişkenine sahip olan genel bir temsilci kullanarak bir örneğine bağlanır. Yöntemin ilk parametresi her zaman ' ın `Example` `Example`bağlantılı örneğini aldığından, temsilcinin türünde bir bağımsız değişkeni yoktur. Temsilci çağrıldığında yalnızca `int` bağımsız değişken sağlanır. Bu dinamik yöntem, `Example` sınıfının özel bir alanına erişir ve özel alanın ve `int` bağımsız değişkenin çarpımını döndürür.  
