...
## <a name="custom-binding"></a>Özel Bağlama  
 ...
 Aşağıdaki örnek, bağımsız değişken türü dönüştürmesi sağlayan basit bir özel cildi gösterir. `Simple_Type.dll` için kod ana örnekle önce gelir. `Simple_Type.dll` oluşturup derleme zamanında projede buna bir başvuru eklediğinizden emin olun.  
 ...
 **BindToMethod** , bu tür bir çağrı mümkün değilse çağrılacak <xref:System.Reflection.MethodBase> veya null başvuru (Visual Basic**hiçbir şey** ) döndürür. **MethodBase** dönüş değeri, *eşleşme* parametresinde yer alan öğelerden biri olmalıdır, ancak bu durum olağan durumdur.  
...
|Başvuru olmayan tür|başvuru türü|  
 ...
 <xref:System.Type> sınıfı, belirli bir üyeye yönelik başvuruları çözümlemek için **Ciltçi** türünde parametreler kullanan **Get** yöntemlerine sahiptir. <xref:System.Type.GetConstructor%2A?displayProperty=nameWithType>, <xref:System.Type.GetMethod%2A?displayProperty=nameWithType>ve <xref:System.Type.GetProperty%2A?displayProperty=nameWithType> Bu üyeye ait imza bilgilerini sağlayarak geçerli türün belirli bir üyesini arama. <xref:System.Reflection.Binder.SelectMethod%2A?displayProperty=nameWithType> ve <xref:System.Reflection.Binder.SelectProperty%2A?displayProperty=nameWithType>, uygun yöntemlerin verilen imza bilgilerini seçmek için yeniden çağrılır.  
