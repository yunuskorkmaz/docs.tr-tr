...
## <a name="static-proxies"></a><span data-ttu-id="b062b-121">Statik proxy'ler</span><span class="sxs-lookup"><span data-stu-id="b062b-121">Static Proxies</span></span>  
 ...
 <span data-ttu-id="b062b-124">Çeşitli seçenekler, statik bir proxy nasıl çalıştığını denetler.</span><span class="sxs-lookup"><span data-stu-id="b062b-124">Several options control how a static proxy operates.</span></span> <span data-ttu-id="b062b-125">Aşağıdakileri belirtebilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="b062b-125">You can specify the following:</span></span>  
  
-   <span data-ttu-id="b062b-126">Proxy adresi.</span><span class="sxs-lookup"><span data-stu-id="b062b-126">The address of the proxy.</span></span>  
  
-   <span data-ttu-id="b062b-127">Proxy yerel adresler için atlanmasına.</span><span class="sxs-lookup"><span data-stu-id="b062b-127">Whether the proxy should be bypassed for local addresses.</span></span>  
  
-   <span data-ttu-id="b062b-128">Proxy için adresleri kümesini atlanmasına.</span><span class="sxs-lookup"><span data-stu-id="b062b-128">Whether the proxy should be bypassed for a set of addresses.</span></span>  
  
 <span data-ttu-id="b062b-129">Aşağıdaki tabloda, statik bir ara sunucu yapılandırma seçenekleri gösterilmektedir.</span><span class="sxs-lookup"><span data-stu-id="b062b-129">The following table shows the configuration options for a static proxy.</span></span>  
  
|<span data-ttu-id="b062b-130">Öznitelik, özellik veya yapılandırma dosyası ayarı</span><span class="sxs-lookup"><span data-stu-id="b062b-130">Attribute, property, or configuration file setting</span></span>|<span data-ttu-id="b062b-131">Açıklama</span><span class="sxs-lookup"><span data-stu-id="b062b-131">Description</span></span>|  
|--------------------------------------------------------|-----------------|  
|<span data-ttu-id="b062b-132">`proxyaddress` veya <xref:System.Net.WebProxy.Address></span><span class="sxs-lookup"><span data-stu-id="b062b-132">`proxyaddress` or <xref:System.Net.WebProxy.Address></span></span>|<span data-ttu-id="b062b-133">Kullanılacak proxy adresi.</span><span class="sxs-lookup"><span data-stu-id="b062b-133">The address of the proxy to use.</span></span>|  
|<span data-ttu-id="b062b-134">`bypassonlocal` veya <xref:System.Net.WebProxy.BypassProxyOnLocal></span><span class="sxs-lookup"><span data-stu-id="b062b-134">`bypassonlocal` or <xref:System.Net.WebProxy.BypassProxyOnLocal></span></span>|<span data-ttu-id="b062b-135">Yerel adresler için proxy atlanır olup olmadığını denetler.</span><span class="sxs-lookup"><span data-stu-id="b062b-135">Controls whether the proxy is bypassed for local addresses.</span></span>|  
|<span data-ttu-id="b062b-136">`bypasslist` veya <xref:System.Net.WebProxy.BypassArrayList></span><span class="sxs-lookup"><span data-stu-id="b062b-136">`bypasslist` or <xref:System.Net.WebProxy.BypassArrayList></span></span>|<span data-ttu-id="b062b-137">, Normal ifadeler ile bir proxy atlama adresleri kümesini açıklar.</span><span class="sxs-lookup"><span data-stu-id="b062b-137">Describes, with regular expressions, a set of addresses that bypass the proxy.</span></span>|  
|`usesystemdefault`|<span data-ttu-id="b062b-138">Kullanıcı için Internet Explorer proxy ayarlarını (proxy adresi atlama listesine ve yerel atlama) statik proxy ayarlarını gerekip gerekmediğini okunması denetler.</span><span class="sxs-lookup"><span data-stu-id="b062b-138">Controls whether the static proxy settings (proxy address, bypass list, and bypass on local) should be read from the Internet Explorer proxy settings for the user.</span></span> <span data-ttu-id="b062b-139">Bu değer ayarlanırsa `true`, Internet Explorer statik proxy ayarları kullanılacaktır.</span><span class="sxs-lookup"><span data-stu-id="b062b-139">If this value is set to `true`, then the static proxy settings from Internet Explorer will be used.</span></span> <span data-ttu-id="b062b-140">Bu değer ayarlandığında .NET Framework 2.0 `true`, Internet Explorer proxy ayarlarını yapılandırma dosyasındaki diğer proxy ayarları geçersiz kılınmaz.</span><span class="sxs-lookup"><span data-stu-id="b062b-140">On .NET Framework 2.0 when this value is set to `true`, the Internet Explorer proxy settings are not overridden by other proxy settings in the configuration file.</span></span> <span data-ttu-id="b062b-141">.NET Framework 1.1, Internet Explorer proxy ayarlarının diğer proxy ayarlarını yapılandırma dosyasındaki kılınabilir.</span><span class="sxs-lookup"><span data-stu-id="b062b-141">On .NET Framework 1.1, the Internet Explorer proxy settings can be overridden by other proxy settings in the configuration file.</span></span><br /><br /> <span data-ttu-id="b062b-142">Bu değer ise `false` veya ayarlanmadı, ardından statik proxy ayarlarını yapılandırmada belirtilen ve Internet Explorer proxy ayarlarını geçersiz kılar.</span><span class="sxs-lookup"><span data-stu-id="b062b-142">If this value is `false` or not set, then the static proxy settings can be specified in the configuration and will override the Internet Explorer proxy settings.</span></span> <span data-ttu-id="b062b-143">Bu değer ayrıca ayarlanmalıdır `false` veya uyarlamalı proxy'leri etkinleştirilmesi için ayarlanmadı.</span><span class="sxs-lookup"><span data-stu-id="b062b-143">This value must also be set to `false` or not set for adaptive proxies to be enabled.</span></span>|  
  
 <span data-ttu-id="b062b-144">Aşağıdaki örnek bir genel statik proxy yapılandırmasını gösterir.</span><span class="sxs-lookup"><span data-stu-id="b062b-144">The following example shows a typical static proxy configuration.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="b062b-145">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="b062b-145">See also</span></span>
