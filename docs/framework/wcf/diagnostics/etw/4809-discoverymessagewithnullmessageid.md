---
title: 4809 - DiscoveryMessageWithNullMessageId
ms.date: 03/30/2017
ms.assetid: d8947028-ae7e-45a2-a438-43162894519e
ms.openlocfilehash: 92ec4466e9c92d81f89ec0efbcc3abc67f10b105
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="4809---discoverymessagewithnullmessageid"></a>4809 - DiscoveryMessageWithNullMessageId
## <a name="properties"></a>Özellikler  
  
|||  
|-|-|  
|Kimlik|4809|  
|Anahtar Sözcükler|Bulma|  
|Düzey|Uyarı|  
|Kanal|Microsoft Windows uygulaması sunucu-uygulamalar/hata ayıklama|  
  
## <a name="description"></a>Açıklama  
 İleti üstbilgisi gerekli MessageID özelliği içermediğinden bulma ileti bırakıldı olduğunda bu olay yayınlanır.  
  
## <a name="message"></a>İleti  
 İleti üstbilgisi gerekli MessageID özelliği içermediğinden bir %1 ileti bırakıldı.  
  
## <a name="details"></a>Ayrıntılar
