---
title: 3825 - RoutingServiceAbandoningReceiveContext
ms.date: 03/30/2017
ms.assetid: 3e16528e-d340-4245-89b2-f8ac16621e94
ms.openlocfilehash: b0b88ae5fcb187d588f098ea17905e706c8fbecd
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33465997"
---
# <a name="3825---routingserviceabandoningreceivecontext"></a>3825 - RoutingServiceAbandoningReceiveContext
## <a name="properties"></a>Özellikler  
  
|||  
|-|-|  
|Kimlik|3825|  
|Anahtar Sözcükler|RoutingServices|  
|Düzey|Uyarı|  
|Kanal|Microsoft Windows uygulaması sunucu-uygulamalar/hata ayıklama|  
  
## <a name="description"></a>Açıklama  
 Yönlendirme hizmeti belirtilen kimliğe sahip bir ileti için ReceiveContext.Abandon çağrılırken bu olay yayılan  
  
## <a name="message"></a>İleti  
 Yönlendirme hizmeti ReceiveContext.Abandon iletinin kimliği çağırma: '%1'.  
  
## <a name="details"></a>Ayrıntılar
