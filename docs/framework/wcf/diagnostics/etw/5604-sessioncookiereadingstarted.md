---
title: 5604 - SessionCookieReadingStarted
ms.date: 03/30/2017
ms.assetid: 7348caaa-add0-473d-8af3-b2dadb68abdb
ms.openlocfilehash: 549ece457f4c4498f38a7c19cf4d31137fbe7eff
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="5604---sessioncookiereadingstarted"></a>5604 - SessionCookieReadingStarted
## <a name="properties"></a>Özellikler  
  
|||  
|-|-|  
|Kimlik|5604|  
|Anahtar Sözcükler|Güvenlik|  
|Düzey|Ayrıntılı|  
|Kanal|Microsoft Windows uygulaması sunucu-uygulamalar/hata ayıklama|  
  
## <a name="description"></a>Açıklama  
 Bu olay gösterilen zaman okuma oturum başlatılan oturum tanımlama bilgisini kullanarak belirteci.  
  
## <a name="message"></a>İleti  
 Oturum belirteci oturum tanımlama bilgisinden okuma başladı.  
  
## <a name="details"></a>Ayrıntılar
