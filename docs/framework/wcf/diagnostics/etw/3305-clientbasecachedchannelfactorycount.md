---
title: 3305 - ClientBaseCachedChannelFactoryCount
ms.date: 03/30/2017
ms.assetid: 6ae6f58f-cee3-4bb1-8402-4445401f3436
ms.openlocfilehash: 1012d36b8dedbdecbf33b9e0c745e07373bb3677
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="3305---clientbasecachedchannelfactorycount"></a>3305 - ClientBaseCachedChannelFactoryCount
## <a name="properties"></a>Özellikler  
  
|||  
|-|-|  
|Kimlik|3305|  
|Anahtar Sözcükler|ServiceModel|  
|Düzey|Bilgiler|  
|Kanal|Microsoft Windows uygulaması sunucu-uygulamalar/hata ayıklama|  
  
## <a name="description"></a>Açıklama  
 Bu olay, önbelleğe alınmış istemci temel kanal fabrikası sayısı bildirmek için yayınlanır.  
  
## <a name="message"></a>İleti  
 Önbelleğe alınmış kanal fabrikaları sayısı: '%1'.  En çok '%2' kanal fabrikaları önbelleğe alınabilir.  
  
## <a name="details"></a>Ayrıntılar
