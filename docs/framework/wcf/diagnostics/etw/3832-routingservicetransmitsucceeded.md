---
title: 3832 - RoutingServiceTransmitSucceeded
ms.date: 03/30/2017
ms.assetid: 72e0c6b0-7ed2-4a48-aaa0-d5a761a9a469
ms.openlocfilehash: a5ba9f0c39b6b48ccefc374adb3e45a09af20e0a
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33466433"
---
# <a name="3832---routingservicetransmitsucceeded"></a>3832 - RoutingServiceTransmitSucceeded
## <a name="properties"></a>Özellikler  
  
|||  
|-|-|  
|Kimlik|3832|  
|Anahtar Sözcükler|RoutingServices|  
|Düzey|Bilgiler|  
|Kanal|Microsoft Windows uygulaması sunucu-uygulamalar/hata ayıklama|  
  
## <a name="description"></a>Açıklama  
 Yönlendirme hizmeti başarıyla belirtilen kimliğe sahip ileti aktarımı sırasında bu olay yayılan  
  
## <a name="message"></a>İleti  
 Yönlendirme hizmeti başarıyla iletilen iletinin kimliği: ' %1 [işlem %2] '% 3'.  
  
## <a name="details"></a>Ayrıntılar
