---
title: ServiceAuthorizationFailure
ms.date: 03/30/2017
ms.assetid: a6a57896-41d5-4c05-a2fb-184d828e20c1
ms.openlocfilehash: 5756e166041920cbf66cd1fd9e9d826f85dc5c07
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33471419"
---
# <a name="serviceauthorizationfailure"></a>ServiceAuthorizationFailure
Kimliği: 168  
  
 Önem derecesi: hata  
  
 Kategori: SecurityAudit  
  
## <a name="description"></a>Açıklama  
 Bu olay, hizmet yetkilendirme başarısız olduğunu gösterir. Olay hizmeti, eylem, istemci kimliği, yetkilendirme bağlamı, etkinlik kimliği ve ServiceAuthorizationManager listeler.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Günlüğe Olay Kaydetme](../../../../../docs/framework/wcf/diagnostics/event-logging/index.md)  
 [Etkinlik Genel Başvurusu](../../../../../docs/framework/wcf/diagnostics/event-logging/events-general-reference.md)
