---
title: Windows Communication Foundation Uç Noktaları
ms.date: 03/30/2017
helpviewer_keywords:
- endpoints [WCF]
ms.assetid: bd0c310f-dd9f-4081-9be2-3db5909850b6
ms.openlocfilehash: 1a18e2ab31998b7759803e023151892694757119
ms.sourcegitcommit: 2701302a99cafbe0d86d53d540eb0fa7e9b46b36
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/28/2019
ms.locfileid: "64613391"
---
# <a name="windows-communication-foundation-endpoints"></a>Windows Communication Foundation Uç Noktaları
Bir Windows Communication Foundation (WCF) hizmetiyle kurulan tüm iletişimlerde üzerinden gerçekleşir *uç noktaları* hizmeti. Uç noktaları, istemcilerin bir WCF hizmeti sunan işlevine erişim sağlar.  
  
 Bir uç nokta oluşturma hakkında genel bir bakış için bkz. [uç nokta oluşturmaya genel bakış](../../../docs/framework/wcf/endpoint-creation-overview.md). Her uç nokta içerir:  
  
- Uç nokta yeri belirten bir adres.  
  
- Bir bağlama için bir istemci uç noktasıyla nasıl iletişim kurabilir belirtir.  
  
- Kullanılabilir yöntemleri tanımlayan bir sözleşme.  
  
 Bu ayrı bir uç nokta bölümlerini belirtme hakkında daha fazla açıklamaları için bkz:  
  
- [Uç Nokta Adresi Belirtme](../../../docs/framework/wcf/specifying-an-endpoint-address.md)  
  
- [Hizmetler ve İstemcileri Yapılandırmak için Bağlamaları Kullanma](../../../docs/framework/wcf/using-bindings-to-configure-services-and-clients.md)  
  
- [Hizmetleri Tasarlama ve Uygulama](../../../docs/framework/wcf/designing-and-implementing-services.md)  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Uç Nokta Oluşturmaya Genel Bakış](../../../docs/framework/wcf/endpoint-creation-overview.md)  
 Bir uç nokta yapısını açıklar ve varsayılan uç noktaları, bağlamalar ve davranışları kullanmak için çalışma zamanı tarafından sağlanan nasıl bir uç nokta yapılandırması ve kodda nasıl tanımlanacağını de açıklar.  
  
 [Uç Nokta Adresi Belirtme](../../../docs/framework/wcf/specifying-an-endpoint-address.md)  
 Bir WCF Hizmeti ile iletişimi uç noktaları yoluyla ne olacağı açıklanır.  
  
 [Nasıl yapılır: Yapılandırma içinde hizmet uç noktası oluşturma](../../../docs/framework/wcf/feature-details/how-to-create-a-service-endpoint-in-configuration.md)  
 Yapılandırma içinde hizmet uç noktası oluşturma işlemi gösterilmektedir.  
  
 [Nasıl yapılır: Kod içinde hizmet uç noktası oluşturma](../../../docs/framework/wcf/feature-details/how-to-create-a-service-endpoint-in-code.md)  
 Kod içinde hizmet uç noktası oluşturma işlemi gösterilmektedir.  
  
 [Meta Veri Uç Noktalarını Yayımlama](../../../docs/framework/wcf/publishing-metadata-endpoints.md)  
 Yapılandırma ve kod meta veri uç noktalarını yayımlama meta veri yayımlama işlemini göstermektedir.  
  
## <a name="reference"></a>Başvuru  
 <xref:System.ServiceModel.EndpointAddress>  
  
## <a name="related-sections"></a>İlgili Bölümler  
 [Temel Programlama Yaşam Döngüsü](../../../docs/framework/wcf/basic-programming-lifecycle.md)
