...
-   <xref:System.ServiceModel.IContextChannel> – Her durum Kanallar ve WCF çalışma zamanı tarafından oluşturulmuş proxy'leri için denetleme bu arabirim sağlar.  Bu sınıf uzantıları ya da davranışını genişletmek <xref:System.ServiceModel.IClientChannel> veya her kanal için durumu depolamak için kullanabilirsiniz.  
