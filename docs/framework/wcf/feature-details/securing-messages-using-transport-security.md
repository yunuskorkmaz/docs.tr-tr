...
>  Bu konuyu okumadan önce okumanız önerilir [güvenlik kavramları](../../../../docs/framework/wcf/feature-details/security-concepts.md).  
 ...
 Aşağıdaki çizimde Windows Communication Foundation (WCF) kullanarak kuyruğa alınan iletişim kavramsal modeli sağlar. Bu çizim ve terminolojisi Aktarım güvenlik kavramları açıklamak için kullanılır.  
 ...
 Ne zaman gönderme sıraya alınan iletileri WCF ile kullanarak <xref:System.ServiceModel.NetMsmqBinding>, WCF iletinin MSMQ İleti gövdesi olarak eklenir. Taşıma güvenliği MSMQ iletinin tamamını (MSMQ ileti üstbilgilerini veya özellikleri ve ileti gövdesini) güvenliğini sağlar. MSMQ İleti gövdesini olduğu için taşıma güveliği kullanarak da WCF ileti güvenliğini sağlar.  
...
## <a name="wcf-msmq-transport-security-properties"></a>WCF MSMQ taşıma güvenlik özellikleri  
 ...
 WCF MSMQ taşıma güvenliği bir parçası olarak bu iki seçeneği sağlar ve bunlar taşıma güvenliği için anahtar Özet.  
...
#### <a name="certificate-authentication-mode"></a>Sertifika kimlik doğrulama modu  
 ...
 İçeren bir WCF ileti gönderirken <xref:System.ServiceModel.MsmqAuthenticationMode.Certificate>, WCF kanalı MSMQ iletisi için bir Windows SID'ye eklemez. Bu nedenle, hedef sıra ACL için izin vermelidir `Anonymous` sıraya göndermek için kullanıcı erişimi. Alıcı Sıra Yöneticisi MSMQ iletisi sertifikayla imzalanmış olmasına rağmen herhangi bir kimlik doğrulaması gerçekleştirmez olup olmadığını denetler.  
  
 İçinde sertifika talep ve kimlik bilgileri ile doldurulur <xref:System.ServiceModel.ServiceSecurityContext> WCF sıraya alınan aktarım kanala göre. Hizmet, gönderenin kendi kimlik doğrulaması gerçekleştirmek için bu bilgileri kullanabilirsiniz.  
