...
---
# <a name="how-to-access-wcf-services-with-one-way-and-request-reply-contracts"></a><span data-ttu-id="68bfa-102">Nasıl yapılır: WCF hizmetlerine tek yönlü ve istek-yanıt sözleşmeleriyle</span><span class="sxs-lookup"><span data-stu-id="68bfa-102">How to: Access WCF Services with One-Way and Request-Reply Contracts</span></span>
<span data-ttu-id="68bfa-103">Aşağıdaki yordamlar, bir Windows Communication Foundation (WCF) hizmetine erişmek için tek yönlü sözleşme ve istek-yanıt sözleşmesi tanımlayan ve çift yönlü iletişim deseni kullanmaz nasıl açıklar.</span><span class="sxs-lookup"><span data-stu-id="68bfa-103">The following procedures describe how to access a Windows Communication Foundation (WCF) service that defines a one-way contract and a request-reply contract and that does not use the duplex communication pattern.</span></span>  
  
### <a name="to-define-the-service"></a><span data-ttu-id="68bfa-104">Hizmet tanımlama</span><span class="sxs-lookup"><span data-stu-id="68bfa-104">To define the service</span></span>  
  
1.  <span data-ttu-id="68bfa-105">Hizmet sözleşmesi bildirin.</span><span class="sxs-lookup"><span data-stu-id="68bfa-105">Declare the service contract.</span></span> <span data-ttu-id="68bfa-106">Tek yönlü olarak işlemleri olmalıdır `IsOneWay` kümesine `true` içinde <xref:System.ServiceModel.OperationContractAttribute>.</span><span class="sxs-lookup"><span data-stu-id="68bfa-106">The operations that are to be one-way must have `IsOneWay` set to `true` within the <xref:System.ServiceModel.OperationContractAttribute>.</span></span> <span data-ttu-id="68bfa-107">Aşağıdaki kod bildirir `IOneWayCalculator` için tek yönlü işlem var sözleşme `Add`, `Subtract`, `Multiply`, ve `Divide`.</span><span class="sxs-lookup"><span data-stu-id="68bfa-107">The following code declares the `IOneWayCalculator` contract that has one-way operations for `Add`, `Subtract`, `Multiply`, and `Divide`.</span></span> <span data-ttu-id="68bfa-108">Ayrıca adlı bir istek yanıt işlem tanımlar `SayHello`.</span><span class="sxs-lookup"><span data-stu-id="68bfa-108">It also defines a request response operation called `SayHello`.</span></span>  
...
2.  <span data-ttu-id="68bfa-109">Hizmet sözleşmesini uygulama.</span><span class="sxs-lookup"><span data-stu-id="68bfa-109">Implement the service contract.</span></span> <span data-ttu-id="68bfa-110">Aşağıdaki kod uygulayan `IOnewayCalculator` arabirimi.</span><span class="sxs-lookup"><span data-stu-id="68bfa-110">The following code implements the `IOnewayCalculator` interface.</span></span>  
...
3.  <span data-ttu-id="68bfa-111">Hizmeti bir konsol uygulamasında barındırır.</span><span class="sxs-lookup"><span data-stu-id="68bfa-111">Host the service in a console application.</span></span> <span data-ttu-id="68bfa-112">Aşağıdaki kod, hizmeti barındırmak nasıl gösterir.</span><span class="sxs-lookup"><span data-stu-id="68bfa-112">The following code shows how to host the service.</span></span>  
...
1.  <span data-ttu-id="68bfa-114">Çalıştırma [ServiceModel meta veri yardımcı Programracı (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) şu komut satırını kullanarak hizmeti istemci sınıf oluşturmak için meta veri değişimi uç nokta adresini kullanarak: `Svcutil http://localhost:8000/Service` [ServiceModel meta veri yardımcı Programracı (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) aşağıdaki örnek kodda gösterildiği gibi arabirimler ve sınıflar, bir dizi oluşturur.</span><span class="sxs-lookup"><span data-stu-id="68bfa-114">Run the [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) using the metadata exchange endpoint address to create the client class for the service using the following command line: `Svcutil http://localhost:8000/Service` The [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) generates a set of interfaces and classes, as shown in the following sample code.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="68bfa-115">İçinde fark `IOneWayCalculator` tek yönlü hizmet işlemleri sahip arabirimi <xref:System.ServiceModel.OperationContractAttribute.IsOneWay%2A> özniteliğini `true` ve istek-yanıt hizmeti işlemi varsayılan değere ayarlanmış özniteliğine sahip `false`.</span><span class="sxs-lookup"><span data-stu-id="68bfa-115">Notice in the `IOneWayCalculator` interface that the one-way service operations have the <xref:System.ServiceModel.OperationContractAttribute.IsOneWay%2A> attribute set to `true` and the request-reply service operation has the attribute set to the default value, `false`.</span></span> <span data-ttu-id="68bfa-116">Ayrıca `OneWayCalculatorClient` sınıfı.</span><span class="sxs-lookup"><span data-stu-id="68bfa-116">Also notice the `OneWayCalculatorClient` class.</span></span> <span data-ttu-id="68bfa-117">Bu hizmeti çağırmak için kullanacağınız sınıftır.</span><span class="sxs-lookup"><span data-stu-id="68bfa-117">This is the class that you will use to call the service.</span></span>  
  
2.  <span data-ttu-id="68bfa-118">İstemci nesnesi oluşturun.</span><span class="sxs-lookup"><span data-stu-id="68bfa-118">Create the client object.</span></span>  
...
3.  <span data-ttu-id="68bfa-119">Hizmet işlemleri çağırın.</span><span class="sxs-lookup"><span data-stu-id="68bfa-119">Call service operations.</span></span>  
...
4.  <span data-ttu-id="68bfa-120">Bağlantıları kapatın ve kaynakları temizlemek için istemciyi kapatın.</span><span class="sxs-lookup"><span data-stu-id="68bfa-120">Close the client to close connections and clean up resources.</span></span>  
...
## <a name="example"></a><span data-ttu-id="68bfa-121">Örnek</span><span class="sxs-lookup"><span data-stu-id="68bfa-121">Example</span></span>  
 <span data-ttu-id="68bfa-122">Bu konu başlığında kullanılan kod tam listesi verilmiştir.</span><span class="sxs-lookup"><span data-stu-id="68bfa-122">The following is a complete listing of the code used  in this topic.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="68bfa-123">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="68bfa-123">See also</span></span>
- [<span data-ttu-id="68bfa-124">Tek Yönlü Hizmetler</span><span class="sxs-lookup"><span data-stu-id="68bfa-124">One-Way Services</span></span>](../../../../docs/framework/wcf/feature-details/one-way-services.md)
