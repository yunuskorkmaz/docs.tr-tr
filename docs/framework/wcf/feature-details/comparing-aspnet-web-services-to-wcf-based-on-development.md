...
|<span data-ttu-id="4e9fb-168">Ad</span><span class="sxs-lookup"><span data-stu-id="4e9fb-168">Name</span></span>|<span data-ttu-id="4e9fb-169">Amaç</span><span class="sxs-lookup"><span data-stu-id="4e9fb-169">Purpose</span></span>|
|----------|-------------|
...
### <a name="security-authentication"></a><span data-ttu-id="4e9fb-290">Güven Kimlik doğrulaması</span><span class="sxs-lookup"><span data-stu-id="4e9fb-290">Security: Authentication</span></span>
...
<span data-ttu-id="4e9fb-322">Talep tabanlı yetkilendirmeyi çalıştırmak için kullanılan sınıflar, <xref:System.ServiceModel.ServiceAuthorizationManager> `AccessCheck()`geçersiz kılmak için yalnızca bir yöntemi olan öğesinden türetilmelidir.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-322">Classes used to run claims-based authorization must derive from <xref:System.ServiceModel.ServiceAuthorizationManager>, which has just one method to override, `AccessCheck()`.</span></span> <span data-ttu-id="4e9fb-323">WCF, her bir hizmet işlemi çağrıldığında bu yöntemi çağırır ve, <xref:System.ServiceModel.OperationContext> `ServiceSecurityContext.AuthorizationContext` özelliği içinde Kullanıcı için talepler içeren bir nesne sağlar.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-323">WCF calls that method whenever an operation of the service is invoked and provides a <xref:System.ServiceModel.OperationContext> object, which has the claims for the user in its `ServiceSecurityContext.AuthorizationContext` property.</span></span> <span data-ttu-id="4e9fb-324">WCF, kullanıcı hakkındaki talepleri, kullanıcının kimlik doğrulaması için verdiği herhangi bir güvenlik belirteciyle, bu taleplerin söz konusu işlem için yeterli olup olmadığını değerlendirme görevini atan bir kullanıcı hakkında bilgi veren çalışmalardır.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-324">WCF does the work of assembling claims about the user from whatever security token the user provided for authentication, which leaves the of task of evaluating whether those claims suffice for the operation in question.</span></span>

<span data-ttu-id="4e9fb-325">Bu WCF, her türlü güvenlik belirtecinin taleplerini otomatik olarak ayrıştırır. Bu, talebe dayalı olarak kimlik doğrulama mekanizmasından bağımsız olarak yetkilendirme kodu oluşturduğundan yüksek ölçüde önemli bir yeniliği oluşturur.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-325">That WCF automatically assembles claims from any kind of security token is a highly significant innovation, because it makes the code for authorization based on the claims entirely independent of the authentication mechanism.</span></span> <span data-ttu-id="4e9fb-326">Bunun aksine, ASP.NET içindeki ACL 'Leri veya rolleri kullanarak yetkilendirme, Windows kimlik doğrulamasına yakından bağlıdır.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-326">By contrast, authorization using ACLs or roles in ASP.NET is closely tied to Windows authentication.</span></span>

### <a name="security-confidentiality"></a><span data-ttu-id="4e9fb-327">Güven Gizlilik</span><span class="sxs-lookup"><span data-stu-id="4e9fb-327">Security: Confidentiality</span></span>

<span data-ttu-id="4e9fb-328">ASP.NET Web hizmetleriyle değiştirilen iletilerin gizliliği, IIS içindeki uygulamayı Güvenli Köprü Metni Aktarım Protokolü (HTTPS) kullanacak şekilde yapılandırarak Aktarım düzeyinde olabilir.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-328">The confidentiality of messages exchanged with ASP.NET Web services can be ensured at the transport level by configuring the application within IIS to use the Secure Hypertext Transfer Protocol (HTTPS).</span></span> <span data-ttu-id="4e9fb-329">IIS içinde barındırılan WCF uygulamaları için de aynı işlem yapılabilir.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-329">The same can be done for WCF applications hosted within IIS.</span></span> <span data-ttu-id="4e9fb-330">Ancak, IIS dışında barındırılan WCF uygulamaları da güvenli bir aktarım protokolü kullanacak şekilde yapılandırılabilir.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-330">However, WCF applications hosted outside of IIS can also be configured to use a secure transport protocol.</span></span> <span data-ttu-id="4e9fb-331">Daha önemli olan WCF uygulamaları, WS-Security protokolü kullanılarak, gönderilmeden önce iletilerin güvenliğini sağlamak için de yapılandırılabilir.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-331">More important, WCF applications can also be configured to secure the messages before they are transported, using the WS-Security protocol.</span></span> <span data-ttu-id="4e9fb-332">WS-Security kullanarak yalnızca bir iletinin gövdesini güvenli hale getirmek, BT 'nin son hedefine ulaşmadan önce aracıların genelinde confidentially aktarılmasını sağlar.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-332">Securing just the body of a message using WS-Security allows it to be transmitted confidentially across intermediaries before reaching its final destination.</span></span>

## <a name="globalization"></a><span data-ttu-id="4e9fb-333">Genelleştirme</span><span class="sxs-lookup"><span data-stu-id="4e9fb-333">Globalization</span></span>

<span data-ttu-id="4e9fb-334">ASP.NET yapılandırma dili, tek tek hizmetler için kültürü belirtmenize olanak tanır.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-334">The ASP.NET configuration language allows you to specify the culture for individual services.</span></span> <span data-ttu-id="4e9fb-335">WCF, ASP.NET uyumluluk modu dışında bu yapılandırma ayarını desteklemez.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-335">The WCF does not support that configuration setting except in ASP.NET compatibility mode.</span></span> <span data-ttu-id="4e9fb-336">ASP.NET uyumluluk modunu kullanmayan bir WCF hizmetini yerelleştirmek için, hizmet türünü kültüre özgü Derlemelerle derleyin ve kültüre özgü her derleme için kültüre özgü ayrı uç noktalara sahip yapın.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-336">To localize a WCF service that does not use ASP.NET compatibility mode, compile the service type into culture-specific assemblies, and have separate culture-specific endpoints for each culture-specific assembly.</span></span>

## <a name="see-also"></a><span data-ttu-id="4e9fb-337">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="4e9fb-337">See also</span></span>

- [<span data-ttu-id="4e9fb-338">ASP.NET Web Hizmetlerini Amaç ve Kullanılan Standartları Temel Alarak WCF ile Karşılaştırma</span><span class="sxs-lookup"><span data-stu-id="4e9fb-338">Comparing ASP.NET Web Services to WCF Based on Purpose and Standards Used</span></span>](../../../../docs/framework/wcf/feature-details/comparing-aspnet-web-services-to-wcf-based-on-purpose-and-standards-used.md)
