---
title: "Güvenlik Kılavuzu ve En İyi Uygulamalar"
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology:
- dotnet-clr
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- secure applications [WCF]
ms.assetid: 79e4e6f7-0c00-4045-b0f0-e6a7c37c7788
caps.latest.revision: 
author: BrucePerlerMS
ms.author: bruceper
manager: mbaldwin
ms.workload:
- dotnet
ms.openlocfilehash: b81a7d8aa46a96fde1285e182e5080732bbef7b4
ms.sourcegitcommit: 16186c34a957fdd52e5db7294f291f7530ac9d24
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/22/2017
---
# <a name="security-guidance-and-best-practices"></a>Güvenlik Kılavuzu ve En İyi Uygulamalar
Bu bölümdeki konular, sunmak için yönergeler ve güvenli oluştururken dikkate alınması gereken öğeleri [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] uygulamalar. [!INCLUDE[crabout](../../../../includes/crabout-md.md)]Windows Server AppFabric ve bakın, güvenlik [güvenlik modeli için Windows Server App Fabric](http://go.microsoft.com/fwlink/?LinkID=201279&clcid=0x409)  
  
## <a name="in-this-section"></a>Bu Bölümdekiler  
 [Güvenlik için En İyi Uygulamalar](../../../../docs/framework/wcf/feature-details/best-practices-for-security-in-wcf.md)  
 Uygulamalarınızın güvenliğini sağlamanıza yardımcı olacak öneriler sunar.  
  
 [Güvenlik Konuları](../../../../docs/framework/wcf/feature-details/security-considerations-in-wcf.md)  
 Öğeleri oluştururken dikkate alınması gereken bir [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] uygulama.  
  
 [Performans Konuları](../../../../docs/framework/wcf/feature-details/performance-considerations.md)  
 Performans ve güvenlik ile ilgili girdiler içeriyor.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Temel WCF Programlama](../../../../docs/framework/wcf/basic-wcf-programming.md)  
 [Windows Server App Fabric için güvenlik modeli](http://go.microsoft.com/fwlink/?LinkID=201279&clcid=0x409)
