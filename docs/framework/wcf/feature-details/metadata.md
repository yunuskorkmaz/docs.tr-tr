---
title: Meta Veriler
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology: dotnet-clr
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords: metadata [WCF]
ms.assetid: 66a61bd0-18d3-4e7e-bf8b-177a10ac2f60
caps.latest.revision: "10"
author: dotnet-bot
ms.author: dotnetcontent
manager: wpickett
ms.workload: dotnet
ms.openlocfilehash: 8e4b0a724653f5366d6da96f511fac9e62f64816
ms.sourcegitcommit: 16186c34a957fdd52e5db7294f291f7530ac9d24
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/22/2017
---
# <a name="metadata"></a>Meta Veriler
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] Dışarı aktarma, yayımlama, alma ve hizmet meta verileri içe aktarma için bir altyapı sağlar. [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]Hizmetleri, böylece Svcutil.exe gibi araçları hizmete erişim için istemci kodu otomatik olarak oluşturabilir hizmetin uç ile etkileşim kurmak nasıl açıklamak için meta verileri kullanın.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Meta Veri Mimarisine Genel Bakış](../../../../docs/framework/wcf/feature-details/metadata-architecture-overview.md)  
 Bir üst düzey genel bakış meta mimarisi.  
  
 [Meta Veri Biçimleri](../../../../docs/framework/wcf/feature-details/metadata-formats.md)  
 Farklı meta veri biçimleri açıklanır.  
  
 [Meta Verileri Dışarı ve İçeri Aktarma](../../../../docs/framework/wcf/feature-details/exporting-and-importing-metadata.md)  
 Meta verileri içeri ve dışarı aktarmak açıklar.  
  
 [Meta Verileri Yayımlama](../../../../docs/framework/wcf/feature-details/publishing-metadata.md)  
 Açıklar nasıl [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] meta veriler yayımlar.  
  
 [Meta Verileri Alma](../../../../docs/framework/wcf/feature-details/retrieving-metadata.md)  
 Meta verilerini almak için farklı yöntemler açıklanmaktadır.  
  
 [Meta Verileri Kullanma](../../../../docs/framework/wcf/feature-details/using-metadata.md)  
 Hizmet meta verilerini kullanmak için farklı yollar açıklanmaktadır.  
  
 [Meta Veriler Hakkında Güvenlik Konuları](../../../../docs/framework/wcf/feature-details/security-considerations-with-metadata.md)  
 Hizmet meta verileri ile ilgilenirken önemli güvenlik konuları açıklanmaktadır.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Meta Veri Sistemini Genişletme](../../../../docs/framework/wcf/extending/extending-the-metadata-system.md)
