...
- Bir IIS/WAS sanal dizininde iş akışı hizmetini tanımlayan bir. xamlx dosyası yerleştirin. . Xamlx dosyası, kullanıma sunulacak uç noktaları belirtir. Uç noktalar, aşağıdaki örnekte gösterildiği gibi bir `WorkflowService.Endpoints` öğesinde belirtilir.
...
## <a name="referencing-custom-activities"></a>Özel etkinliklere başvurma
 Özel etkinliklere yapılan başvuruların, uygulama etki alanına yüklenmesi ve XAML seri hale getiricisi türleri bulabilmesi için <`System.Web.Compilation`> altındaki <`Assemblies`> bölümüne eklenmesi gerekir. Bu ayarlar, uygulama düzeyinde veya ayarların makinedeki tüm uygulamalara uygulanması gerekiyorsa kök Web. config ' de yapılabilir.
