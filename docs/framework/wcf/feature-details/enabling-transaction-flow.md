...
Windows Communication Foundation (WCF) işlem akışını denetlemek için yüksek oranda esnek seçenekler sunar. Bir hizmetin işlem akışı ayarlarını öznitelikleri ve yapılandırma birleşimini kullanarak ifade edilebilir.  
...
|İzin verilen|true|OleTransactions|İşlem WCF OleTransactions biçiminde akışı yapılan işlem.|  
...
## <a name="enabling-transaction-flow-at-the-endpoint-level"></a>Uç nokta düzeyinde işlem akışını etkinleştirme  
 ...
 WCF sağlar içeren konumu bağlamaları çoğu `transactionFlow` ve `transactionProtocol` gelen işlemler kabul etmek için belirli bağlama yapılandırmanızı sağlayan öznitelikler. Yapılandırma öğelerini ayarlama hakkında daha fazla bilgi için bkz: [ \<bağlama >](../../../../docs/framework/misc/binding.md).  
...
## <a name="security"></a>Güvenlik  
 ...
 Bilinmeyen veya güvenilmeyen Web hizmeti meta veri değişimi kullanımı ile WCF istemcilere oluştururken, bu Web hizmetleri üzerinde işlem çağrıları geçerli işlem mümkünse bastırmak. Aşağıdaki örnek bunun nasıl yapılacağını göstermektedir.  
