---
title: Başarım Değerlendirmeleri
ms.date: 03/30/2017
ms.assetid: 83082d4a-7350-4b04-ab2d-1fa5ffd643f5
ms.openlocfilehash: 46e5b5c392ff3a0d401bb709b21637b3d2e784ab
ms.sourcegitcommit: 5b6d778ebb269ee6684fb57ad69a8c28b06235b9
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/08/2019
ms.locfileid: "59139743"
---
# <a name="performance-considerations"></a>Başarım Değerlendirmeleri
Performansla ilgili önemli noktalar, her zaman uygulamalar oluştururken önemlidir. Bu bölümde, performans ve güvenlik ile ilgili girdiler içeriyor.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Kimlik Modeliyle Beyanlar ve Yetkilendirmeyi Yönetme](../../../../docs/framework/wcf/feature-details/managing-claims-and-authorization-with-the-identity-model.md)  
 Talepler bir talep koleksiyonda bulma için en hızlı mekanizması açıklanmaktadır.  
  
 [Dijital İmzaların Şifrelenmesi](../../../../docs/framework/wcf/feature-details/encryption-of-digital-signatures.md)  
 Dijital imza şifreleme bir performans cezası nasıl ekleyebilir ve bunu devre dışı bırakma açıklar.  
  
## <a name="see-also"></a>Ayrıca bkz.

- [Güvenlik için En İyi Uygulamalar](../../../../docs/framework/wcf/feature-details/best-practices-for-security-in-wcf.md)
- [Güvenlik Değerlendirmeleri](../../../../docs/framework/wcf/feature-details/security-considerations-in-wcf.md)
