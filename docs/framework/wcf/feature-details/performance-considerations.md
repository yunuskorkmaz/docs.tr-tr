---
title: Başarım Değerlendirmeleri
ms.date: 03/30/2017
ms.assetid: 83082d4a-7350-4b04-ab2d-1fa5ffd643f5
ms.openlocfilehash: 78daff1e0cf219f7ee66ae0b0493762113a49c93
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="performance-considerations"></a>Başarım Değerlendirmeleri
Performans noktalar, her zaman uygulamaları oluştururken önemlidir. Bu bölümde, performans ve güvenlik ile ilgili girdiler içeriyor.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Kimlik Modeliyle Talep ve Yetkilendirmeyi Yönetme](../../../../docs/framework/wcf/feature-details/managing-claims-and-authorization-with-the-identity-model.md)  
 Talepler bir talep koleksiyonunda bulmak için hızlı mekanizmasını açıklar.  
  
 [Dijital İmzaların Şifrelenmesi](../../../../docs/framework/wcf/feature-details/encryption-of-digital-signatures.md)  
 Dijital imza şifreleme performans cezası nasıl ekleyebileceğinizi ve devre dışı bırakmak açıklar.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Güvenlik için En İyi Uygulamalar](../../../../docs/framework/wcf/feature-details/best-practices-for-security-in-wcf.md)  
 [Güvenlik Konuları](../../../../docs/framework/wcf/feature-details/security-considerations-in-wcf.md)
