...
|Mod|Açıklama|  
|----------|-----------------|  
|Hiçbiri|Aktarım katmanında veya ileti katmanında güvenlik sağlanmaz. Önceden tanımlı bağlamalardan hiçbiri, bu modu öğesi hariç, varsayılan olarak, [\<basicHttpBinding>](../../configure-apps/file-schema/wcf/basichttpbinding.md) kod kullanırken <xref:System.ServiceModel.BasicHttpBinding> sınıfı kullanır.|  
...
|Ayar|Açıklama|  
|-------------|-----------------|  
|Hiçbiri|Hizmetin anonim istemcilerle etkileşime geçmesini sağlar.|  
