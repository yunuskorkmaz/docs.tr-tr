...
8. Geçersiz kılma <xref:System.ServiceModel.Discovery.DiscoveryProxy.OnEndResolve%2A?displayProperty=nameWithType> yöntemi. Keşif proxy'si bir Çözümle iletiyi işlemeyi tamamladığında bu yöntem çağrılır.
 ...
 OnBegin... / OnEnd... yöntemler sonraki bulma işlemleri için mantığı sağlar. Örneğin <xref:System.ServiceModel.Discovery.DiscoveryProxy.OnBeginFind%2A> ve <xref:System.ServiceModel.Discovery.DiscoveryProxy.OnEndFind%2A> yöntemleri keşif proxy'si bulma mantığını uygulayın. Keşif proxy'si bir yoklama iletisi aldığında bu yöntemler bir yanıtı istemciye geri gönderilecek yürütülür. İstediğiniz gibi bulma mantığını değiştirebilir, örneğin algoritmalar veya ayrıştırma, bulma işleminin bir parçası uygulama belirli XML meta verileri tarafından eşleşen özel kapsam ekleyebilirsiniz.
