...
- Hizmetin App.config, `messageVersion` öğesinin özniteliğini olarak ayarlayan özel bır POX bağlaması kullanmalıdır `textMessageEncoding` `None` .  
...
- Hizmetin App.config Ayrıca, `EnableHttpGetRequestsBehavior` davranışı davranış uzantıları bölümüne ekleyerek ve kullanarak özel ' i belirtmesi gerekir.  
...
- İstemcinin App.config, öğesinin özniteliğini ayarlayan özel bir POX bağlaması kullanmalıdır `messageVersion` `textMessageEncoding` `None` . Hizmetin bir farkı, istemcinin el ile adreslemeyi etkinleştirmek zorunda olduğundan, giden adresinin değiştirilmesini sağlar.  
