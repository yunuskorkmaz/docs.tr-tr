...
```  
 ...
 Örnek iki ASP.NET web uygulamasından oluşur: yalnızca bir WCF hizmetini içerir ve başka bir hizmeti çağıran .aspx Web sayfası içerir. Çözüm çalıştırırken [!INCLUDE[vs_current_long](../../../../includes/vs-current-long-md.md)] iki Web sitesi farklı noktalarında nerede hizmet ve istemci Canlı farklı etki alanlarında bir ortamı oluşturan barındıracak.  
