...
## <a name="adding-an-operation-selector-to-a-service"></a>Bir hizmet için bir işlem seçici ekleme  
 Hizmeti dağıtma işlemi seçiciler uzantıları olan [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] dağıtıcısı. Çift yönlü sözleşmeler geri çağırma kanalda yöntemlerini seçmek için de vardır çok gönderme işlemi seçiciler burada açıklanan ancak, bu örnekte açıkça kapsanmayan gibi iş istemci işlemi seçicileri.  
