...
-   Şablonları oluşturmaya yönelik söz dizimi.  
  
-   Gelen bir URI'leri örnekleme bir `UriTemplate` kullanarak <xref:System.UriTemplate.BindByName%2A> ve <xref:System.UriTemplate.BindByPosition%2A>.  
  
-   <xref:System.UriTemplateTable.Match%2A>, ters işlemi olduğu `BindByName` ve `BindByPosition`.  
