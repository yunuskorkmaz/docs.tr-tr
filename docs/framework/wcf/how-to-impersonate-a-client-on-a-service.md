...
2. Windows kimlik doğrulaması kullanan bir bağlama kullanın ve veya gibi bir oturum oluşturur <xref:System.ServiceModel.NetTcpBinding> <xref:System.ServiceModel.WSHttpBinding> .  
  
3. Hizmet arabiriminin uygulamasını oluştururken, <xref:System.ServiceModel.OperationBehaviorAttribute> sınıfı istemci kimliğe bürünme gerektiren yönteme uygulayın. <xref:System.ServiceModel.OperationBehaviorAttribute.Impersonation%2A>Özelliğini olarak ayarlayın <xref:System.ServiceModel.ImpersonationOption.Required> .  
