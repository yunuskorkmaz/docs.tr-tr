---
title: "İstemci Oluşturma"
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology: dotnet-clr
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- clients [WCF], building
- clients [WCF]
ms.assetid: 28dfff47-d95b-4cc2-9d5c-b3b9ba71fd50
caps.latest.revision: "19"
author: Erikre
ms.author: erikre
manager: erikre
ms.openlocfilehash: 098af2ba342b780f7af7479c640e1acc5d44e4cc
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 10/18/2017
---
# <a name="building-clients"></a>İstemci Derleme
Bu bölümdeki konular, oluşturma ve kullanma gösteren bir [!INCLUDE[indigo1](../../../includes/indigo1-md.md)] istemci.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [WCF istemcisi genel bakış](../../../docs/framework/wcf/wcf-client-overview.md)  
 Genel bir bakış sağlar [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] istemcileri ve nasıl çalışırlar.  
  
 [Bir WCF istemcisi kullanarak hizmetlere erişme](../../../docs/framework/wcf/accessing-services-using-a-wcf-client.md)  
 Oluşturma temel yöntemi özetlenmektedir bir [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] bir istemci uygulaması kullanmak için istemci nesne.  
  
 [İstemcilerinin güvenliğini sağlama](../../../docs/framework/wcf/securing-clients.md)  
 Güvenlik için yapılandırmayı açıklar [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] istemciler.  
  
## <a name="reference"></a>Başvuru  
 <xref:System.ServiceModel.ClientBase%601>  
  
 <xref:System.ServiceModel.DuplexClientBase%601>  
  
 <xref:System.ServiceModel.DuplexChannelFactory%601>  
  
 <xref:System.ServiceModel.ServiceContractAttribute>  
  
 <xref:System.ServiceModel.OperationContractAttribute>  
  
 <xref:System.Runtime.Serialization.DataContractAttribute>  
  
 <xref:System.Runtime.Serialization.DataMemberAttribute>  
  
## <a name="related-sections"></a>İlgili Bölümler  
 [Barındırma hizmetleri](../../../docs/framework/wcf/hosting-services.md)  
  
 [Hizmetleri Tasarlama ve uygulama](../../../docs/framework/wcf/designing-and-implementing-services.md)
