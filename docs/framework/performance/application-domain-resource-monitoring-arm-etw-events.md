...
<a name="top"></a>Bu olaylar, bir uygulama etki alanının durumu hakkında ayrıntılı tanılama bilgileri sağlar. Aynı bilgileri elde etmek için bu olayları kullanabilir veya uygulama etki alanı kaynak izleme (ARM) özelliğini kullanabilirsiniz.  
  
 Bu kategori aşağıdaki olaylardan oluşur:  
  
- [ThreadCreated olayı](#threadcreated_event)  
  
- [Appdomainmemalbulunan olay](#appdomainmemallocated_event)  
  
- [Appdomainmemsurvilenmiş olay](#appdomainmemsurvived_event)  
  
- [ThreadAppDomainEnter olayı](#threadappdomainenter_event)  
  
- [Threadsonlandırılmış olay](#threadterminated_event)  
  
<a name="threadcreated_event"></a>   
## <a name="threadcreated-event"></a>ThreadCreated olayı  
 Bu olay Ayrıca, Özet sağlayıcının `ThreadDC` altında ( `AppDomainResourceManagementRundownKeyword` anahtar sözcüğünün altında) de oluşturulur. Bu, bu kategorideki özet sağlayıcı altında oluşturulan tek olaydır.  
