...
```  
 ...
 PerfView 'ın varsayılan olarak açtığı diğer sağlayıcıları kapatır ve MyCompany-MyApp sağlayıcıyı açar.  (Yıldız işareti bir olduğunu gösterir <xref:System.Diagnostics.Tracing.EventSource> .)  
...
- **Olay türleri filtre** kutusunda, öğesini belirtin `Process/Start | MyCompany-MyApp` . Bu, MyCompany-MyApp ve Windows çekirdek/Işlem/başlatma olayından olaylar için bir filtre ayarlar.  
