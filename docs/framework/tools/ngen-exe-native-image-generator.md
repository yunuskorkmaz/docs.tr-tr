...
<span data-ttu-id="bf78d-108">.NET Framework 4 ' te Ngen. exe ' ye yapılan değişiklikler:</span><span class="sxs-lookup"><span data-stu-id="bf78d-108">Changes to Ngen.exe in the .NET Framework 4:</span></span>
...
## <a name="priority-levels"></a><span data-ttu-id="bf78d-166">Öncelik Düzeyleri</span><span class="sxs-lookup"><span data-stu-id="bf78d-166">Priority Levels</span></span>
...
|------------|-----------------|
|`/nologo`|<span data-ttu-id="bf78d-192">Microsoft başlangıç başlığını bastır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-192">Suppress the Microsoft startup banner display.</span></span>|
...
- [<span data-ttu-id="bf78d-224">Farklı senaryolar için görüntü oluşturma</span><span class="sxs-lookup"><span data-stu-id="bf78d-224">Generating images for different scenarios</span></span>](#Scenarios)
...
<span data-ttu-id="bf78d-241">Bir derleme için yerel görüntü oluşturduktan sonra, çalışma zamanı derlemeyi her çalıştırdığında otomatik olarak bu yerel görüntüyü bulmayı ve kullanmayı dener.</span><span class="sxs-lookup"><span data-stu-id="bf78d-241">After you have generated a native image for an assembly, the runtime automatically attempts to locate and use this native   image each time it runs the assembly.</span></span> <span data-ttu-id="bf78d-242">Kullanım senaryolarına göre, birden fazla görüntüler oluşturulabilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-242">Multiple images can be generated, depending on usage scenarios.</span></span>
...
<span data-ttu-id="bf78d-252">Yerel görüntüler kod işlemler arasında paylaşıldığında bellek kullanımını önemli ölçüde iyileştirebilirler.</span><span class="sxs-lookup"><span data-stu-id="bf78d-252">Native images can significantly improve memory use when code is shared between processes.</span></span> <span data-ttu-id="bf78d-253">Yerel görüntüler Windows PE dosyalarıdır, yani bir .dll dosyasının tek kopyası birden çok işlem tarafından paylaşılabilir; buna karşılık, JIT derleyicisi tarafından üretilen yerel kod özel bellekte tutulur ve paylaşılamaz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-253">Native images are Windows PE files, so a single copy of a .dll file can be shared by multiple processes; by contrast, native code produced by the JIT compiler is stored in private memory and cannot be shared.</span></span>
...
## <a name="native-images-and-jit-compilation"></a><span data-ttu-id="bf78d-320">Yerel görüntüler ve JıT derlemesi</span><span class="sxs-lookup"><span data-stu-id="bf78d-320">Native images and JIT compilation</span></span>
...
- <span data-ttu-id="bf78d-336">.NET Framework sürümü.</span><span class="sxs-lookup"><span data-stu-id="bf78d-336">The version of the .NET Framework.</span></span>
...
## <a name="examples"></a><span data-ttu-id="bf78d-369">Örnekler</span><span class="sxs-lookup"><span data-stu-id="bf78d-369">Examples</span></span>
...
<span data-ttu-id="bf78d-402">Tüm görüntüleri güncelleştirmek uzun süren bir işlem olabilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-402">Updating all images can be a lengthy process.</span></span> <span data-ttu-id="bf78d-403">`/queue` Seçeneğini kullanarak, yerel görüntü hizmeti tarafından yürütme için güncelleştirmeleri sıraya alabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-403">You can queue the updates for execution by the native image service by using the `/queue` option.</span></span> <span data-ttu-id="bf78d-404">`/queue` Seçenek ve yükleme öncelikleri hakkında daha fazla bilgi için bkz. [Native Image Service](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="bf78d-404">For more information on the `/queue` option and installation priorities, see [Native Image Service](#native-image-service).</span></span>

```console
ngen update /queue
```

### <a name="uninstalling-images"></a><span data-ttu-id="bf78d-405">Görüntüleri Kaldırma</span><span class="sxs-lookup"><span data-stu-id="bf78d-405">Uninstalling Images</span></span>

<span data-ttu-id="bf78d-406">Ngen.exe bağımlılıkların bir listesini tutar, yani paylaşılan bileşenler yalnızca onlara bağlı olan tüm derlemeler kaldırıldığında kaldırılır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-406">Ngen.exe maintains a list of dependencies, so that shared components are removed only when all assemblies that depend on them have been removed.</span></span> <span data-ttu-id="bf78d-407">Ek olarak kök olarak yüklenmiş ortak bileşenler silinmez.</span><span class="sxs-lookup"><span data-stu-id="bf78d-407">In addition, a shared component is not removed if it has been installed as a root.</span></span>

<span data-ttu-id="bf78d-408">Aşağıdaki komut, kök `ClientApp.exe`için tüm senaryoları kaldırır:</span><span class="sxs-lookup"><span data-stu-id="bf78d-408">The following command uninstalls all scenarios for the root `ClientApp.exe`:</span></span>

```console
ngen uninstall ClientApp
```

<span data-ttu-id="bf78d-409">Eylem `uninstall` , belirli senaryoları kaldırmak için kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-409">The `uninstall` action can be used to remove specific scenarios.</span></span> <span data-ttu-id="bf78d-410">Aşağıdaki komut, için `ClientApp.exe`tüm hata ayıklama senaryolarını kaldırır:</span><span class="sxs-lookup"><span data-stu-id="bf78d-410">The following command uninstalls all debug scenarios for `ClientApp.exe`:</span></span>
...
> [!NOTE]
> <span data-ttu-id="bf78d-411">Senaryoları `/debug` kaldırmak, hem `/profile` hem de içeren bir senaryoyu kaldırmaz`/debug.`</span><span class="sxs-lookup"><span data-stu-id="bf78d-411">Uninstalling `/debug` scenarios does not uninstall a scenario that includes both `/profile` and `/debug.`</span></span>

<span data-ttu-id="bf78d-412">Aşağıdaki komut belirli bir sürümü `ClientApp.exe`için tüm senaryoları kaldırır:</span><span class="sxs-lookup"><span data-stu-id="bf78d-412">The following command uninstalls all scenarios for a specific version of `ClientApp.exe`:</span></span>

```console
ngen uninstall "ClientApp, Version=1.0.0.0"
```

<span data-ttu-id="bf78d-413">Aşağıdaki komutlar, bu derleme için tüm `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` senaryoları veya yalnızca hata ayıklama senaryosunu kaldırır:</span><span class="sxs-lookup"><span data-stu-id="bf78d-413">The following commands uninstall all scenarios for `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` or just the debug scenario for that assembly:</span></span>
...
<span data-ttu-id="bf78d-414">`install` Eylemde olduğu gibi, bir uzantı sağlamak, derlemeyi içeren dizinden Ngen. exe ' nin yürütülmesini ya da tam bir yol belirtmeyi gerektirir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-414">As with the `install` action, supplying an extension requires either executing Ngen.exe from the directory containing the assembly or specifying a full path.</span></span>

<span data-ttu-id="bf78d-415">Yerel görüntü hizmetiyle ilgili örnekler için bkz. [Native Image Service](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="bf78d-415">For examples relating to the native image service, see [Native Image Service](#native-image-service).</span></span>

## <a name="native-image-task"></a><span data-ttu-id="bf78d-416">Yerel Görüntü Görevi</span><span class="sxs-lookup"><span data-stu-id="bf78d-416">Native Image Task</span></span>

<span data-ttu-id="bf78d-417">Yerel görüntü görevi, yerel görüntüler üreten ve tutan bir Windows görevidir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-417">The native image task is a Windows task that generates and maintains native images.</span></span> <span data-ttu-id="bf78d-418">Yerel görüntü görevi, desteklenen senaryolar için otomatik olarak yerel görüntüler oluşturur ve geri kazanır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-418">The native image task generates and reclaims native images automatically for supported scenarios.</span></span> <span data-ttu-id="bf78d-419">Ayrıca, yükleyicilerin [Ngen. exe (yerel görüntü Oluşturucu)](ngen-exe-native-image-generator.md) kullanarak yerel görüntüleri ertelenmiş bir sürede oluşturmasını ve güncelleştirmesini de sağlar.</span><span class="sxs-lookup"><span data-stu-id="bf78d-419">It also enables installers to use [Ngen.exe (Native Image Generator)](ngen-exe-native-image-generator.md) to create and update native images at a deferred time.</span></span>

<span data-ttu-id="bf78d-420">Yerel görüntü görevi, her bir mimariyi hedefleyen uygulamalar için derlemeye izin vermek üzere bir bilgisayarda desteklenen her CPU mimarisi için bir kez kaydedilir:</span><span class="sxs-lookup"><span data-stu-id="bf78d-420">The native image task is registered once for each CPU architecture supported on a computer, to allow compilation for applications that target each architecture:</span></span>

|<span data-ttu-id="bf78d-421">Görev adı</span><span class="sxs-lookup"><span data-stu-id="bf78d-421">Task name</span></span>|<span data-ttu-id="bf78d-422">32 bit bilgisayar</span><span class="sxs-lookup"><span data-stu-id="bf78d-422">32-bit computer</span></span>|<span data-ttu-id="bf78d-423">64 bit bilgisayar</span><span class="sxs-lookup"><span data-stu-id="bf78d-423">64-bit computer</span></span>|
|---------------|----------------------|----------------------|
|<span data-ttu-id="bf78d-424">NET Framework NGEN v 4.0.30319</span><span class="sxs-lookup"><span data-stu-id="bf78d-424">NET Framework NGEN v4.0.30319</span></span>|<span data-ttu-id="bf78d-425">Evet</span><span class="sxs-lookup"><span data-stu-id="bf78d-425">Yes</span></span>|<span data-ttu-id="bf78d-426">Evet</span><span class="sxs-lookup"><span data-stu-id="bf78d-426">Yes</span></span>|
|<span data-ttu-id="bf78d-427">NET Framework NGEN v 4.0.30319 64</span><span class="sxs-lookup"><span data-stu-id="bf78d-427">NET Framework NGEN v4.0.30319 64</span></span>|<span data-ttu-id="bf78d-428">Hayır</span><span class="sxs-lookup"><span data-stu-id="bf78d-428">No</span></span>|<span data-ttu-id="bf78d-429">Evet</span><span class="sxs-lookup"><span data-stu-id="bf78d-429">Yes</span></span>|

<span data-ttu-id="bf78d-430">Yerel görüntü görevi, Windows 8 veya sonraki sürümlerde çalışırken .NET Framework 4,5 ve sonraki sürümlerinde kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-430">The native image task is available in the .NET Framework 4.5 and later versions, when running on Windows 8 or later.</span></span> <span data-ttu-id="bf78d-431">Windows 'un önceki sürümlerinde, .NET Framework [yerel görüntü hizmetini](#native-image-service)kullanır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-431">On earlier versions of Windows, the .NET Framework uses the [Native Image Service](#native-image-service).</span></span>

### <a name="task-lifetime"></a><span data-ttu-id="bf78d-432">Görev ömrü</span><span class="sxs-lookup"><span data-stu-id="bf78d-432">Task Lifetime</span></span>

<span data-ttu-id="bf78d-433">Genellikle, Windows Görev Zamanlayıcı bilgisayar boştayken her gece yerel görüntü görevini başlatır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-433">In general, the Windows Task Scheduler starts the native image task every night when the computer is idle.</span></span> <span data-ttu-id="bf78d-434">Görev, uygulama yükleyicileri tarafından kuyruğa alınan ertelenmiş işleri, ertelenmiş yerel görüntü güncelleştirme isteklerini ve herhangi bir otomatik görüntü oluşturmayı denetler.</span><span class="sxs-lookup"><span data-stu-id="bf78d-434">The task checks for any deferred work that is queued by application installers, any deferred native image update requests, and any automatic image creation.</span></span> <span data-ttu-id="bf78d-435">Görev, bekleyen iş öğelerini tamamlar ve sonra kapatır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-435">The task completes outstanding work items and then shuts down.</span></span> <span data-ttu-id="bf78d-436">Görev çalışırken bilgisayar boşta çalışmayı durduktan sonra görev durmaktadır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-436">If the computer stops being idle while the task is running, the task stops.</span></span>

<span data-ttu-id="bf78d-437">Yerel görüntü görevini Görev Zamanlayıcı kullanıcı arabiriminden el ile veya NGen. exe ' ye elle yapılan çağrılar aracılığıyla da başlatabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-437">You can also start the native image task manually through the Task Scheduler UI or through manual calls to NGen.exe.</span></span> <span data-ttu-id="bf78d-438">Görev bu yöntemlerden birini kullanarak başlatılırsa, bilgisayar artık boşta olmadığında çalışmaya devam eder.</span><span class="sxs-lookup"><span data-stu-id="bf78d-438">If the task is started through either of these methods, it will continue running when the computer is no longer idle.</span></span> <span data-ttu-id="bf78d-439">NGen. exe kullanılarak elle oluşturulan görüntüler, uygulama yükleyicilerine yönelik öngörülebilir davranışı etkinleştirmek için önceliklendirilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-439">Images created manually by using NGen.exe are prioritized to enable predictable behavior for application installers.</span></span>

## <a name="native-image-service"></a><span data-ttu-id="bf78d-440">Yerel Görüntü Hizmeti</span><span class="sxs-lookup"><span data-stu-id="bf78d-440">Native Image Service</span></span>

<span data-ttu-id="bf78d-441">Yerel görüntü hizmeti, yerel görüntüleri üreten ve tutan bir Windows hizmetidir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-441">The native image service is a Windows service that generates and maintains native images.</span></span> <span data-ttu-id="bf78d-442">Yerel görüntü hizmeti, geliştiricinin yerel görüntülerin yükleme ve güncelleştirme işlemini bilgisayar boştayken dönemlere erteetmesine olanak tanır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-442">The native image service allows the developer to defer the installation and update of native images to periods when the computer is idle.</span></span>

<span data-ttu-id="bf78d-443">Normal olarak, yerel görüntü hizmeti bir uygulama veya güncelleştirme için yükleme programı (Yükleyici) tarafından başlatılır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-443">Normally, the native image service is initiated by the installation program (installer) for an application or update.</span></span> <span data-ttu-id="bf78d-444">Öncelik 3 işlemleri için, hizmet bilgisayarda boşta kalma süresi boyunca yürütülür.</span><span class="sxs-lookup"><span data-stu-id="bf78d-444">For priority 3 actions, the service executes during idle time on the computer.</span></span> <span data-ttu-id="bf78d-445">Hizmet, durumunu kaydeder ve gerekirse birden çok yeniden başlatma işlemi için devam etme yeteneğine sahiptir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-445">The service saves its state and is capable of continuing through multiple reboots if necessary.</span></span> <span data-ttu-id="bf78d-446">Birden çok görüntü derlemesi kuyruğa alınabilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-446">Multiple image compilations can be queued.</span></span>

<span data-ttu-id="bf78d-447">Hizmet el ile Ngen. exe komutuyla da etkileşime girer.</span><span class="sxs-lookup"><span data-stu-id="bf78d-447">The service also interacts with the manual Ngen.exe command.</span></span> <span data-ttu-id="bf78d-448">El ile gerçekleştirilen komutların arka plan etkinliğine göre önceliği vardır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-448">Manual commands take precedence over background activity.</span></span>

> [!NOTE]
> <span data-ttu-id="bf78d-449">Windows Vista 'da, yerel görüntü hizmeti için görünen ad "Microsoft.NET Framework NGEN v 2.0.50727 _X86" veya "Microsoft.NET Framework NGEN v 2.0.50727 _X64" olur.</span><span class="sxs-lookup"><span data-stu-id="bf78d-449">On Windows Vista, the name displayed for the native image service is "Microsoft.NET Framework NGEN v2.0.50727_X86" or "Microsoft.NET Framework NGEN v2.0.50727_X64".</span></span> <span data-ttu-id="bf78d-450">Microsoft Windows 'un önceki tüm sürümlerinde, ad ".NET Runtime Optimization Service v 2.0.50727 _X86" veya ".NET Runtime Optimization Service v 2.0.50727 _X64" olur.</span><span class="sxs-lookup"><span data-stu-id="bf78d-450">On all earlier versions of Microsoft Windows, the name is ".NET Runtime Optimization Service v2.0.50727_X86" or ".NET Runtime Optimization Service v2.0.50727_X64".</span></span>

### <a name="launching-deferred-operations"></a><span data-ttu-id="bf78d-451">Ertelenmiş Işlemler başlatılıyor</span><span class="sxs-lookup"><span data-stu-id="bf78d-451">Launching Deferred Operations</span></span>

<span data-ttu-id="bf78d-452">Yükleme veya yükseltmeye başlamadan önce, hizmeti duraklatma önerilir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-452">Before beginning an installation or upgrade, pausing the service is recommended.</span></span> <span data-ttu-id="bf78d-453">Bu, yükleyici dosyaları kopyalarken veya derlemeleri genel bütünleştirilmiş kod önbelleğine yerleştirirken hizmetin yürütülmemesini sağlar.</span><span class="sxs-lookup"><span data-stu-id="bf78d-453">This ensures that the service does not execute while the installer is copying files or putting assemblies in the global assembly cache.</span></span> <span data-ttu-id="bf78d-454">Aşağıdaki Ngen. exe komut satırı Hizmeti duraklatır:</span><span class="sxs-lookup"><span data-stu-id="bf78d-454">The following Ngen.exe command line pauses the service:</span></span>

```console
ngen queue pause
```

<span data-ttu-id="bf78d-455">Tüm ertelenmiş işlemler sıraya alınmışsa aşağıdaki komut hizmetin sürdürülmesine izin verir:</span><span class="sxs-lookup"><span data-stu-id="bf78d-455">When all deferred operations have been queued, the following command allows the service to resume:</span></span>

```console
ngen queue continue
```

<span data-ttu-id="bf78d-456">Yeni bir uygulama yüklerken veya paylaşılan bir bileşeni güncelleştirirken yerel görüntü üretimini ertelemek için `/queue` seçeneğini `install` veya `update` eylemleriyle birlikte kullanın.</span><span class="sxs-lookup"><span data-stu-id="bf78d-456">To defer native image generation when installing a new application or when updating a shared component, use the `/queue` option with the `install` or `update` actions.</span></span> <span data-ttu-id="bf78d-457">Aşağıdaki Ngen. exe komut satırları paylaşılan bir bileşen için yerel bir görüntü yükler ve etkilenen tüm köklerin güncelleştirilmesini gerçekleştirir:</span><span class="sxs-lookup"><span data-stu-id="bf78d-457">The following Ngen.exe command lines install a native image for a shared component and perform an update of all roots that may have been affected:</span></span>
...
<span data-ttu-id="bf78d-458">Eylem, yalnızca kullananlarla `MyComponent`değil, geçersiz kılınan tüm yerel görüntüleri yeniden oluşturur. `update`</span><span class="sxs-lookup"><span data-stu-id="bf78d-458">The `update` action regenerates all native images that have been invalidated, not just those that use `MyComponent`.</span></span>

<span data-ttu-id="bf78d-459">Uygulamanız birçok kökten oluşuyorsa, ertelenmiş eylemlerin önceliğini kontrol edebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-459">If your application consists of many roots, you can control the priority of the deferred actions.</span></span> <span data-ttu-id="bf78d-460">Aşağıdaki komutlar, üç kök yüklemeyi sıraya alma.</span><span class="sxs-lookup"><span data-stu-id="bf78d-460">The following commands queue the installation of three roots.</span></span> <span data-ttu-id="bf78d-461">`Assembly1`, boşta kalma süresi beklememeden önce yüklenir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-461">`Assembly1` is installed first, without waiting for idle time.</span></span> <span data-ttu-id="bf78d-462">`Assembly2`, boşta kalma süresi beklememeden da yüklenir, ancak tüm öncelik 1 eylemleri tamamlandıktan sonra.</span><span class="sxs-lookup"><span data-stu-id="bf78d-462">`Assembly2` is also installed without waiting for idle time, but after all priority 1 actions have completed.</span></span> <span data-ttu-id="bf78d-463">`Assembly3`, hizmet bilgisayarın boşta olduğunu algıladığında yüklenir.</span><span class="sxs-lookup"><span data-stu-id="bf78d-463">`Assembly3` is installed when the service detects that the computer is idle.</span></span>
...
<span data-ttu-id="bf78d-464">`executeQueuedItems` Eylemi kullanarak sıraya alınan eylemleri zaman uyumlu olarak gerçekleşecek şekilde zorlayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-464">You can force queued actions to occur synchronously by using the `executeQueuedItems` action.</span></span> <span data-ttu-id="bf78d-465">İsteğe bağlı öncelik sağlarsanız, bu eylem yalnızca eşit veya daha düşük önceliğe sahip olan sıraya alınmış eylemleri etkiler.</span><span class="sxs-lookup"><span data-stu-id="bf78d-465">If you supply the optional priority, this action affects only the queued actions that have equal or lower priority.</span></span> <span data-ttu-id="bf78d-466">Varsayılan öncelik 3 ' dir, bu nedenle aşağıdaki Ngen. exe komutu sıraya alınan tüm eylemleri hemen işler ve tamamlanana kadar döndürmez:</span><span class="sxs-lookup"><span data-stu-id="bf78d-466">The default priority is 3, so the following Ngen.exe command processes all queued actions immediately, and does not return until they are finished:</span></span>

```console
ngen executeQueuedItems
```

<span data-ttu-id="bf78d-467">Zaman uyumlu komutlar Ngen. exe tarafından yürütülür ve yerel görüntü hizmetini kullanmaz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-467">Synchronous commands are executed by Ngen.exe and do not use the native image service.</span></span> <span data-ttu-id="bf78d-468">Yerel görüntü hizmeti çalışırken Ngen. exe ' yi kullanarak eylemleri çalıştırabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-468">You can execute actions using Ngen.exe while the native image service is running.</span></span>

### <a name="service-shutdown"></a><span data-ttu-id="bf78d-469">Hizmet kapatılıyor</span><span class="sxs-lookup"><span data-stu-id="bf78d-469">Service Shutdown</span></span>

<span data-ttu-id="bf78d-470">`/queue` Seçeneğini içeren bir Ngen. exe komutunun yürütülmesi tarafından başlatıldıktan sonra, hizmet tüm eylemler tamamlanana kadar arka planda çalışır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-470">After being initiated by the execution of an Ngen.exe command that includes the `/queue` option, the service runs in the background until all actions have been completed.</span></span> <span data-ttu-id="bf78d-471">Hizmet, gerektiğinde birden çok yeniden başlatma işleminin devam edebilmesi için durumunu kaydeder.</span><span class="sxs-lookup"><span data-stu-id="bf78d-471">The service saves its state so that it can continue through multiple reboots if necessary.</span></span> <span data-ttu-id="bf78d-472">Hizmet sıraya alınmış daha fazla eylem olmadığını algıladığında, bilgisayarın bir sonraki önyüklenilişinde yeniden başlatmaması için durumunu sıfırlar ve sonra kendisini kapatır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-472">When the service detects that there are no more actions queued, it resets its status so that it will not restart the next time the computer is booted, and then it shuts itself down.</span></span>

### <a name="service-interaction-with-clients"></a><span data-ttu-id="bf78d-473">Istemcilerle hizmet etkileşimi</span><span class="sxs-lookup"><span data-stu-id="bf78d-473">Service Interaction with Clients</span></span>

<span data-ttu-id="bf78d-474">.NET Framework sürüm 2,0 ' de, yerel görüntü hizmeti ile tek etkileşim, Ngen. exe komut satırı aracıdır.</span><span class="sxs-lookup"><span data-stu-id="bf78d-474">In the .NET Framework version 2.0, the only interaction with the native image service is through the command-line tool Ngen.exe.</span></span> <span data-ttu-id="bf78d-475">Yerel görüntü hizmeti için eylemleri sıraya almak ve hizmetle etkileşimde bulunmak için yükleme betiklerinde komut satırı aracını kullanın.</span><span class="sxs-lookup"><span data-stu-id="bf78d-475">Use the command-line tool in installation scripts to queue actions for the native image service and to interact with the service.</span></span>

## <a name="see-also"></a><span data-ttu-id="bf78d-476">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="bf78d-476">See also</span></span>

- [<span data-ttu-id="bf78d-477">Araçlar</span><span class="sxs-lookup"><span data-stu-id="bf78d-477">Tools</span></span>](index.md)
- [<span data-ttu-id="bf78d-478">Yönetilen Yürütme İşlemi</span><span class="sxs-lookup"><span data-stu-id="bf78d-478">Managed Execution Process</span></span>](../../standard/managed-execution-process.md)
- [<span data-ttu-id="bf78d-479">Çalışma Zamanının Bütünleştirilmiş Kodların Konumunu Bulması</span><span class="sxs-lookup"><span data-stu-id="bf78d-479">How the Runtime Locates Assemblies</span></span>](../deployment/how-the-runtime-locates-assemblies.md)
- [<span data-ttu-id="bf78d-480">Komut İstemleri</span><span class="sxs-lookup"><span data-stu-id="bf78d-480">Command Prompts</span></span>](developer-command-prompt-for-vs.md)
