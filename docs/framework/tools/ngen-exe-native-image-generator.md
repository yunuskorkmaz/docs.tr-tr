...
<span data-ttu-id="6b28c-121">Komut satırına şunu yazın:</span><span class="sxs-lookup"><span data-stu-id="6b28c-121">At the command prompt, type the following:</span></span>
...
|<span data-ttu-id="6b28c-142">`display` [`assemblyName` &#124; `assemblyPath`]</span><span class="sxs-lookup"><span data-stu-id="6b28c-142">`display` [`assemblyName` &#124; `assemblyPath`]</span></span>|<span data-ttu-id="6b28c-143">Bir derleme ve bağımlılıkları için yerel görüntülerin durumunu görüntüle.</span><span class="sxs-lookup"><span data-stu-id="6b28c-143">Display the state of the native images for an assembly and its dependencies.</span></span><br /><br /> <span data-ttu-id="6b28c-144">Eğer bağımsız değişken sağlanmazsa, yerel görüntü önbelleğindeki her şey görüntülenir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-144">If no argument is supplied, everything in the native image cache is displayed.</span></span>|
...
- [<span data-ttu-id="6b28c-222">Farklı senaryolar için görüntü oluşturma</span><span class="sxs-lookup"><span data-stu-id="6b28c-222">Generating images for different scenarios</span></span>](#Scenarios)
...
<span data-ttu-id="6b28c-250">Yerel görüntüler kod işlemler arasında paylaşıldığında bellek kullanımını önemli ölçüde iyileştirebilirler.</span><span class="sxs-lookup"><span data-stu-id="6b28c-250">Native images can significantly improve memory use when code is shared between processes.</span></span> <span data-ttu-id="6b28c-251">Yerel görüntüler Windows PE dosyalarıdır, yani bir .dll dosyasının tek kopyası birden çok işlem tarafından paylaşılabilir; buna karşılık, JIT derleyicisi tarafından üretilen yerel kod özel bellekte tutulur ve paylaşılamaz.</span><span class="sxs-lookup"><span data-stu-id="6b28c-251">Native images are Windows PE files, so a single copy of a .dll file can be shared by multiple processes; by contrast, native code produced by the JIT compiler is stored in private memory and cannot be shared.</span></span>
...
## <a name="native-images-and-jit-compilation"></a><span data-ttu-id="6b28c-318">Yerel görüntüler ve JIT derlemesi</span><span class="sxs-lookup"><span data-stu-id="6b28c-318">Native images and JIT compilation</span></span>
...
- <span data-ttu-id="6b28c-334">.NET Framework sürümü.</span><span class="sxs-lookup"><span data-stu-id="6b28c-334">The version of the .NET Framework.</span></span>
...
## <a name="examples"></a><span data-ttu-id="6b28c-367">Örnekler</span><span class="sxs-lookup"><span data-stu-id="6b28c-367">Examples</span></span>
...
<span data-ttu-id="6b28c-396">Aşağıdaki komut yerel görüntü önbelleğinde ada sahip tüm yerel görüntüleri görüntüler `MyAssembly` ve sürüm 1.0.0.0.</span><span class="sxs-lookup"><span data-stu-id="6b28c-396">The following command displays all native images in the native image cache with the name `MyAssembly` and the version 1.0.0.0.</span></span>

```
ngen display "myAssembly, version=1.0.0.0"
```

### <a name="updating-images"></a><span data-ttu-id="6b28c-397">Görüntüleri Güncelleştirme</span><span class="sxs-lookup"><span data-stu-id="6b28c-397">Updating Images</span></span>

<span data-ttu-id="6b28c-398">Görüntüler genellikle paylaşılan bir bileşen yükseltildikten sonra güncelleştirilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-398">Images are typically updated after a shared component has been upgraded.</span></span> <span data-ttu-id="6b28c-399">Değişen veya bağımlılıkları değişen tüm yerel görüntüleri güncelleştirmek için `update` eylemini bağımsız değişken olmadan.</span><span class="sxs-lookup"><span data-stu-id="6b28c-399">To update all native images that have changed, or whose dependencies have changed, use the `update` action with no arguments.</span></span>

```
ngen update
```

<span data-ttu-id="6b28c-400">Tüm görüntüleri güncelleştirmek uzun süren bir işlem olabilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-400">Updating all images can be a lengthy process.</span></span> <span data-ttu-id="6b28c-401">Yürütme için güncelleştirmeleri kullanarak yerel görüntü hizmeti tarafından sıraya alabilirsiniz `/queue` seçeneği.</span><span class="sxs-lookup"><span data-stu-id="6b28c-401">You can queue the updates for execution by the native image service by using the `/queue` option.</span></span> <span data-ttu-id="6b28c-402">Daha fazla bilgi için `/queue` seçeneği ve yükleme öncelikleri bkz [Native Image Service](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="6b28c-402">For more information on the `/queue` option and installation priorities, see [Native Image Service](#native-image-service).</span></span>

```
ngen update /queue
```

### <a name="uninstalling-images"></a><span data-ttu-id="6b28c-403">Görüntüleri Kaldırma</span><span class="sxs-lookup"><span data-stu-id="6b28c-403">Uninstalling Images</span></span>

<span data-ttu-id="6b28c-404">Ngen.exe bağımlılıkların bir listesini tutar, yani paylaşılan bileşenler yalnızca onlara bağlı olan tüm derlemeler kaldırıldığında kaldırılır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-404">Ngen.exe maintains a list of dependencies, so that shared components are removed only when all assemblies that depend on them have been removed.</span></span> <span data-ttu-id="6b28c-405">Ek olarak kök olarak yüklenmiş ortak bileşenler silinmez.</span><span class="sxs-lookup"><span data-stu-id="6b28c-405">In addition, a shared component is not removed if it has been installed as a root.</span></span>

<span data-ttu-id="6b28c-406">Aşağıdaki komut kök için tüm senaryoları kaldırır `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="6b28c-406">The following command uninstalls all scenarios for the root `ClientApp.exe`:</span></span>

```
ngen uninstall ClientApp
```

<span data-ttu-id="6b28c-407">`uninstall` Eylemi belirli senaryoları kaldırmak için kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-407">The `uninstall` action can be used to remove specific scenarios.</span></span> <span data-ttu-id="6b28c-408">Aşağıdaki komutu için tüm hata ayıklama senaryolarını kaldırır `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="6b28c-408">The following command uninstalls all debug scenarios for `ClientApp.exe`:</span></span>
...
> [!NOTE]
> <span data-ttu-id="6b28c-409">Kaldırma `/debug` senaryoları hem de içeren bir senaryoyu kaldırmaz `/profile` ve `/debug.`</span><span class="sxs-lookup"><span data-stu-id="6b28c-409">Uninstalling `/debug` scenarios does not uninstall a scenario that includes both `/profile` and `/debug.`</span></span>

<span data-ttu-id="6b28c-410">Aşağıdaki komut belirli bir sürümü için tüm senaryoları kaldırır `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="6b28c-410">The following command uninstalls all scenarios for a specific version of `ClientApp.exe`:</span></span>

```
ngen uninstall "ClientApp, Version=1.0.0.0"
```

<span data-ttu-id="6b28c-411">Tüm senaryolar için aşağıdaki komutları kaldırma `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` veya yalnızca o derleme için hata ayıklama senaryosunu:</span><span class="sxs-lookup"><span data-stu-id="6b28c-411">The following commands uninstall all scenarios for `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` or just the debug scenario for that assembly:</span></span>
...
<span data-ttu-id="6b28c-412">Olduğu gibi `install` eylemi, bir uzantı sağlamak ya Ngen.exe'nin derlemeyi içeren veya bir tam yol belirtilmesini dizinden yürütülmesini gerektirir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-412">As with the `install` action, supplying an extension requires either executing Ngen.exe from the directory containing the assembly or specifying a full path.</span></span>

<span data-ttu-id="6b28c-413">Yerel görüntü hizmetiyle ilgili örnekler için bkz. [Native Image Service](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="6b28c-413">For examples relating to the native image service, see [Native Image Service](#native-image-service).</span></span>

## <a name="native-image-task"></a><span data-ttu-id="6b28c-414">Yerel Görüntü Görevi</span><span class="sxs-lookup"><span data-stu-id="6b28c-414">Native Image Task</span></span>

<span data-ttu-id="6b28c-415">Yerel görüntü görevi oluşturur ve yerel görüntüler tutar Windows bir görevdir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-415">The native image task is a Windows task that generates and maintains native images.</span></span> <span data-ttu-id="6b28c-416">Yerel görüntü görevi oluşturur ve yerel görüntüleri için desteklenen senaryolar için bir otomatik olarak geri kazanır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-416">The native image task generates and reclaims native images automatically for supported scenarios.</span></span> <span data-ttu-id="6b28c-417">Ayrıca, kullanmak yükleyicileri sağlar [Ngen.exe (yerel Görüntü Oluşturucu)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) oluşturup ertelenmiş aynı anda yerel görüntüleri güncelleştir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-417">It also enables installers to use [Ngen.exe (Native Image Generator)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) to create and update native images at a deferred time.</span></span>

<span data-ttu-id="6b28c-418">Yerel görüntü görevi her CPU için mimarisini hedefleyen uygulamalar için derleme izin vermek için bilgisayarda her mimari desteklenen sonra kayıtlı:</span><span class="sxs-lookup"><span data-stu-id="6b28c-418">The native image task is registered once for each CPU architecture supported on a computer, to allow compilation for applications that target each architecture:</span></span>

|<span data-ttu-id="6b28c-419">Görev adı</span><span class="sxs-lookup"><span data-stu-id="6b28c-419">Task name</span></span>|<span data-ttu-id="6b28c-420">32 bit bilgisayar</span><span class="sxs-lookup"><span data-stu-id="6b28c-420">32-bit computer</span></span>|<span data-ttu-id="6b28c-421">64 bit bilgisayar</span><span class="sxs-lookup"><span data-stu-id="6b28c-421">64-bit computer</span></span>|
|---------------|----------------------|----------------------|
|<span data-ttu-id="6b28c-422">NET Framework NGEN v4.0.30319</span><span class="sxs-lookup"><span data-stu-id="6b28c-422">NET Framework NGEN v4.0.30319</span></span>|<span data-ttu-id="6b28c-423">Evet</span><span class="sxs-lookup"><span data-stu-id="6b28c-423">Yes</span></span>|<span data-ttu-id="6b28c-424">Evet</span><span class="sxs-lookup"><span data-stu-id="6b28c-424">Yes</span></span>|
|<span data-ttu-id="6b28c-425">NET Framework NGEN v4.0.30319 64</span><span class="sxs-lookup"><span data-stu-id="6b28c-425">NET Framework NGEN v4.0.30319 64</span></span>|<span data-ttu-id="6b28c-426">Hayır</span><span class="sxs-lookup"><span data-stu-id="6b28c-426">No</span></span>|<span data-ttu-id="6b28c-427">Evet</span><span class="sxs-lookup"><span data-stu-id="6b28c-427">Yes</span></span>|

<span data-ttu-id="6b28c-428">Yerel görüntü Görevi çalıştırırken Windows 8 veya daha sonra .NET Framework 4.5 ve sonraki sürümlerinde kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-428">The native image task is available in the .NET Framework 4.5 and later versions, when running on Windows 8 or later.</span></span> <span data-ttu-id="6b28c-429">Önceki Windows sürümlerinde .NET Framework kullanan [Native Image Service](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="6b28c-429">On earlier versions of Windows, the .NET Framework uses the [Native Image Service](#native-image-service).</span></span>

### <a name="task-lifetime"></a><span data-ttu-id="6b28c-430">Görev ömrü</span><span class="sxs-lookup"><span data-stu-id="6b28c-430">Task Lifetime</span></span>

<span data-ttu-id="6b28c-431">Genel olarak, Windows Görev Zamanlayıcısı'nı bilgisayar boşta olduğunda her gece yerel görüntü görevi başlatır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-431">In general, the Windows Task Scheduler starts the native image task every night when the computer is idle.</span></span> <span data-ttu-id="6b28c-432">Görev uygulama yükleyicileri, tüm ertelenmiş yerel görüntü güncelleştirme istekleri ve herhangi bir otomatik görüntü oluşturma tarafından sıraya alınan tüm ertelenmiş çalışmadır denetler.</span><span class="sxs-lookup"><span data-stu-id="6b28c-432">The task checks for any deferred work that is queued by application installers, any deferred native image update requests, and any automatic image creation.</span></span> <span data-ttu-id="6b28c-433">Görev, bekleyen iş öğeleri tamamlandıktan ve kapanır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-433">The task completes outstanding work items and then shuts down.</span></span> <span data-ttu-id="6b28c-434">Bilgisayar, Görev yürütülürken boşta kaldıktan durduğunda, görev durdurur.</span><span class="sxs-lookup"><span data-stu-id="6b28c-434">If the computer stops being idle while the task is running, the task stops.</span></span>

<span data-ttu-id="6b28c-435">Yerel görüntü görevi Ngen.exe'ye el ile yapılan çağrıları veya Görev Zamanlayıcı kullanıcı Arabirimi aracılığıyla el ile de başlatabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="6b28c-435">You can also start the native image task manually through the Task Scheduler UI or through manual calls to NGen.exe.</span></span> <span data-ttu-id="6b28c-436">Görev bu yöntemlerden birini başlatılırsa, bilgisayar artık boştayken çalışmaya devam edecek.</span><span class="sxs-lookup"><span data-stu-id="6b28c-436">If the task is started through either of these methods, it will continue running when the computer is no longer idle.</span></span> <span data-ttu-id="6b28c-437">NGen.exe kullanarak el ile oluşturulmuş görüntüleri uygulama yükleyicileri için tahmin edilebilir davranış sağlamak önceliklendirilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-437">Images created manually by using NGen.exe are prioritized to enable predictable behavior for application installers.</span></span>

## <a name="native-image-service"></a><span data-ttu-id="6b28c-438">Yerel Görüntü Hizmeti</span><span class="sxs-lookup"><span data-stu-id="6b28c-438">Native Image Service</span></span>

<span data-ttu-id="6b28c-439">Yerel görüntü hizmeti oluşturur ve yerel görüntüler tutan bir Windows hizmetidir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-439">The native image service is a Windows service that generates and maintains native images.</span></span> <span data-ttu-id="6b28c-440">Yerel görüntü hizmetini yükleme ve bilgisayar boşta olduğunda dönemlere yerel görüntülerin güncelleştirmesi erteleme Geliştirici sağlar.</span><span class="sxs-lookup"><span data-stu-id="6b28c-440">The native image service allows the developer to defer the installation and update of native images to periods when the computer is idle.</span></span>

<span data-ttu-id="6b28c-441">Normalde, yerel görüntü hizmeti, bir uygulama veya güncelleştirme için yükleme programını (Yükleyici) tarafından başlatılır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-441">Normally, the native image service is initiated by the installation program (installer) for an application or update.</span></span> <span data-ttu-id="6b28c-442">Öncelik 3 eylemler için bilgisayardaki boşta kalma süresi boyunca hizmeti yürütür.</span><span class="sxs-lookup"><span data-stu-id="6b28c-442">For priority 3 actions, the service executes during idle time on the computer.</span></span> <span data-ttu-id="6b28c-443">Hizmet durumunu kaydeder ve birden çok yeniden başlatma gerekiyorsa devam etmeden özelliğine sahiptir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-443">The service saves its state and is capable of continuing through multiple reboots if necessary.</span></span> <span data-ttu-id="6b28c-444">Birden çok görüntü derlemeleri sıraya alınmış.</span><span class="sxs-lookup"><span data-stu-id="6b28c-444">Multiple image compilations can be queued.</span></span>

<span data-ttu-id="6b28c-445">Hizmet ayrıca el ile Ngen.exe komut ile etkileşime girer.</span><span class="sxs-lookup"><span data-stu-id="6b28c-445">The service also interacts with the manual Ngen.exe command.</span></span> <span data-ttu-id="6b28c-446">El ile komutları arka plan etkinliği daha önceliklidir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-446">Manual commands take precedence over background activity.</span></span>

> [!NOTE]
> <span data-ttu-id="6b28c-447">Windows Vista'da, yerel görüntü hizmetinde görüntülenen "Microsoft.NET Framework NGEN sürüm 2.0.50727_X86" veya "Microsoft.NET Framework NGEN sürüm 2.0.50727_x64" addır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-447">On Windows Vista, the name displayed for the native image service is "Microsoft.NET Framework NGEN v2.0.50727_X86" or "Microsoft.NET Framework NGEN v2.0.50727_X64".</span></span> <span data-ttu-id="6b28c-448">Tüm önceki sürümlerinde Microsoft Windows, adı ".NET çalışma zamanı en iyi duruma getirme hizmeti sürüm 2.0.50727_X86" veya ".NET çalışma zamanı en iyi duruma getirme hizmeti sürüm 2.0.50727_x64" şeklindedir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-448">On all earlier versions of Microsoft Windows, the name is ".NET Runtime Optimization Service v2.0.50727_X86" or ".NET Runtime Optimization Service v2.0.50727_X64".</span></span>

### <a name="launching-deferred-operations"></a><span data-ttu-id="6b28c-449">Ertelenmiş işlem başlatma</span><span class="sxs-lookup"><span data-stu-id="6b28c-449">Launching Deferred Operations</span></span>

<span data-ttu-id="6b28c-450">Bir yükleme veya yükseltme başlamadan önce hizmetin duraklatılması önerilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-450">Before beginning an installation or upgrade, pausing the service is recommended.</span></span> <span data-ttu-id="6b28c-451">Bu yükleyici dosyalarını kopyalama veya genel derleme önbelleğinde derlemeleri yerleştirme sırasında hizmet yürütmez sağlar.</span><span class="sxs-lookup"><span data-stu-id="6b28c-451">This ensures that the service does not execute while the installer is copying files or putting assemblies in the global assembly cache.</span></span> <span data-ttu-id="6b28c-452">Ngen.exe komut satırını hizmet duraklatır:</span><span class="sxs-lookup"><span data-stu-id="6b28c-452">The following Ngen.exe command line pauses the service:</span></span>

```
ngen queue pause
```

<span data-ttu-id="6b28c-453">Tüm ertelenmiş işlemler kuyruğa atılmış hizmeti sürdürmek aşağıdaki komutu sağlar:</span><span class="sxs-lookup"><span data-stu-id="6b28c-453">When all deferred operations have been queued, the following command allows the service to resume:</span></span>

```
ngen queue continue
```

<span data-ttu-id="6b28c-454">Yerel görüntü oluşturma yeni bir uygulama yüklerken veya paylaşılan bir bileşen güncelleştirirken ertelemek için kullanmak `/queue` seçeneğini `install` veya `update` eylemler.</span><span class="sxs-lookup"><span data-stu-id="6b28c-454">To defer native image generation when installing a new application or when updating a shared component, use the `/queue` option with the `install` or `update` actions.</span></span> <span data-ttu-id="6b28c-455">Aşağıdaki Ngen.exe komut satırları, paylaşılan bir bileşen için yerel bir görüntü yükleyin ve etkilenmiş tüm kökleri güncelleştirmesi gerçekleştirin:</span><span class="sxs-lookup"><span data-stu-id="6b28c-455">The following Ngen.exe command lines install a native image for a shared component and perform an update of all roots that may have been affected:</span></span>
...
<span data-ttu-id="6b28c-456">`update` Eylemi yeniden oluşturur kılınan, tüm yerel görüntüleri yalnızca kullananlar `MyComponent`.</span><span class="sxs-lookup"><span data-stu-id="6b28c-456">The `update` action regenerates all native images that have been invalidated, not just those that use `MyComponent`.</span></span>

<span data-ttu-id="6b28c-457">Uygulamanız birçok kökleri oluşuyorsa, ertelenmiş işlemler önceliğini kontrol edebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="6b28c-457">If your application consists of many roots, you can control the priority of the deferred actions.</span></span> <span data-ttu-id="6b28c-458">Aşağıdaki komutları üç kökleri yüklenmesini kuyruğa alın.</span><span class="sxs-lookup"><span data-stu-id="6b28c-458">The following commands queue the installation of three roots.</span></span> <span data-ttu-id="6b28c-459">`Assembly1` boşta kalma süresi beklenmeden önce yüklenir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-459">`Assembly1` is installed first, without waiting for idle time.</span></span> <span data-ttu-id="6b28c-460">`Assembly2` boşta kalma süresi için ancak tüm 1 öncelikli Eylemler tamamladıktan sonra beklenmeden de yüklenir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-460">`Assembly2` is also installed without waiting for idle time, but after all priority 1 actions have completed.</span></span> <span data-ttu-id="6b28c-461">`Assembly3` hizmeti bilgisayarın boşta olduğunu algıladığında yüklenir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-461">`Assembly3` is installed when the service detects that the computer is idle.</span></span>
...
<span data-ttu-id="6b28c-462">Kuyruğa Alınan eylemleri kullanarak zaman uyumlu olarak gerçekleşmesi için zorlayabilirsiniz `executeQueuedItems` eylem.</span><span class="sxs-lookup"><span data-stu-id="6b28c-462">You can force queued actions to occur synchronously by using the `executeQueuedItems` action.</span></span> <span data-ttu-id="6b28c-463">İsteğe bağlı öncelik sağlarsanız, bu eylem, eşit veya daha düşük önceliğe sahip kuyruğa alınan eylemleri etkiler.</span><span class="sxs-lookup"><span data-stu-id="6b28c-463">If you supply the optional priority, this action affects only the queued actions that have equal or lower priority.</span></span> <span data-ttu-id="6b28c-464">Varsayılan öncelik aşağıdaki Ngen.exe komut tüm kuyruğa alınan eylemleri hemen işler ve bitene kadar döndürmeyen 3, olduğundan:</span><span class="sxs-lookup"><span data-stu-id="6b28c-464">The default priority is 3, so the following Ngen.exe command processes all queued actions immediately, and does not return until they are finished:</span></span>

```
ngen executeQueuedItems
```

<span data-ttu-id="6b28c-465">Zaman uyumlu komutları tarafından Ngen.exe yürütüldüğü, yerel görüntü hizmeti kullanmayın.</span><span class="sxs-lookup"><span data-stu-id="6b28c-465">Synchronous commands are executed by Ngen.exe and do not use the native image service.</span></span> <span data-ttu-id="6b28c-466">Ngen.exe yerel görüntü hizmeti çalışırken kullanarak eylemleri yürütebilir.</span><span class="sxs-lookup"><span data-stu-id="6b28c-466">You can execute actions using Ngen.exe while the native image service is running.</span></span>

### <a name="service-shutdown"></a><span data-ttu-id="6b28c-467">Hizmet kapanması</span><span class="sxs-lookup"><span data-stu-id="6b28c-467">Service Shutdown</span></span>

<span data-ttu-id="6b28c-468">İçeren bir Ngen.exe komut yürütülmesi tarafından başlatılan sonra `/queue` seçeneği, hizmetin arka planda çalışır tüm eylemleri tamamlanana kadar.</span><span class="sxs-lookup"><span data-stu-id="6b28c-468">After being initiated by the execution of an Ngen.exe command that includes the `/queue` option, the service runs in the background until all actions have been completed.</span></span> <span data-ttu-id="6b28c-469">Birden çok yeniden başlatma gerekirse sürdürebilmek hizmet durumunu kaydeder.</span><span class="sxs-lookup"><span data-stu-id="6b28c-469">The service saves its state so that it can continue through multiple reboots if necessary.</span></span> <span data-ttu-id="6b28c-470">Hizmet olduğunu algıladığında kuyruğa başka eylem yok, böylece bir bilgisayar önyüklendiğinde sonraki sefer yeniden başlatmaz durumunu sıfırlar ve ardından kendini kapatır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-470">When the service detects that there are no more actions queued, it resets its status so that it will not restart the next time the computer is booted, and then it shuts itself down.</span></span>

### <a name="service-interaction-with-clients"></a><span data-ttu-id="6b28c-471">İstemcilerin hizmet etkileşim</span><span class="sxs-lookup"><span data-stu-id="6b28c-471">Service Interaction with Clients</span></span>

<span data-ttu-id="6b28c-472">.NET Framework sürüm 2. 0'da, yerel görüntü hizmeti ilgili tek etkileşim komut satırı Ngen.exe aracıdır.</span><span class="sxs-lookup"><span data-stu-id="6b28c-472">In the .NET Framework version 2.0, the only interaction with the native image service is through the command-line tool Ngen.exe.</span></span> <span data-ttu-id="6b28c-473">Komut satırı aracı kuyruğu eylemlerini yükleme betikleri yerel görüntü hizmeti ve hizmetiyle etkileşim kurmak için kullanın.</span><span class="sxs-lookup"><span data-stu-id="6b28c-473">Use the command-line tool in installation scripts to queue actions for the native image service and to interact with the service.</span></span>

## <a name="see-also"></a><span data-ttu-id="6b28c-474">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="6b28c-474">See also</span></span>

- [<span data-ttu-id="6b28c-475">Araçlar</span><span class="sxs-lookup"><span data-stu-id="6b28c-475">Tools</span></span>](../../../docs/framework/tools/index.md)
- [<span data-ttu-id="6b28c-476">Yönetilen Yürütme İşlemi</span><span class="sxs-lookup"><span data-stu-id="6b28c-476">Managed Execution Process</span></span>](../../../docs/standard/managed-execution-process.md)
- [<span data-ttu-id="6b28c-477">Çalışma Zamanının Bütünleştirilmiş Kodların Konumunu Bulması</span><span class="sxs-lookup"><span data-stu-id="6b28c-477">How the Runtime Locates Assemblies</span></span>](../../../docs/framework/deployment/how-the-runtime-locates-assemblies.md)
- [<span data-ttu-id="6b28c-478">Komut İstemleri</span><span class="sxs-lookup"><span data-stu-id="6b28c-478">Command Prompts</span></span>](../../../docs/framework/tools/developer-command-prompt-for-vs.md)
