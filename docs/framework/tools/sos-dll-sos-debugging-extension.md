...
|Komut|Açıklama|
|-------------|-----------------|
|**AnalyzeOOM** (**ao**)|Bir çöp toplama yığınındaki ayırma isteği için yetersiz bellek (OOM) gerçekleşen en son bilgileri görüntüler. (Sunucu çöpü toplamada, her bir çöp toplama yığınında varsa OOM görüntüler.)|
...
SOS hata ayıklama uzantısı Visual Studio'da kullanmak için yükleyin [Windows Sürücü Seti'nin (WDK)](/windows-hardware/drivers/download-the-wdk). Visual Studio tümleşik hata ayıklama ortamında hakkında daha fazla bilgi için bkz. [hata ayıklama ortamları](/windows-hardware/drivers/debugger/debuggers-in-the-debugging-tools-for-windows-package).

SOS hata ayıklama uzantısı içine yükleyerek kullanabilirsiniz [WinDbg.exe hata ayıklayıcısındaki](/windows-hardware/drivers/debugger/debugger-download-tools) ve WinDbg.exe içindeki komutları.
