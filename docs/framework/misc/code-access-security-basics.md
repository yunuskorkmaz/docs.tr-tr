...
Bildirim temelli güvenlik çağrıları kullanmak için, izin nesnesinin durum verilerini, ihtiyacınız olan belirli bir izin biçimini temsil etmek üzere başlatmalısınız. Her yerleşik izin, gerçekleştirmek istediğiniz güvenlik işleminin türünü belirtmek için <xref:System.Security.Permissions.SecurityAction> numaralandırması iletilen bir özniteliğe sahiptir. Ancak izinler, kendilerine özel olan kendi parametrelerini de kabul eder.

Aşağıdaki kod parçası, kodunuzun çağıranlarının `MyPermission`adlı özel bir izne sahip olmasını istemek için bildirime dayalı sözdizimi gösterir. Bu izin kuramsal bir özel izindir ve .NET Framework yok. Bu örnekte, bildirim temelli çağrı doğrudan sınıf tanımından yerleştirilir ve bu iznin sınıf düzeyine uygulanacağını belirtin. Bu özniteliğe, çağıranların çalıştırılabilmesi için bu izne sahip olması gerektiğini belirtmek üzere bir **SecurityAction. Demand** yapısı geçirilir.
...
Aşağıdaki kod parçası, kodunuzun çağıranlarının `MyPermission`adlı özel bir izne sahip olmasını istemek için zorunlu sözdizimi gösterir. Bu izin kuramsal bir özel izindir ve .NET Framework yok. `MyPermission` yeni bir örneği, güvenlik çağrısıyla yalnızca bu yöntemin korunması `MyMethod`oluşturulur.
...
Ayrıca, yönetilmeyen koda erişim izni olan kod muhtemelen yönetilmeyen bir API 'ye çağırarak neredeyse tüm işlemleri gerçekleştirebilir. Örneğin, yönetilmeyen kodu çağırma izni olan kodun bir dosyaya erişmesi için <xref:System.Security.Permissions.FileIOPermission> gerekmez; yalnızca yönetilmeyen (Win32) dosya API 'sini doğrudan **çağırmak için dosya**API 'sini atlayarak YÖNETILEN dosya API 'sini atlar. Yönetilen kodun yönetilmeyen koda çağrı yapmak ve doğrudan yönetilmeyen koda çağrı yapmak için izni varsa, güvenlik sistemi güvenlik kısıtlamalarını güvenilir bir şekilde zorlayamayacak ve bu nedenle çalışma zamanı, yönetilmeyen koddaki kısıtlamaları zorlayamıyor.
