...
## <a name="service-lifetime"></a>Hizmet ömrü  
 ...
 Hizmet yüklendikten sonra başlatılmış olması gerekir. Hizmeti başlatmak, BT 'nin çalışmaya başlamasını sağlar. Service **Control Manager**'dan, **Sunucu Gezgini**'den veya <xref:System.ServiceProcess.ServiceController.Start%2A> yöntemi çağırarak koddan bir hizmet başlatabilirsiniz. <xref:System.ServiceProcess.ServiceController.Start%2A> Yöntemi uygulama <xref:System.ServiceProcess.ServiceBase.OnStart%2A> yöntemine işleme geçirir ve burada tanımladığınız kodu işler.  
