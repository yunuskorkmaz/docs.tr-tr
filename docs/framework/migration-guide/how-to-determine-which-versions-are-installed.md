...
Bir makinede yüklü .NET Framework (4,5 ve üzeri) sürümü, **Microsoft\\net Framework kurulum\\NDP\\v4\\Full\\HKEY_LOCAL_MACHINE\\yazılımda**kayıt defterinde listelenmiştir. **Tam** alt anahtar eksikse, .NET Framework 4,5 veya üzeri yüklü değildir.
...
| .NET Framework 4,8     | Windows 10 Mayıs 2019 güncelleştirmesi ve Windows 10 Kasım 2019 güncelleştirmesi: 528040<br/>Windows 10 ve Windows Server 'da sürüm 1909:528209<br/>Diğer tüm Windows işletim sistemlerinde (diğer Windows 10 işletim sistemleri dahil): 528049 |
...
01. Kayıt Defteri Düzenleyicisi 'nde, aşağıdaki alt anahtarı açın: **HKEY_LOCAL_MACHINE\\yazılım\\Microsoft\\net Framework kurulum\\NDP\\v4\\Full**. **Tam** alt anahtar yoksa, .NET Framework 4,5 veya sonraki bir sürümü yüklü değildir.
...
PowerShell komutlarını, **Microsoft\\net Framework Setup\\NDP\\v4\\Full altanahtar\\HKEY_LOCAL_MACHINE\\yazılımının** **yayın** girişinin değerini denetlemek için kullanın.
...
01. Windows kayıt defterindeki **HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\net Framework Setup\\NDP\\v4\\Full** alt anahtarına erişmek için <xref:Microsoft.Win32.RegistryKey.OpenBaseKey%2A?displayProperty=nameWithType> ve <xref:Microsoft.Win32.RegistryKey.OpenSubKey%2A?displayProperty=nameWithType> yöntemlerini kullanın.

    > [!IMPORTANT]
    > Çalıştırdığınız uygulama 32-bit ise ve 64 bit Windows 'da çalışıyorsa, kayıt defteri yolları daha önce listelenenden farklı olacaktır. 64 bit kayıt defteri, **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\** alt anahtarında bulunabilir. Örneğin, .NET Framework 4,5 için kayıt defteri alt anahtarı **HKEY_LOCAL_MACHINE\\yazılım\\Wow6432Node\\Microsoft\\net Framework kurulum\\NDP\\v4\\Full**.
...
1,1 ' den 4,0 ' e .NET Framework her sürümü **HKEY_LOCAL_MACHINE\\yazılım\\Microsoft\\net Framework kurulum\\NDP**' de bir alt anahtar olarak listelenmiştir. Aşağıdaki tabloda her bir .NET Framework sürümünün yolu listelenmektedir. Çoğu sürüm için, bu sürümün yüklü olduğunu göstermek üzere `1` bir **yükleme** REG_DWORD değeri vardır. Bu alt anahtarlarda, sürüm dizesi içeren bir **sürüm** REG_SZ değeri de vardır.

> [!NOTE]
> Kayıt defteri yolundaki **net Framework kurulum** alt anahtarı bir noktayla *başlamaz.*

| Framework sürümü  | Kayıt defteri alt anahtarı | Value |
| ------------------ | --------------- | ----- |
| 1.0                | **HKLM\\yazılım\\Microsoft\\. NETFramework\\Ilkesi\\v 1.0\\3705**     | **Yüklemesi** REG_SZ eşittir `1` |
| 1.1                | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v 1.1.4322**   | **Yüklemesi** REG_DWORD eşittir `1` |
| 2,0                | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v 2.0.50727**  | **Yüklemesi** REG_DWORD eşittir `1` |
| 3.0                | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v 3.0\\kurulum** | **Installsuccess** REG_DWORD eşittir `1` |
| 3,5                | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v 3.5**        | **Yüklemesi** REG_DWORD eşittir `1` |
| 4,0 istemci profili | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v4\\Client**  | **Yüklemesi** REG_DWORD eşittir `1` |
| 4,0 tam profil   | **HKLM\\yazılım\\Microsoft\\NET Framework kurulum\\NDP\\v4\\Full**    | **Yüklemesi** REG_DWORD eşittir `1` |

> [!IMPORTANT]
> Çalıştırdığınız uygulama 32-bit ise ve 64 bit Windows 'da çalışıyorsa, kayıt defteri yolları daha önce listelenenden farklı olacaktır. 64 bit kayıt defteri, **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\** alt anahtarında bulunabilir. Örneğin, .NET Framework 3,5 kayıt defteri alt anahtarı **HKEY_LOCAL_MACHINE\\yazılım\\Wow6432Node\\Microsoft\\net Framework kurulumu\\NDP\\v 3.5**' dir.
...
Windows kayıt defterindeki **Microsoft\\net Framework Setup\\NDP alt anahtarına HKEY_LOCAL_MACHINE\\yazılım\\** erişmek için <xref:Microsoft.Win32.RegistryKey?displayProperty=nameWithType> sınıfını kullanın.
...
- **Clrver. exe aracı**
  ...
  Clr [Sürüm aracı 'nı (Clrver. exe)](../tools/clrver-exe-clr-version-tool.md) kullanarak bir bılgısayarda hangi CLR sürümlerinin yüklü olduğunu belirlersiniz. [Visual Studio için geliştirici komut istemi](../tools/developer-command-prompt-for-vs.md) açın ve `clrver`girin.
...
- **`Environment` sınıfı**
  ...
  > [!IMPORTANT]
  > .NET Framework 4,5 ve sonraki sürümlerinde, CLR sürümünü algılamak için <xref:System.Environment.Version%2A?displayProperty=nameWithType> özelliğini kullanmayın. Bunun yerine, kayıt defterini [.NET Framework 4,5 ve sonraki sürümlerinde](#detect-net-framework-45-and-later-versions)açıklandığı gibi sorgulayın.
  
  01. <xref:System.Version> nesnesini almak için <xref:System.Environment.Version?displayProperty=nameWithType> özelliğini sorgulayın.
