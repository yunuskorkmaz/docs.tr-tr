...
Derleme zamanında kaynak app.config dosyasına el ile bağlama yeniden yönlendirmeleri eklerseniz, Visual Studio, eklediğiniz bağlama yeniden yönlendirmeleri temelinde derlemeleri birleştirmeye çalışır. Örneğin, bir derleme için aşağıdaki bağlama yeniden yönlendirmesini eklemenizi söyleyin:
...
## <a name="specifying-assembly-binding-in-configuration-files"></a>Yapılandırma dosyalarında bütünleştirilmiş kod bağlamayı belirtme
 ...
 Bir derlemeyi bağlamak için, etiketindeki **xmlns** özniteliğiyle birlikte "urn: schemas-microsoft-com: asm. v1" dizesini belirtmeniz gerekir [\<assemblyBinding>](./file-schema/runtime/assemblybinding-element-for-runtime.md) .
