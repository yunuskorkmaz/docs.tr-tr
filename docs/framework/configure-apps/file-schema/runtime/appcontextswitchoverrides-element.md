...
|`Switch.System.Runtime.InteropServices.`<br/>`DoNotMarshalOutByrefSafeArrayOnInvoke`|COM birlikte çalışma olayları parametreleri ByRef SAFEARRAY'i yerel koda geri sıralamanız olup olmadığını kontrol eder (`false`) veya yerel koda geri hazırlama devre dışıdır (`true`).|.NET Framework 4.8|
