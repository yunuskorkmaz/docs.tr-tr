...
```
 ...
 Aşağıdaki örnek, `Switch.System.Globalization.NoAsyncCurrentCulture` ve `Switch.System.IO.BlockLongPaths`iki uygulama uyumluluk anahtarı tanımlamak için `AppContextSwitchOverrides` öğesini kullanır. Noktalı virgül iki ad/değer çiftini ayırır.
