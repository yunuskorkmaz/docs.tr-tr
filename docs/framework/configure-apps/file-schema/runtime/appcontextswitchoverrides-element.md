...
|-----------------|-----------------|----------------|  
|`Switch.MS.Internal.`<br/>`DoNotApplyLayoutRoundingToMarginsAndBorderThickness`|Windows Presentation Foundation denetim düzeni için eski bir algoritma kullanıp kullanmadığını denetler. Daha fazla bilgi için [azaltma: WPF Düzen](~/docs/framework/migration-guide/mitigation-wpf-layout.md).|.NET Framework 4.6|  
