...
|Minfreememoryyüztagetoactivateservice|Bir WCF hizmeti etkinleştirilmeden önce, sistem için kullanılabilir olması gereken minimum boş bellek miktarını belirten bir tamsayı. **Dikkat:**  Bu özniteliği bir WCF hizmetinin Web. config dosyasında kısmi güven ile birlikte belirtmek <xref:System.Security.SecurityException> , hizmet çalıştırıldığında bir ile sonuçlanır.|  
