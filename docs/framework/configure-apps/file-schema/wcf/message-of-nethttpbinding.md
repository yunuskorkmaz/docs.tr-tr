...
[\<netHttpBinding >](nethttpbinding.md)ileti düzeyinde güvenliğe yönelik ayarları tanımlar.  
...
|Öğe|Açıklama|  
|-------------|-----------------|  
|`netHttpBinding`> `security`öğesi <>|<`netHttpBinding`> öğesi için güvenlik yeteneklerini tanımlar.|  
