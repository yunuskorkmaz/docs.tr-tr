...
|`hostnameComparisonMode`|URI ayrıştırmak için kullanılan HTTP ana bilgisayar adını karşılaştırma modunu belirtir. Bu öznitelik türünde `HostnameComparisonMode`, ana bilgisayar üzerinde URI'yi eşleştirirken hizmete erişmek için kullanılıp kullanılmayacağını belirtir. Varsayılan değer `StrongWildcard`, ana bilgisayar adı eşleşme yok sayar.|  
...
## <a name="remarks"></a>Açıklamalar  
 NetHttpsBinding, aktarım olarak ileti göndermek için HTTPS kullanır. Çift yönlü sözleşme ile kullanıldığında, Web yuvaları kullanılır.  NetHttpsBinding ikili Kodlayıcı ile bir BasicHttpsBinding gibi davranacak bir istek-yanıt Sözleşmesi ile kullanıldığında.  
