...
Bu konudaki örneklerde, sorgu ifadesi söz dizimini kullanarak [AdventureWorks Sales modelini](https://archive.codeplex.com/?p=msftdbprodsamples) sorgulamak `From … From …` için `Select` yönteminin ve anahtar sözcüklerinin nasıl kullanılacağı gösterilmektedir. `From … From …`, `SelectMany` yönteminin sorgu tabanlı eşdeğeri olur. Bu örneklerde kullanılan AdventureWorks Sales modeli, AdventureWorks örnek veritabanındaki Contact, Address, Product, SalesOrderHeader ve SalesOrderDetail tablolarından oluşturulmuştur.  
