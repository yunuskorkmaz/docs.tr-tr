...
### <a name="webconfig-example"></a>Web. config örneği  
 Bu örnek <xref:System.Web.Configuration.WebConfigurationManager.OpenWebConfiguration%2A> , yöntemini kullanır `WebConfigurationManager` . Bu durumda, bir tilde kullanarak **Web. config** dosyasının göreli yolunu sağlayabileceğinizi unutmayın. Kod, sınıfa bir başvuru gerektirir `System.Web.Configuration` .  
