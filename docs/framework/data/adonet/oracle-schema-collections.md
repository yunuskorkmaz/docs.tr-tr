...
|----------------|--------------|-----------------|
|SAHİP|Dize|Tablo, görünüm veya küme sahibi.|
...
|LENGTH|Ondalık|Sütunun bayt cinsinden uzunluğu.|
|DUYARLıLıK|Ondalık|Sayı veri türü için ondalık duyarlık; FLOAT veri türü için ikili duyarlık, diğer tüm veri türleri için null.|
|ÖLÇEK|Ondalık|Bir sayı içinde ondalık noktanın sağına doğru olan rakamlar.|
|YAPıLAMAZ|Dize|Bir sütunun null değerlere izin verip Vermemediğini belirtir. Sütunda NULL olmayan bir kısıtlama varsa veya sütun BIRINCIL ANAHTARıN parçasıysa değer N.|

## <a name="indexes"></a>Dizinler

|ColumnName|DataType|Description|
|----------------|--------------|-----------------|
|SAHİP|Dize|Dizinin sahibi|
...
|MASıYLA|Dize|Dizinin ETKIN mi yoksa devre DıŞı mı olduğunu belirtir.|
...
|DURUM|Dize|Bölümlenmemiş dizinin GEÇERLI veya kullanılamaz durumda olup olmadığı.|
...
|LARıNDA|Dize|Üzerinde taranacak dizinlerin sayısını gösteren örnek sayısı.|
|ILDI|Dize|Bu dizinin bölümlenmiş olup olmadığı (Evet &#124; Hayır).|
|AYıRMAYA|Dize|Dizinin geçici bir tabloda olup olmadığı.|
|ÜRET|Dize|Dizinin adının sistem tarafından oluşturulup oluşturulmayacağını belirtir (Y&#124;N).|
|IK|Dize|Dizinin, Oracle9i veri kartuşunun Odcıındexcreate yöntemi tarafından oluşturulan ikincil nesne olup olmadığı (Y&#124;N).|
...
|DURATION|Dize|Geçici bir tablonun süresini gösterir: 1) SYS $ SESSION: satırlar, oturum süresince saklanır, 2) SYS $ TRANSACTION: satır, IŞLEMEDEN sonra silinir, 3) kalıcı tablo için null.|
...
|PARAMETRELER|Dize|Bir etki alanı dizini için parametre dizesi.|
...
|DOMIDX_OPSTATUS|Dize|Bir etki alanı dizininde gerçekleştirilen işlemin durumunu yansıtır: NULL: belirtilen dizin bir etki alanı dizini değil. GEÇERLI: işlem hata olmadan gerçekleştirildi. Başarısız oldu: işlem bir hata ile başarısız oldu.|
|FUNCIDX_STATUS|Dize|İşlev tabanlı bir dizinin durumunu gösterir: NULL: Bu bir işlev tabanlı dizin değildir, ETKIN: işlev tabanlı dizin etkin, devre DıŞı: işlev tabanlı dizin devre dışı bırakıldı.|
...
|DURUM|Dize|Nesnenin durumu (GEÇERLI, GEÇERSIZ veya yok).|
|AYıRMAYA|Dize|Nesnenin geçici olup olmadığı (geçerli oturum yalnızca bu nesnenin kendisine yerleştirilmiş verileri görebilir).|
|ÜRET|Dize|Bu nesne sisteminin adı üretildi mi? (Y &#124; N).|
|IK|Dize|Bu, Oracle9i Data kartuş 'nın Odcıındexcreate yöntemi tarafından oluşturulan ikincil bir nesne olup olmadığı (Y &#124; N).|
|YARATıL|DateTime|Nesnenin oluşturulduğu tarih.|
...
|ColumnName|DataType|Description|
|----------------|--------------|-----------------|
|SAHİP|Dize|Eş anlamlının sahibi.|
...
|TYPE_TEXT|Dize|Türü belirlenmiş görünümün tür tümcesi.|
...
|ColumnName|DataType|Description|
|----------------|--------------|-----------------|
|SAHİP|Dize|Nesnenin sahibinin adı.|
...
|SıRASıNA|Ondalık|Tüm iç içe düzeyler dahil bağımsız değişken sırası.|
...
|ColumnName|DataType|Description|
|----------------|--------------|-----------------|
|SAHİP|Dize|Kısıtlama tanımının sahibi.|
...
|DURUM|Dize|Kısıtlamanın zorlama durumu (ETKIN veya devre DıŞı).|
|ERTETE|Dize|Kısıtlamanın erteedilip edilmeyeceğini belirtir.|
|DOĞRULANMASı|Dize|Tüm verilerin kısıtlamaya ulanmayacağı (DOĞRULANAN veya DOĞRULANMADı).|
|ÜRET|Dize|Kısıtlamanın adının Kullanıcı veya sistem tarafından oluşturulup oluşturulmayacağını belirtir.|
|Hatalı|Dize|Evet değeri, bu kısıtlamanın bir yüzün belirsiz şekilde olduğunu belirtir. Bu belirsizlik nedeniyle oluşan hataları önlemek için, TO_DATE işlevini kullanarak kısıtlamayı dört basamaklı bir yıl ile yeniden yazın.|
|ALMASı|Dize|Etkin bir kısıtlamanın zorlanıp zorlanmayacağı.|
...
|TYPE_LINK|Dize|Yalnızca TYPE_NAME sütununda tanımlanan paket uzak bir paket olduğunda, yalnızca paket yerel türleri için geçerlidir. Bu sütun, uzak pakete başvurmak için kullanılan veritabanı bağlantısını görüntüler.|
