...
```  
 ...
 Veri hizmeti oturum açma kimlik bilgilerini gerektirdiğinde belirtilemez kullanarak bir <xref:System.Net.NetworkCredential> nesne, beyana dayalı belirteç veya kimlik bilgisi gibi el ile üst bilgileri HTTP isteğinde genellikle ayarlamalısınız `Authorization` ve `Cookie` üstbilgileri. Bu tür bir kimlik doğrulama senaryosu hakkında daha fazla bilgi için gönderiye bakın [OData ve kimlik doğrulama: İstemci tarafı kancaları](https://go.microsoft.com/fwlink/?LinkID=200385). İstek iletisinde HTTP üstbilgilerini ayarlama örneği için bkz: [nasıl yapılır: İstemci isteğinde üst bilgileri ayarlama](../../../../docs/framework/data/wcf/how-to-set-headers-in-the-client-request-wcf-data-services.md).  
