...
Bu sınıf, sözleşme türü `IMyAddin` `MyAttribute` adlı özel bir özniteliği ve `MyMetadata`adlı bazı meta verileri tanımlar. `MetadataAttribute` özniteliğiyle işaretlenmiş bir sınıftaki tüm özellikler özel özniteliğinde tanımlanmış meta veriler olarak değerlendirilir. Aşağıdaki iki bildirim eşdeğerdir.
