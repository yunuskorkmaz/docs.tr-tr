...
Değer türü, karşılık gelen null yapılabilir değer türüne örtük olarak dönüştürülebilir olduğundan, onun temel alınan değer türü için yaptığınız gibi, null olabilen değer türünde bir değişkene bir değer atayabilirsiniz. Ayrıca değeri de atayabilirsiniz `null` . Örneğin:
...
Önceden tanımlanmış birli ve ikili [işleçler](../operators/index.md) ya da bir değer türü tarafından desteklenen aşırı yüklenmiş işleçler, `T` karşılık gelen Nullable değer türü tarafından da desteklenir `T?` . *Yükseltilmemiş işleçleri*olarak da bilinen bu işleçler, `null` bir veya her iki işlenen ise üretir `null` ; Aksi takdirde işleç, sonucu hesaplamak için işlenenlerinin kapsanan değerlerini kullanır. Örneğin:
