...
-   Kullandınız mı bir *ALIAS yönergesi kullanarak* tam tür adı niteleme olmadan? A `using` ALIAS yönergesi kullanmaz `using` türleri çözümlemek için kaynak kodu dosyasının yönergeleri. Aşağıdaki örnek CS0246 oluşturur çünkü türü `List<int>` tam değil. `using` İçin yönerge `System.Collections.Generic` hata engellemez.  
    ...
    ```  
     ...
     Başvuru henüz başarıyla erişmediğiniz, ad var olduğundan emin olun ve bu ad alanı içeren gerektiği derlemeyi incelemek için nesne tarayıcısı kullanın. Nesne Tarayıcısı ile derleme ad alanı bulunduğunu doğrulayın, ad alanı için "kullanarak" yönergesi kaldırmayı deneyin ve hangi başka sonları bakın. Kök sorunun başka bir derlemede diğer bazı türüne sahip olabilir.  
