...
Yöntem 'yöntem' için aşırı yük 'sayı' bağımsız değişkenleri alır  
  
 Bir sınıf yöntemine çağrı yapıldı, ancak yöntemin hiçbir tanımı belirtilen sayıda bağımsız değişkeni alır.  
