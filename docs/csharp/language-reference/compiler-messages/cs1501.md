...
' Method ' metodu için hiçbir aşırı yükleme ' number ' bağımsız değişken almaz  
  
 Bir sınıf yöntemine bir çağrı yapıldı, ancak yöntemin tanımı belirtilen sayıda bağımsız değişken almıyor.  
