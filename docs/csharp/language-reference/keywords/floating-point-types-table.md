---
title: Kayan Nokta Türleri Tablosu (C# Başvurusu)
ms.date: 07/20/2015
helpviewer_keywords:
- floating-point numbers [C#]
- ranges of floating-point types [C#]
- types [C#], floating-point types
ms.assetid: da886cc5-e01e-4f62-b3ec-6428c8f7a102
ms.openlocfilehash: 0a0d59e653138d5037eaec8d494313222e526ad8
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33219280"
---
# <a name="floating-point-types-table-c-reference"></a>Kayan Nokta Türleri Tablosu (C# Başvurusu)
Aşağıdaki tabloda duyarlık ve kayan nokta türleri için yaklaşık aralıkları gösterir.  
  
|Tür|Yaklaşık aralığı|Duyarlık|  
|----------|-----------------------|---------------|  
|[float](float.md)|±3.4e38 ±1.5e−45|7 basamağa|  
|[double](double.md)|±1.7e308 ±5.0E−324|15-16 basamak|  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [C# başvurusu](../../../csharp/language-reference/index.md)  
 [C# Programlama Kılavuzu](../../../csharp/programming-guide/index.md)  
 [Varsayılan Değerler Tablosu](default-values-table.md)  
 [Yerleşik Türler Tablosu](built-in-types-table.md)  
 [Tam Sayı Türleri Tablosu](integral-types-table.md)  
 [Sayısal Sonuçlar Tablosunu Biçimlendirme](formatting-numeric-results-table.md)  
 [Türler için Başvuru Tabloları](reference-tables-for-types.md)  
 [decimal](decimal.md)
