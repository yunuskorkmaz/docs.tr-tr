...
C# (matematik, dizin oluşturma, işlev çağrısı, vb.) bir ifadede gerçekleştirmek için hangi işlemleri belirtin simgelerdir birçok işleçleri sağlar.  Yapabilecekleriniz [aşırı](../../../csharp/programming-guide/statements-expressions-operators/overloadable-operators.md) kullanıcı tanımlı bir tür uygulandığında anlamları değiştirmek için çok sayıda işleci.  
 ...
 Bölümleri için en düşük önceliği en yüksek başlayarak C# işleçleri listeler.  Her bölüm içerisindeki işleçleri aynı öncelik düzeyine paylaşır.  
  
## <a name="primary-operators"></a>Birincil Operatörler  
 En yüksek öncelik işleçleri bunlar.  Not: örnekler ayrıntılı sayfalarıyla Git işleçleri tıklatabilirsiniz.  
  
 [x.y](../../../csharp/language-reference/operators/member-access-operator.md) – üye erişimi.  
  
 [x?. y](../../../csharp/language-reference/operators/null-conditional-operators.md) – null koşullu üye erişimi.  Döndürür `null` sol işleneni ise `null`.  
 
 [x? [y] ](../../../csharp/language-reference/operators/null-conditional-operators.md) -null koşullu dizin erişim. Döndürür `null` sol işleneni ise `null`.
 ...
 [typeof](../../../csharp/language-reference/keywords/typeof.md) – işleneni temsil eden System.Type nesnesi döndürür.  
 ...
 [Default(T)](../../../csharp/programming-guide/statements-expressions-operators/default-value-expressions.md) – T türü, varsayılan değerini döndürür `null` başvuru türleri için sıfır sayısal türler için ve sıfır /`null` üyeleri struct türleri için koyar.  
...
## <a name="unary-operators"></a>Birli İşleçler  
 ...
 [--x](../../../csharp/language-reference/operators/decrement-operator.md) – azaltma önek.  Depolama konumu daha az biridir x değerini güncelleştirdikten sonra x değeri döndürür (genellikle 1 tamsayı ekler).  
...
## <a name="logical-and-operator"></a>Mantıksal AND işleci  
 Bu işleç, sonraki bölümde daha yüksek önceliğe ve önceki bölümde düşük önceliğe sahiptir.  Örnekleri içeren ayrıntıları sayfasına gitmek için işleci tıklatabilirsiniz unutmayın.  
  
 [x ve y](../../../csharp/language-reference/operators/and-operator.md) – mantıksal ve bit düzeyinde and  Tamsayı türleri ile kullanmak ve `enum` türleri genellikle izin verilir.  
...
## <a name="logical-or-operator"></a>Mantıksal OR işleci  
 ...
 [x &#124; y](../../../csharp/language-reference/operators/or-operator.md) – mantıksal ve bit düzeyinde OR.  Tamsayı türleri ile kullanmak ve `enum` türleri genellikle izin verilir.  
  
## <a name="conditional-and-operator"></a>Koşullu AND işleci  
 Bu işleç, sonraki bölümde daha yüksek önceliğe ve önceki bölümde düşük önceliğe sahiptir.  Örnekleri içeren ayrıntıları sayfasına gitmek için işleci tıklatabilirsiniz unutmayın.  
  
 [x & & y](../../../csharp/language-reference/operators/conditional-and-operator.md) – mantıksal and  İlk işlenen false ise, ardından C# ikinci işlenen değerlendirmez.  
  
## <a name="conditional-or-operator"></a>Koşullu OR işleci  
 Bu işleç, sonraki bölümde daha yüksek önceliğe ve önceki bölümde düşük önceliğe sahiptir.  Örnekleri içeren ayrıntıları sayfasına gitmek için işleci tıklatabilirsiniz unutmayın.  
  
 [x &#124; &#124; y](../../../csharp/language-reference/operators/conditional-or-operator.md) – mantıksal OR.  İlk işlenen true ise, ardından C# ikinci işlenen değerlendirmez.  
...
## <a name="conditional-operator"></a>Koşullu işleç  
 ...
 [t? x: y](../../../csharp/language-reference/operators/conditional-operator.md) – varsa test `t` true, ardından değerlendirmek ve dönüş `x`; Aksi halde, değerlendirmek ve dönüş `y`.  
