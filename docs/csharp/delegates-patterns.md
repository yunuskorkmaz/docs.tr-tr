...
`Where`Yöntemi, bir dizinin hangi öğelerinin filtreyi geçiğini belirleyen bir temsilci kullanır. Bir LINQ sorgusu oluşturduğunuzda, bu belirli amaçla temsilci uygulamasını sağlarsınız.
...
Daha sonra, `LogMessage()` günlük sınıfınızın daha yapılandırılmış iletiler oluşturması için yönteme birkaç bağımsız değişken ekleyelim:
...
Daha sonra, `Severity` günlük çıktısına gönderilen iletileri filtrelemek için bu bağımsız değişkeni kullanalım.
...
Son olarak, LogMessage yöntemini, hiçbir çıkış mekanizması seçili olmadığında bu durumlar için sağlam olacak şekilde güncelleştirlim. `NullReferenceException` `WriteMessage` Temsilciye bağlı bir çağırma listesi olmadığında geçerli uygulama bir oluşturacak.
Hiçbir yöntem ekli olmadığında sessizce devam eden bir tasarım tercih edebilirsiniz. Bu, yöntemi ile birlikte boş koşullu işleç kullanılarak kolaydır `Delegate.Invoke()` :
...
`?.`Sol işlenen (Bu örnekte) null olan null koşullu işleç (), `WriteMessage` bir iletiyi günlüğe kaydetmek için bir deneme yapılmayacağı anlamına gelir.

`Invoke()`Veya belgelerinde listelenen yöntemi bulamayamayacağız `System.Delegate` `System.MulticastDelegate` . Derleyici, `Invoke` belirtilen herhangi bir temsilci türü için tür güvenli bir yöntem oluşturur. Bu örnekte, bu anlamı `Invoke` tek bir `string` bağımsız değişken alır ve void dönüş türüne sahiptir.
