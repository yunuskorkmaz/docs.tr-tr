...
## <a name="pre-requisites"></a><span data-ttu-id="1ddf8-113">Ön koşullar</span><span class="sxs-lookup"><span data-stu-id="1ddf8-113">Pre-requisites</span></span>
...
1. <span data-ttu-id="1ddf8-181">"ObjectDetection" adlı bir **.NET Core konsol uygulaması** oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-181">Create a **.NET Core Console Application** called "ObjectDetection".</span></span>
...
1. <span data-ttu-id="1ddf8-192">[Onnx model Zoo](https://github.com/onnx/models/tree/master/vision/object_detection_segmentation/tiny_yolov2)ve unzip 'Ten [küçük YOLOv2 modelini](https://onnxzoo.blob.core.windows.net/models/opset_8/tiny_yolov2/tiny_yolov2.tar.gz) indirin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-192">Download the [Tiny YOLOv2 model](https://onnxzoo.blob.core.windows.net/models/opset_8/tiny_yolov2/tiny_yolov2.tar.gz) from the [ONNX Model Zoo](https://github.com/onnx/models/tree/master/vision/object_detection_segmentation/tiny_yolov2), and unzip.</span></span>
    ...
    <span data-ttu-id="1ddf8-193">Komut istemi ' ni açın ve şu komutu girin:</span><span class="sxs-lookup"><span data-stu-id="1ddf8-193">Open the command prompt and enter the following command:</span></span>
...
<span data-ttu-id="1ddf8-204">**Çözüm Gezgini**, projeye sağ tıklayın ve ardından**Yeni klasör** **Ekle** > ' yi seçin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-204">In **Solution Explorer**, right-click the project, and then select **Add** > **New Folder**.</span></span> <span data-ttu-id="1ddf8-205">Yeni klasör Çözüm Gezgini göründüğünde, "Datayapýlarý" olarak adlandırın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-205">When the new folder appears in the Solution Explorer, name it "DataStructures".</span></span>
...
1. <span data-ttu-id="1ddf8-218">**Çözüm Gezgini**, *datayapýlarý* dizinine sağ tıklayıp**Yeni öğe** **Ekle** > ' yi seçin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-218">In **Solution Explorer**, right-click the *DataStructures* directory, and then select **Add** > **New Item**.</span></span>
...
<span data-ttu-id="1ddf8-245">Ayrıştırıcı sınıfları kümesini düzenlemek için projenize yeni bir dizin ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-245">Add a new directory to your project to organize the set of parser classes.</span></span>
...
<span data-ttu-id="1ddf8-262">Ardından, sınırlayıcı kutularınız için bir sınıf oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-262">Next, create a class for your bounding boxes.</span></span>
...
### <a name="create-the-parser"></a><span data-ttu-id="1ddf8-276">Ayrıştırıcı oluşturma</span><span class="sxs-lookup"><span data-stu-id="1ddf8-276">Create the parser</span></span>
...
1. <span data-ttu-id="1ddf8-285">`YoloOutputParser` Sınıf tanımı içinde aşağıdaki sabiti ve alanları ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-285">Inside the `YoloOutputParser` class definition, add the following constant and fields.</span></span>
    ...
    - <span data-ttu-id="1ddf8-291">`CLASS_COUNT`, her bir sınırlayıcı kutusunda bulunan sınıf tahminleri sayısıdır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-291">`CLASS_COUNT` is the number of class predictions contained in each bounding box.</span></span>
...
- <span data-ttu-id="1ddf8-323">`GetTopResult`en yüksek olasılığa sahip tahmin edilen sınıflar listesinden sınıfı seçer.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-323">`GetTopResult` selects the class from the list of predicted classes with the highest probability.</span></span>
...
1. <span data-ttu-id="1ddf8-369">**Yeni öğe Ekle** Iletişim kutusunda **sınıf** ' ı seçin ve **ad** alanını *OnnxModelScorer.cs*olarak değiştirin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-369">In the **Add New Item** dialog box, select **Class** and change the **Name** field to *OnnxModelScorer.cs*.</span></span> <span data-ttu-id="1ddf8-370">Sonra **Ekle** düğmesini seçin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-370">Then, select the **Add** button.</span></span>
    ...
    <span data-ttu-id="1ddf8-379">Ardından, Puanlama için kullanılan ilk yöntem kümesini oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-379">Next, create the first set of methods use for scoring.</span></span> <span data-ttu-id="1ddf8-380">Sınıfınızıniçindeki`OnnxModelScorer`yöntemioluşturun. `LoadModel`</span><span class="sxs-lookup"><span data-stu-id="1ddf8-380">Create the `LoadModel` method inside of your `OnnxModelScorer` class.</span></span>
    ...
    <span data-ttu-id="1ddf8-381">`LoadModel` Yöntemi içinde günlüğe kaydetmek için aşağıdaki kodu ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-381">Inside the `LoadModel` method, add the following code for logging.</span></span>

    [!code-csharp [LoadModelLog](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L47-L49)]

    <span data-ttu-id="1ddf8-382">ML.NET işlem hatları, [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit*) yöntemi çağrıldığında üzerinde çalışılacak veri şemasını bilmelidir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-382">ML.NET pipelines need to know the data schema to operate on when the [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit*) method is called.</span></span> <span data-ttu-id="1ddf8-383">Bu durumda, eğitimle benzer bir süreç kullanılacaktır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-383">In this case, a process similar to training will be used.</span></span> <span data-ttu-id="1ddf8-384">Ancak, hiçbir gerçek eğitim gerçekleşmediğinden boş [`IDataView`](xref:Microsoft.ML.IDataView)kullanılması kabul edilebilir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-384">However, because no actual training is happening, it is acceptable to use an empty [`IDataView`](xref:Microsoft.ML.IDataView).</span></span> <span data-ttu-id="1ddf8-385">Boş bir listeden [`IDataView`](xref:Microsoft.ML.IDataView) işlem hattı için yeni bir oluştur.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-385">Create a new [`IDataView`](xref:Microsoft.ML.IDataView) for the pipeline from an empty list.</span></span>

    [!code-csharp [LoadEmptyIDV](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L52)]

    <span data-ttu-id="1ddf8-386">Bunun altında, işlem hattını tanımlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-386">Below that, define the pipeline.</span></span> <span data-ttu-id="1ddf8-387">İşlem hattı dört dönüşümden oluşur.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-387">The pipeline will consist of four transforms.</span></span>

    - <span data-ttu-id="1ddf8-388">[`LoadImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.LoadImages*)görüntüyü bir bit eşlem olarak yükler.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-388">[`LoadImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.LoadImages*) loads the image as a Bitmap.</span></span>
    - <span data-ttu-id="1ddf8-389">[`ResizeImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.ResizeImages*)görüntüyü belirtilen boyuta ölçeklendirin (Bu durumda, `416 x 416`).</span><span class="sxs-lookup"><span data-stu-id="1ddf8-389">[`ResizeImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.ResizeImages*) rescales the image to the size specified (in this case, `416 x 416`).</span></span>
    - <span data-ttu-id="1ddf8-390">[`ExtractPixels`](xref:Microsoft.ML.ImageEstimatorsCatalog.ExtractPixels*)görüntünün piksel temsilini bir bit eşlemden sayısal bir Vector öğesine değiştirir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-390">[`ExtractPixels`](xref:Microsoft.ML.ImageEstimatorsCatalog.ExtractPixels*) changes the pixel representation of the image from a Bitmap to a numerical vector.</span></span>
    - <span data-ttu-id="1ddf8-391">[`ApplyOnnxModel`](xref:Microsoft.ML.OnnxCatalog.ApplyOnnxModel*)ONNX modelini yükler ve belirtilen verileri öğrenmek için onu kullanır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-391">[`ApplyOnnxModel`](xref:Microsoft.ML.OnnxCatalog.ApplyOnnxModel*) loads the ONNX model and uses it to score on the data provided.</span></span>

    <span data-ttu-id="1ddf8-392">İşlem hattınızı `LoadModel` `data` değişkenin altındaki yöntemde tanımlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-392">Define your pipeline in the `LoadModel` method below the `data` variable.</span></span>

    [!code-csharp [ScoringPipeline](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L55-L58)]

    <span data-ttu-id="1ddf8-393">Şimdi Puanlama için model oluşturma zamanı.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-393">Now it's time to instantiate the model for scoring.</span></span> <span data-ttu-id="1ddf8-394">İşlem hattında [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit*) yöntemi çağırın ve daha fazla işleme için döndürün.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-394">Call the [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit*) method on the pipeline and return it for further processing.</span></span>

    [!code-csharp [FitReturnModel](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L61-L63)]

<span data-ttu-id="1ddf8-395">Model yüklendikten sonra tahmin yapmak için kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-395">Once the model is loaded, it can then be used to make predictions.</span></span> <span data-ttu-id="1ddf8-396">Bu işlemi kolaylaştırmak için `PredictDataUsingModel` `LoadModel` yönteminin altında adlı bir yöntem oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-396">To facilitate that process, create a method called `PredictDataUsingModel` below the `LoadModel` method.</span></span>
...
<span data-ttu-id="1ddf8-397">`PredictDataUsingModel`İçinde, günlüğe kaydetmek için aşağıdaki kodu ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-397">Inside the `PredictDataUsingModel`, add the following code for logging.</span></span>

[!code-csharp [PredictDataLog](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L68-L71)]

<span data-ttu-id="1ddf8-398">Ardından, verileri öğrenmek [`Transform`](xref:Microsoft.ML.ITransformer.Transform*) için yöntemini kullanın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-398">Then, use the [`Transform`](xref:Microsoft.ML.ITransformer.Transform*) method to score the data.</span></span>

[!code-csharp [ScoreImages](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L73)]

<span data-ttu-id="1ddf8-399">Tahmin edilen olasılıkların ayıklanmasını ayıklayın ve bunları ek işleme için geri döndürün.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-399">Extract the predicted probabilities and return them for additional processing.</span></span>

[!code-csharp [ReturnModelOutput](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L75-L77)]

<span data-ttu-id="1ddf8-400">Her iki adım de ayarlandığına göre, bunları tek bir yöntemde birleştirin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-400">Now that both steps are set up, combine them into a single method.</span></span> <span data-ttu-id="1ddf8-401">Yönteminin altında adlı `Score`yeni bir yöntem ekleyin. `PredictDataUsingModel`</span><span class="sxs-lookup"><span data-stu-id="1ddf8-401">Below the `PredictDataUsingModel` method, add a new method called `Score`.</span></span>

[!code-csharp [ScoreMethod](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L80-L85)]

<span data-ttu-id="1ddf8-402">Neredeyse bitti!</span><span class="sxs-lookup"><span data-stu-id="1ddf8-402">Almost there!</span></span> <span data-ttu-id="1ddf8-403">Şimdi bunu tüm kullanıma yerleştirme zamanı.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-403">Now it's time to put it all to use.</span></span>

## <a name="detect-objects"></a><span data-ttu-id="1ddf8-404">Nesneleri Algıla</span><span class="sxs-lookup"><span data-stu-id="1ddf8-404">Detect objects</span></span>

<span data-ttu-id="1ddf8-405">Tüm kurulumun tamamlandığına göre, bazı nesneleri algılamaya zaman atalım.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-405">Now that all of the setup is complete, it's time to detect some objects.</span></span> <span data-ttu-id="1ddf8-406">*Program.cs* sınıfınıza scorer ve ayrıştırıcıya başvurular ekleyerek başlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-406">Start off by adding references to the scorer and parser in your *Program.cs* class.</span></span>

[!code-csharp [ReferenceScorerParser](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L8-L9)]

### <a name="score-and-parse-model-outputs"></a><span data-ttu-id="1ddf8-407">Model çıkışlarını puan ve ayrıştırma</span><span class="sxs-lookup"><span data-stu-id="1ddf8-407">Score and parse model outputs</span></span>

<span data-ttu-id="1ddf8-408">`Main` *Program.cs* sınıfınızın yöntemi içinde, bir try-catch ifadesini ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-408">Inside the `Main` method of your *Program.cs* class, add a try-catch statement.</span></span>
...
<span data-ttu-id="1ddf8-409">`try` Bloğun içinde, nesne algılama mantığını uygulamaya başlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-409">Inside of the `try` block, start implementing the object detection logic.</span></span> <span data-ttu-id="1ddf8-410">İlk olarak, verileri bir [`IDataView`](xref:Microsoft.ML.IDataView)öğesine yükleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-410">First, load the data into an [`IDataView`](xref:Microsoft.ML.IDataView).</span></span>

[!code-csharp [LoadData](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L29-L30)]

<span data-ttu-id="1ddf8-411">Ardından, bir örneği `OnnxModelScorer` oluşturun ve yüklenen verileri almak için onu kullanın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-411">Then, create an instance of `OnnxModelScorer` and use it to score the loaded data.</span></span>

[!code-csharp [ScoreData](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L33-L36)]

<span data-ttu-id="1ddf8-412">Şimdi işleme sonrası adımının zamanı.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-412">Now it's time for the post-processing step.</span></span> <span data-ttu-id="1ddf8-413">Bir örneği `YoloOutputParser` oluşturun ve model çıkışını işlemek için onu kullanın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-413">Create an instance of `YoloOutputParser` and use it to process the model output.</span></span>

[!code-csharp [ParsePredictions](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L39-L44)]

<span data-ttu-id="1ddf8-414">Model çıkışı işlendikten sonra, görüntülerde sınırlayıcı kutuları çizmeniz zaman alır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-414">Once the model output has been processed, it's time to draw the bounding boxes on the images.</span></span>

### <a name="visualize-predictions"></a><span data-ttu-id="1ddf8-415">Tahminleri görselleştirin</span><span class="sxs-lookup"><span data-stu-id="1ddf8-415">Visualize predictions</span></span>

<span data-ttu-id="1ddf8-416">Model, görüntüleri puanladıktan ve çıktılar işlendikten sonra, görüntü üzerinde sınırlayıcı kutular çizmelidir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-416">After the model has scored the images and the outputs have been processed, the bounding boxes have to be drawn on the image.</span></span> <span data-ttu-id="1ddf8-417">Bunu yapmak için `DrawBoundingBox` *program.cs*içindeki `GetAbsolutePath` yönteminin altına adlı bir yöntem ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-417">To do so, add a method called `DrawBoundingBox` below the `GetAbsolutePath` method inside of *Program.cs*.</span></span>
...
<span data-ttu-id="1ddf8-418">İlk olarak, görüntüyü yükleyin ve `DrawBoundingBox` yöntemdeki yükseklik ve genişlik boyutlarını alın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-418">First, load the image and get the height and width dimensions in the `DrawBoundingBox` method.</span></span>

[!code-csharp [LoadImage](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L78-L81)]

<span data-ttu-id="1ddf8-419">Ardından, model tarafından algılanan her bir sınırlayıcı kutu üzerinde yinelemek için her bir For-Each döngüsü oluşturun.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-419">Then, create a for-each loop to iterate over each of the bounding boxes detected by the model.</span></span>
...
<span data-ttu-id="1ddf8-420">For-each döngüsünün içinde, sınırlayıcı kutunun boyutlarını alın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-420">Inside of the for-each loop, get the dimensions of the bounding box.</span></span>

[!code-csharp [GetBBoxDimensions](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L86-L89)]

<span data-ttu-id="1ddf8-421">Sınırlayıcı kutunun boyutları model girişine `416 x 416`karşılık geldiğinden, sınırlayıcı kutu boyutlarını görüntünün gerçek boyutuyla eşleşecek şekilde ölçeklendirin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-421">Because the dimensions of the bounding box correspond to the model input of `416 x 416`, scale the bounding box dimensions to match the actual size of the image.</span></span>

[!code-csharp [ScaleImage](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L92-L95)]

<span data-ttu-id="1ddf8-422">Ardından, her bir sınırlayıcı kutusunun üzerinde görünecek metin için bir şablon tanımlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-422">Then, define a template for text that will appear above each bounding box.</span></span> <span data-ttu-id="1ddf8-423">Metin, ilgili sınırlayıcı kutusunun içindeki nesnenin sınıfını ve güveni içerir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-423">The text will contain the class of the object inside of the respective bounding box as well as the confidence.</span></span>

[!code-csharp [DefineBBoxText](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L98)]

<span data-ttu-id="1ddf8-424">Görüntüde çizim yapmak için bir [`Graphics`](xref:System.Drawing.Graphics) nesneye dönüştürün.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-424">In order to draw on the image, convert it to a [`Graphics`](xref:System.Drawing.Graphics) object.</span></span>
...
<span data-ttu-id="1ddf8-425">Kod bloğunun içinde, [`Graphics`](xref:System.Drawing.Graphics) grafiğin nesne ayarlarını ayarlayın. `using`</span><span class="sxs-lookup"><span data-stu-id="1ddf8-425">Inside the `using` code block, tune the graphic's [`Graphics`](xref:System.Drawing.Graphics) object settings.</span></span>

[!code-csharp [TuneGraphicSettings](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L102-L104)]

<span data-ttu-id="1ddf8-426">Bunun altında metin ve sınırlayıcı kutusu için yazı tipi ve renk seçeneklerini ayarlayın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-426">Below that, set the font and color options for the text and bounding box.</span></span>

[!code-csharp [SetColorOptions](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L106-L114)]

<span data-ttu-id="1ddf8-427">[`FillRectangle`](xref:System.Drawing.Graphics.FillRectangle*) Yöntemi kullanarak metni içermesi için sınırlayıcı kutunun üzerindeki bir dikdörtgeni oluşturun ve girin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-427">Create and fill a rectangle above the bounding box to contain the text using the [`FillRectangle`](xref:System.Drawing.Graphics.FillRectangle*) method.</span></span> <span data-ttu-id="1ddf8-428">Bu, metnin karşıtlığına ve okunabilirliği iyileştirmenize yardımcı olur.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-428">This will help contrast the text and improve readability.</span></span>

[!code-csharp [DrawTextBackground](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L117)]

<span data-ttu-id="1ddf8-429">Ardından, [`DrawString`](xref:System.Drawing.Graphics.DrawString*) ve [`DrawRectangle`](xref:System.Drawing.Graphics.DrawRectangle*) yöntemlerini kullanarak görüntüye metin ve sınırlayıcı kutusunu çizin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-429">Then, Draw the text and bounding box on the image using the [`DrawString`](xref:System.Drawing.Graphics.DrawString*) and [`DrawRectangle`](xref:System.Drawing.Graphics.DrawRectangle*) methods.</span></span>

[!code-csharp [DrawClassAndBBox](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L118-L121)]

<span data-ttu-id="1ddf8-430">For-each döngüsünün dışında, içindeki `outputDirectory`görüntüleri kaydetmek için kod ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-430">Outside of the for-each loop, add code to save the images in the `outputDirectory`.</span></span>

[!code-csharp [SaveImage](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L125-L130)]

<span data-ttu-id="1ddf8-431">Uygulamanın çalışma zamanında beklendiği gibi tahmine dayalı hale getiren ek geri bildirimde bulunmak için, algılanan nesneleri konsola çıkarmak `LogDetectedObjects` üzere program.cs `DrawBoundingBox` dosyasındaki yönteminin altına adlı bir yöntem ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-431">For additional feedback that the application is making predictions as expected at runtime, add a method called `LogDetectedObjects` below the `DrawBoundingBox` method in the *Program.cs* file to output the detected objects to the console.</span></span>

[!code-csharp [LogOutputs](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L133-L143)]

<span data-ttu-id="1ddf8-432">Tahmine dayalı olarak görsel geri bildirim oluşturmaya yönelik yardımcı yöntemlere sahip olduğunuza göre, her bir puanlanmış görüntünün üzerinde yinelemek için bir for döngüsü ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-432">Now that you have helper methods to create visual feedback from the predictions, add a for-loop to iterate over each of the scored images.</span></span>
...
<span data-ttu-id="1ddf8-433">For döngüsü içinde, görüntü dosyasının adını ve onunla ilişkili sınırlayıcı kutuları alın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-433">Inside of the for-loop, get the name of the image file and the bounding boxes associated with it.</span></span>

[!code-csharp [GetImageFileName](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L49-L50)]

<span data-ttu-id="1ddf8-434">Bunun altında, görüntüdeki sınırlayıcı `DrawBoundingBox` kutuları çizmek için yöntemini kullanın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-434">Below that, use the `DrawBoundingBox` method to draw the bounding boxes on the image.</span></span>

[!code-csharp [DrawBBoxes](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L52)]

<span data-ttu-id="1ddf8-435">Son olarak,, `LogDetectedObjects` tahmine dayalı olarak konsola çıkış yapmak için yöntemini kullanın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-435">Lastly, use the `LogDetectedObjects` method to output predictions to the console.</span></span>

[!code-csharp [LogPredictionsOutput](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L54)]

<span data-ttu-id="1ddf8-436">Try-catch ifadesinden sonra, işlemin çalıştığını göstermek için ek mantık ekleyin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-436">After the try-catch statement, add additional logic to indicate the process is done running.</span></span>

[!code-csharp [EndProcessLog](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L62-L63)]

<span data-ttu-id="1ddf8-437">İşte bu kadar!</span><span class="sxs-lookup"><span data-stu-id="1ddf8-437">That's it!</span></span>

## <a name="results"></a><span data-ttu-id="1ddf8-438">Sonuçlar</span><span class="sxs-lookup"><span data-stu-id="1ddf8-438">Results</span></span>

<span data-ttu-id="1ddf8-439">Önceki adımları uyguladıktan sonra konsol uygulamanızı çalıştırın (CTRL + F5).</span><span class="sxs-lookup"><span data-stu-id="1ddf8-439">After following the previous steps, run your console app (Ctrl + F5).</span></span> <span data-ttu-id="1ddf8-440">Sonuçlarınız aşağıdaki çıktıya benzer olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-440">Your results should be similar to the following output.</span></span> <span data-ttu-id="1ddf8-441">Uyarıları veya işlem iletilerini görebilirsiniz, ancak bu iletiler netme için aşağıdaki sonuçlardan kaldırılmıştır.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-441">You may see warnings or processing messages, but these messages have been removed from the following results for clarity.</span></span>
...
<span data-ttu-id="1ddf8-442">Sınırlayıcı kutuları olan görüntüleri görmek için `assets/images/output/` dizine gidin.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-442">To see the images with bounding boxes, navigate to the `assets/images/output/` directory.</span></span> <span data-ttu-id="1ddf8-443">Aşağıda, işlenen görüntülerden birindeki bir örnek verilmiştir.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-443">Below is a sample from one of the processed images.</span></span>

![Bir atma odasının örnek işlenmiş görüntüsü](./media/object-detection-onnx/image3.jpg)

<span data-ttu-id="1ddf8-445">Tebrikler!</span><span class="sxs-lookup"><span data-stu-id="1ddf8-445">Congratulations!</span></span> <span data-ttu-id="1ddf8-446">Artık ml.net ' de önceden eğitilen `ONNX` bir modeli yeniden çalıştırarak nesne algılama için bir makine öğrenimi modelini başarıyla oluşturdunuz.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-446">You've now successfully built a machine learning model for object detection by reusing a pre-trained `ONNX` model in ML.NET.</span></span>

<span data-ttu-id="1ddf8-447">Bu öğreticinin kaynak kodunu [DotNet/Samples](https://github.com/dotnet/machinelearning-samples/tree/master/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx) deposunda bulabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-447">You can find the source code for this tutorial at the [dotnet/samples](https://github.com/dotnet/machinelearning-samples/tree/master/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx) repository.</span></span>

<span data-ttu-id="1ddf8-448">Bu öğreticide, şunların nasıl yapıldığını öğrendiniz:</span><span class="sxs-lookup"><span data-stu-id="1ddf8-448">In this tutorial, you learned how to:</span></span>
> [!div class="checklist"]
>
> - <span data-ttu-id="1ddf8-449">Sorunu anlama</span><span class="sxs-lookup"><span data-stu-id="1ddf8-449">Understand the problem</span></span>
> - <span data-ttu-id="1ddf8-450">ONNX 'in ne olduğunu ve ML.NET ile nasıl çalıştığını öğrenin</span><span class="sxs-lookup"><span data-stu-id="1ddf8-450">Learn what ONNX is and how it works with ML.NET</span></span>
> - <span data-ttu-id="1ddf8-451">Modeli anlama</span><span class="sxs-lookup"><span data-stu-id="1ddf8-451">Understand the model</span></span>
> - <span data-ttu-id="1ddf8-452">Önceden eğitilen modeli yeniden kullanma</span><span class="sxs-lookup"><span data-stu-id="1ddf8-452">Reuse the pre-trained model</span></span>
> - <span data-ttu-id="1ddf8-453">Yüklü bir modelle nesneleri Algıla</span><span class="sxs-lookup"><span data-stu-id="1ddf8-453">Detect objects with a loaded model</span></span>

<span data-ttu-id="1ddf8-454">Genişletilmiş bir nesne algılama örneğini araştırmak için Machine Learning örnekleri GitHub deposuna göz atın.</span><span class="sxs-lookup"><span data-stu-id="1ddf8-454">Check out the Machine Learning samples GitHub repository to explore an expanded object detection sample.</span></span>
> [!div class="nextstepaction"]
> [<span data-ttu-id="1ddf8-455">DotNet/machinöğrenim-Samples GitHub deposu</span><span class="sxs-lookup"><span data-stu-id="1ddf8-455">dotnet/machinelearning-samples GitHub repository</span></span>](https://github.com/dotnet/machinelearning-samples/tree/master/samples/csharp/end-to-end-apps/DeepLearning_ObjectDetection_Onnx)
