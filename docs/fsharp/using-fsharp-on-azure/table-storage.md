...
Bu öğretici oluşturma ve bir tablo silme ve ekleme, güncelleştirme, silme ve tablo verileri Sorgulama dahil olmak üzere Azure Table storage kullanarak bazı genel görevleri gerçekleştirmek için F # kodunun nasıl yazılacağını gösterir.

Tablo depolama kavramsal bir genel bakış için lütfen bakın [tablo depolaması için .NET Kılavuzu](/azure/storage/storage-dotnet-how-to-use-tables)
...
Ardından, kullanın bir [Paket Yöneticisi](package-management.md) gibi [Paket](https://fsprojects.github.io/Paket/) veya [NuGet](https://www.nuget.org/) yüklemek için `WindowsAzure.Storage` paket ve başvuru `WindowsAzure.Storage.dll` bir kullanarakkomut`#r`yönergesi. Bunu yeniden 'Microsoft.WindowsAzure.ConfigurationManager' için Microsoft.Azure ad alanı alabilmek için.
...
`CloudTableClient` Sınıfı tabloları ve varlıkları Table storage'da depolanan almanıza olanak sağlar. Hizmet istemcisini oluşturma yöntemlerinden biri aşağıda verilmiştir:
...
Ekleyeceğiz artık bizim `Customer` tablosu. Bunu yapmak için oluşturduğunuz bir `TableOperation` tabloda yürütülecek. Bu durumda, oluşturduğunuz bir `Insert` işlemi.
...
Bir bölümdeki tüm varlıklar için bir tabloyu sorgulamak için kullanın bir `TableQuery` nesnesi. Burada, "Buster" Bölüm anahtarı olduğu varlıklar için filtreleyin.
...
Bir bölümdeki tüm varlıkları sorgulamak istemiyorsanız bölüm anahtarı Filtresi ile bir satır anahtarı filtresini birleştirerek bir aralık belirtebilirsiniz. Burada, iki filtre "Buster" bölümünde tüm varlıkları almak için burada satır anahtarı (ad) alfabede "M"'den önceki bir harfle başlayan kullanın.
...
Tek, belirli bir varlığı almak üzere bir sorgu yazabilirsiniz. Burada, kullandığınız bir `TableOperation` "Larry Buster" Müşteri belirtmek için. Bir koleksiyon yerine ulaşırsınız bir `Customer`. Bir sorguda hem Bölüm anahtarı hem de satır anahtarını belirtmek tablo hizmetinden tek bir varlık almak için en hızlı yoludur.
...
Bir varlığı güncelleştirmek için tablo hizmetinden alın, varlık nesnesini değiştirin ve sonra geri tablo hizmeti kullanmaya değişiklikleri kaydetmek bir `Replace` işlemi. Bu, işlem başarısız olur; bu durumda alındığından beri sunucunun varlıkta değiştirilmediği sürece bu sunucu üzerinde tamamen değiştirilmesini varlık neden olur. Bu hata uygulamanızı değişiklikler diğer kaynaklardan yanlışlıkla üzerine yazılmasını önlemektir.

[!code-fsharp[TableStorage](../../../samples/snippets/fsharp/azure/table-storage.fsx#L121-L128)]

## <a name="insert-or-replace-an-entity"></a>Ekleme veya değiştirme bir varlık

Bazı durumlarda, varlık tablosunda varsa mevcut bilinmiyor. Ve içinde saklı geçerli değerlerin bulursa, artık gerekli değildir. Kullanabileceğiniz `InsertOrReplace` varlığı oluşturun veya, durumuna bakılmaksızın varsa değiştirin.
