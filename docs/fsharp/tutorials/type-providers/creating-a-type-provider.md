...
- <span data-ttu-id="f3dc5-112">[FSharp.Data.TypeProviders](https://fsprojects.github.io/FSharp.Data.TypeProviders/) kullanmak için tür sağlayıcıları SQL, Entity Framework, OData ve WSDL veri hizmetlerine erişmek için yalnızca .NET Framework programlama ile daha eski bir kümesidir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-112">[FSharp.Data.TypeProviders](https://fsprojects.github.io/FSharp.Data.TypeProviders/) is an older set of type providers for use only with .NET Framework programming for accessing SQL, Entity Framework, OData and WSDL data services.</span></span>
...
- <span data-ttu-id="f3dc5-214">Ne zaman yalnızca verilere ve yöntemlere içeren bir bilgi alanı için bir sağlayıcı yazıyorsunuz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-214">When you are writing a provider for an information space that contains only data and methods.</span></span>
...
### <a name="key-lessons"></a><span data-ttu-id="f3dc5-309">Önemli dersler</span><span class="sxs-lookup"><span data-stu-id="f3dc5-309">Key Lessons</span></span>
...
- <span data-ttu-id="f3dc5-328">Üst bilgi adları olan ya da birimi daha az veya "(birim) adı" sahiptir ve virgül içermeyen.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-328">Header names are either unit-less or have the form "Name (unit)" and don't contain commas.</span></span>
...
<span data-ttu-id="f3dc5-342">Aşağıdaki kod, çekirdek uygulamasının gösterir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-342">The following code shows the core of the implementation.</span></span>
...
<span data-ttu-id="f3dc5-343">Uygulama hakkında aşağıdaki noktalara dikkat edin:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-343">Note the following points about the implementation:</span></span>

- <span data-ttu-id="f3dc5-344">Özgün dosya veya okumak için bir aynı şemaya sahip bir aşırı yüklü oluşturucular sağlar.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-344">Overloaded constructors allow either the original file or one that has an identical schema to be read.</span></span> <span data-ttu-id="f3dc5-345">Bu düzen, yerel veya uzak veri kaynakları için bir tür sağlayıcısı yazma ve uzak veri için şablon olarak kullanılacak bir yerel dosya bu deseni tanır yaygındır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-345">This pattern is common when you write a type provider for local or remote data sources, and this pattern allows a local file to be used as the template for remote data.</span></span>

- <span data-ttu-id="f3dc5-346">Kullanabileceğiniz [TypeProviderConfig](https://msdn.microsoft.com/library/1cda7b9a-3d07-475d-9315-d65e1c97eb44) göreli dosya adlarını çözümlemek için tür sağlayıcı oluşturucuya geçirilen değer.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-346">You can use the [TypeProviderConfig](https://msdn.microsoft.com/library/1cda7b9a-3d07-475d-9315-d65e1c97eb44) value that’s passed in to the type provider constructor to resolve relative file names.</span></span>

- <span data-ttu-id="f3dc5-347">Kullanabileceğiniz `AddDefinitionLocation` sağlanan özellikler konumunu tanımlamak için yöntemi.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-347">You can use the `AddDefinitionLocation` method to define the location of the provided properties.</span></span> <span data-ttu-id="f3dc5-348">Bu nedenle, kullanırsanız `Go To Definition` sağlanan bir özellik, CSV dosyasını Visual Studio'da açılır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-348">Therefore, if you use `Go To Definition` on a provided property, the CSV file will open in Visual Studio.</span></span>

- <span data-ttu-id="f3dc5-349">Kullanabileceğiniz `ProvidedMeasureBuilder` türü sı birimler arayın ve ilgili oluşturmak üzere `float<_>` türleri.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-349">You can use the `ProvidedMeasureBuilder` type to look up the SI units and to generate the relevant `float<_>` types.</span></span>

### <a name="key-lessons"></a><span data-ttu-id="f3dc5-350">Önemli dersler</span><span class="sxs-lookup"><span data-stu-id="f3dc5-350">Key Lessons</span></span>

<span data-ttu-id="f3dc5-351">Bu bölümde, veri kaynağında kendisini içeren basit bir şemaya sahip bir yerel veri kaynağı için bir tür sağlayıcısı oluşturma açıklanmıştır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-351">This section explained how to create a type provider for a local data source with a simple schema that's contained in the data source itself.</span></span>

## <a name="going-further"></a><span data-ttu-id="f3dc5-352">Daha fazla devam</span><span class="sxs-lookup"><span data-stu-id="f3dc5-352">Going Further</span></span>

<span data-ttu-id="f3dc5-353">Aşağıdaki bölümlerde daha fazla araştırma için öneriler içerir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-353">The following sections include suggestions for further study.</span></span>

### <a name="a-look-at-the-compiled-code-for-erased-types"></a><span data-ttu-id="f3dc5-354">Silinen türleri için derlenmiş kod bakma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-354">A Look at the Compiled Code for Erased Types</span></span>

<span data-ttu-id="f3dc5-355">Ne tür sağlayıcısını kullanımını yayılan koduna karşılık gelen hakkında biraz fikir vermek için aşağıdaki işlevi kullanarak araması `HelloWorldTypeProvider` bu konunun önceki bölümlerinde kullanılır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-355">To give you some idea of how the use of the type provider corresponds to the code that's emitted, look at the following function by using the `HelloWorldTypeProvider` that's used earlier in this topic.</span></span>
...
<span data-ttu-id="f3dc5-356">Sonuç kodunu ildasm.exe kullanarak decompiled görüntüsü aşağıda verilmiştir:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-356">Here’s an image of the resulting code decompiled by using ildasm.exe:</span></span>
...
<span data-ttu-id="f3dc5-357">Tüm bahsetmeleri türü örnekte gösterildiği gibi `Type1` ve `InstanceProperty` özelliği silinmesi, yalnızca çalışma zamanı türleri üzerinde işlemler dahil çıkılıyor.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-357">As the example shows, all mentions of the type `Type1` and the `InstanceProperty` property have been erased, leaving only operations on the runtime types involved.</span></span>

### <a name="design-and-naming-conventions-for-type-providers"></a><span data-ttu-id="f3dc5-358">Tasarım ve adlandırma kuralları için tür sağlayıcıları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-358">Design and Naming Conventions for Type Providers</span></span>

<span data-ttu-id="f3dc5-359">Tür sağlayıcıları yazarken aşağıdaki kurallar gözlemleyin.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-359">Observe the following conventions when authoring type providers.</span></span>

<span data-ttu-id="f3dc5-360">**Bağlantı protokoller için sağlayıcıları** genel olarak, veri ve hizmet bağlantısı gibi protokolleri OData veya SQL bağlantıları için çoğu sağlayıcısı DLL'leri adlarını bitmelidir `TypeProvider` veya `TypeProviders`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-360">**Providers for Connectivity Protocols** In general, names of most provider DLLs for data and service connectivity protocols, such as OData or SQL connections, should end in `TypeProvider` or `TypeProviders`.</span></span> <span data-ttu-id="f3dc5-361">Örneğin, aşağıdaki dize şuna benzer bir DLL adı kullanın:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-361">For example, use a DLL name that resembles the following string:</span></span>

```
  Fabrikam.Management.BasicTypeProviders.dll
```

<span data-ttu-id="f3dc5-362">Sağlanan türlerinizi karşılık gelen ad alanının üyeleri ve uygulanan bağlantı protokolü belirtmek emin olun:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-362">Ensure that your provided types are members of the corresponding namespace, and indicate the connectivity protocol that you implemented:</span></span>
...
<span data-ttu-id="f3dc5-363">**Genel olarak kodlamak için yardımcı program sağlayıcıları**.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-363">**Utility Providers for General Coding**.</span></span>  <span data-ttu-id="f3dc5-364">Yardımcı programı için bir tür sağlayıcısı gibi normal ifadeler için tür sağlayıcısı aşağıdaki örnekte gösterildiği gibi bir temel kitaplığının bir parçası olabilir:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-364">For a utility type provider such as that for regular expressions, the type provider may be part of a base library, as the following example shows:</span></span>

```fsharp
  #r "Fabrikam.Core.Text.Utilities.dll"
```

<span data-ttu-id="f3dc5-365">Bu durumda, sağlanan türü normal .NET Tasarım Kuralları göre uygun bir noktada görünür:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-365">In this case, the provided type would appear at an appropriate point according to normal .NET design conventions:</span></span>
...
<span data-ttu-id="f3dc5-366">**Tekli veri kaynakları**.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-366">**Singleton Data Sources**.</span></span> <span data-ttu-id="f3dc5-367">Bazı tür sağlayıcılarını tek bir adanmış veri kaynağına bağlanmak ve yalnızca veriler sağlar.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-367">Some type providers connect to a single dedicated data source and provide only data.</span></span> <span data-ttu-id="f3dc5-368">Bu durumda, düşmelidir `TypeProvider` sonek ve .NET normal kuralları kullanın:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-368">In this case, you should drop the `TypeProvider` suffix and use normal conventions for .NET naming:</span></span>
...
<span data-ttu-id="f3dc5-369">Daha fazla bilgi için `GetConnection` tasarım bu konunun ilerleyen bölümlerinde açıklanan kuralı.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-369">For more information, see the `GetConnection` design convention that's described later in this topic.</span></span>

### <a name="design-patterns-for-type-providers"></a><span data-ttu-id="f3dc5-370">Tür sağlayıcıları için Tasarım desenleri</span><span class="sxs-lookup"><span data-stu-id="f3dc5-370">Design Patterns for Type Providers</span></span>

<span data-ttu-id="f3dc5-371">Aşağıdaki bölümlerde tasarım desenleri tür sağlayıcıları yazarken kullanabileceğiniz açıklanmaktadır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-371">The following sections describe design patterns you can use when authoring type providers.</span></span>

#### <a name="the-getconnection-design-pattern"></a><span data-ttu-id="f3dc5-372">GetConnection tasarım deseni</span><span class="sxs-lookup"><span data-stu-id="f3dc5-372">The GetConnection Design Pattern</span></span>

<span data-ttu-id="f3dc5-373">Çoğu tür sağlayıcıları kullanan yazılması gerektiğini `GetConnection` aşağıdaki örnekte gösterildiği gibi FSharp.Data.typeproviders.dll tür sağlayıcıları tarafından kullanılan Desen:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-373">Most type providers should be written to use the `GetConnection` pattern that's used by the type providers in FSharp.Data.TypeProviders.dll, as the following example shows:</span></span>
...
#### <a name="type-providers-backed-by-remote-data-and-services"></a><span data-ttu-id="f3dc5-374">Uzak Veri ve hizmetler tarafından desteklenen tür sağlayıcıları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-374">Type Providers Backed By Remote Data and Services</span></span>

<span data-ttu-id="f3dc5-375">Uzak Veri ve hizmetler tarafından desteklenen bir tür sağlayıcısı oluşturmadan önce bir dizi bağlı programlamada belirlidir sorunları dikkate almanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-375">Before you create a type provider that's backed by remote data and services, you must consider a range of issues that are inherent in connected programming.</span></span> <span data-ttu-id="f3dc5-376">Bu sorunları aşağıdaki önemli noktalar şunlardır:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-376">These issues include the following considerations:</span></span>

- <span data-ttu-id="f3dc5-377">Şema eşleme</span><span class="sxs-lookup"><span data-stu-id="f3dc5-377">schema mapping</span></span>

- <span data-ttu-id="f3dc5-378">canlılık ve şema değişikliği varlığında geçersiz kılma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-378">liveness and invalidation in the presence of schema change</span></span>

- <span data-ttu-id="f3dc5-379">Şema önbelleğe alma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-379">schema caching</span></span>

- <span data-ttu-id="f3dc5-380">Veri erişim işlemleri zaman uyumsuz uygulamaları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-380">asynchronous implementations of data access operations</span></span>

- <span data-ttu-id="f3dc5-381">LINQ sorguları da dahil olmak üzere, destekleyici sorguları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-381">supporting queries, including LINQ queries</span></span>

- <span data-ttu-id="f3dc5-382">kimlik bilgileri ve kimlik doğrulaması</span><span class="sxs-lookup"><span data-stu-id="f3dc5-382">credentials and authentication</span></span>

<span data-ttu-id="f3dc5-383">Bu konu bu sorunların daha fazla keşfedin değil.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-383">This topic doesn't explore these issues further.</span></span>

### <a name="additional-authoring-techniques"></a><span data-ttu-id="f3dc5-384">Ek geliştirme teknikleri</span><span class="sxs-lookup"><span data-stu-id="f3dc5-384">Additional Authoring Techniques</span></span>

<span data-ttu-id="f3dc5-385">Kendi tür sağlayıcıları yazdığınızda, aşağıdaki ek teknikler kullanmak isteyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-385">When you write your own type providers, you might want to use the following additional techniques.</span></span>

### <a name="creating-types-and-members-on-demand"></a><span data-ttu-id="f3dc5-386">Türler ve üyeler üzerine oluşturma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-386">Creating Types and Members On-Demand</span></span>

<span data-ttu-id="f3dc5-387">ProvidedType API sürümlerini AddMember Gecikmeli.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-387">The ProvidedType API has delayed versions of AddMember.</span></span>
...
<span data-ttu-id="f3dc5-388">Bu sürümler, isteğe bağlı alanları türleri oluşturmak için kullanılır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-388">These versions are used to create on-demand spaces of types.</span></span>

### <a name="providing-array-types-and-generic-type-instantiations"></a><span data-ttu-id="f3dc5-389">Dizi türleri ve genel tür Örneklemeleri sağlama</span><span class="sxs-lookup"><span data-stu-id="f3dc5-389">Providing Array types and Generic Type Instantiations</span></span>

<span data-ttu-id="f3dc5-390">Belirtilen üye (dizi türleri ve byref türleriyle genel türlerin örneklemeleri dahil olan imzaları) normal kullanarak yaptığınız `MakeArrayType`, `MakePointerType`, ve `MakeGenericType` herhangi bir örneği üzerinde <xref:System.Type>de dahil olmak üzere `ProvidedTypeDefinitions`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-390">You make provided members (whose signatures include array types, byref types, and instantiations of generic types) by using the normal `MakeArrayType`, `MakePointerType`, and `MakeGenericType` on any instance of <xref:System.Type>, including `ProvidedTypeDefinitions`.</span></span>

> [!NOTE]
> <span data-ttu-id="f3dc5-391">Bazı durumlarda yardımcı kullanmak zorunda kalabilirsiniz `ProvidedTypeBuilder.MakeGenericType`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-391">In some cases you may have to use the helper in `ProvidedTypeBuilder.MakeGenericType`.</span></span>  <span data-ttu-id="f3dc5-392">Bkz: [tür sağlayıcısı SDK Belgeleri](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) daha fazla ayrıntı için.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-392">See the [Type Provider SDK documentation](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) for more details.</span></span>

### <a name="providing-unit-of-measure-annotations"></a><span data-ttu-id="f3dc5-393">Birim ölçü ek açıklamaları sağlama</span><span class="sxs-lookup"><span data-stu-id="f3dc5-393">Providing Unit of Measure Annotations</span></span>

<span data-ttu-id="f3dc5-394">ProvidedTypes API ölçü ek açıklamalar sağlayan Yardımcıları sağlar.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-394">The ProvidedTypes API provides helpers for providing measure annotations.</span></span> <span data-ttu-id="f3dc5-395">Örneğin, türü sağlamak için `float<kg>`, aşağıdaki kodu kullanın:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-395">For example, to provide the type `float<kg>`, use the following code:</span></span>
...
```
  ...
  <span data-ttu-id="f3dc5-396">Türü sağlamak `Nullable<decimal<kg/m^2>>`, aşağıdaki kodu kullanın:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-396">To provide the type `Nullable<decimal<kg/m^2>>`, use the following code:</span></span>
...
### <a name="accessing-project-local-or-script-local-resources"></a><span data-ttu-id="f3dc5-397">Proje yerel veya komut dosyası yerel kaynaklara erişme</span><span class="sxs-lookup"><span data-stu-id="f3dc5-397">Accessing Project-Local or Script-Local Resources</span></span>

<span data-ttu-id="f3dc5-398">Her bir tür sağlayıcısı örneğini verilen bir `TypeProviderConfig` oluşturma sırasında değeri.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-398">Each instance of a type provider can be given a `TypeProviderConfig` value during construction.</span></span> <span data-ttu-id="f3dc5-399">Bu değer, "Çözüm klasörü" sağlayıcısı (diğer bir deyişle, proje klasörü derleme veya bir komut dosyasını içeren dizin), başvurulan derlemelerin listesini ve diğer bilgileri içerir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-399">This value contains the "resolution folder" for the provider (that is, the project folder for the compilation or the directory that contains a script), the list of referenced assemblies, and other information.</span></span>

### <a name="invalidation"></a><span data-ttu-id="f3dc5-400">Geçersiz kılma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-400">Invalidation</span></span>

<span data-ttu-id="f3dc5-401">Sağlayıcıları şema varsayımların değişmiş olan F # dil hizmeti bildirmek için geçersiz kılma sinyalleri yükseltebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-401">Providers can raise invalidation signals to notify the F# language service that the schema assumptions may have changed.</span></span> <span data-ttu-id="f3dc5-402">Geçersiz kılma meydana geldiğinde, sağlayıcı Visual Studio'da barındırılıyorsa bir typecheck alınabilir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-402">When invalidation occurs, a typecheck is redone if the provider is being hosted in Visual Studio.</span></span> <span data-ttu-id="f3dc5-403">Bu sinyal sağlayıcısı F # Etkileşimli veya F # derleyici (fsc.exe) tarafından barındırıldığında yoksayılacak.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-403">This signal will be ignored when the provider is hosted in F# Interactive or by the F# Compiler (fsc.exe).</span></span>

### <a name="caching-schema-information"></a><span data-ttu-id="f3dc5-404">Şema bilgileri önbelleğe alma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-404">Caching Schema Information</span></span>

<span data-ttu-id="f3dc5-405">Sağlayıcıları, genellikle erişim için şema bilgileri önbelleğe gerekir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-405">Providers must often cache access to schema information.</span></span> <span data-ttu-id="f3dc5-406">Kullanıcı verileri veya statik bir parametre olarak verilen dosya adı kullanarak önbelleğe alınmış verilerin depolanması gerekir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-406">The cached data should be stored by using a file name that's given as a static parameter or as user data.</span></span> <span data-ttu-id="f3dc5-407">Şema önbelleğe alma, bir örnek verilmiştir `LocalSchemaFile` tür sağlayıcıları parametresinde `FSharp.Data.TypeProviders` derleme.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-407">An example of schema caching is the `LocalSchemaFile` parameter in the type providers in the `FSharp.Data.TypeProviders` assembly.</span></span> <span data-ttu-id="f3dc5-408">Uygulama bu sağlayıcıları, ağ üzerinden veri kaynağına erişim yerine yerel belirtilen dosyada şema bilgileri kullanmak için tür sağlayıcısını statik Bu parametre yönlendirir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-408">In the implementation of these providers, this static parameter directs the type provider to use the schema information in the specified local file instead of accessing the data source over the network.</span></span> <span data-ttu-id="f3dc5-409">Önbelleğe alınan şema bilgileri kullanmak için de statik parametresinin ayarlamalısınız `ForceUpdate` için `false`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-409">To use cached schema information, you must also set the static parameter `ForceUpdate` to `false`.</span></span> <span data-ttu-id="f3dc5-410">Benzer bir teknik, çevrimiçi ve çevrimdışı veri erişimi etkinleştirmek için kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-410">You could use a similar technique to enable online and offline data access.</span></span>

### <a name="backing-assembly"></a><span data-ttu-id="f3dc5-411">Derleme yedekleme</span><span class="sxs-lookup"><span data-stu-id="f3dc5-411">Backing Assembly</span></span>

<span data-ttu-id="f3dc5-412">Derleme yaparken bir `.dll` veya `.exe` dosyası, yedekleme .dll dosyası oluşturulan türleri statik olarak bağlanan elde edilen bütünleştirilmiş kod içine.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-412">When you compile a `.dll` or `.exe` file, the backing .dll file for generated types is statically linked into the resulting assembly.</span></span> <span data-ttu-id="f3dc5-413">Bu bağlantı, son derlemeye yedekleme derlemeye Ara dil (IL) tür tanımları ve yönetilen kaynaklar kopyalayarak oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-413">This link is created by copying the Intermediate Language (IL) type definitions and any managed resources from the backing assembly into the final assembly.</span></span> <span data-ttu-id="f3dc5-414">F # Etkileşimli kullandığınızda, yedekleme .dll dosyası kopyalanmıyor ve bunun yerine doğrudan F # Etkileşimli işlem yüklenir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-414">When you use F# Interactive, the backing .dll file isn't copied and is instead loaded directly into the F# Interactive process.</span></span>

### <a name="exceptions-and-diagnostics-from-type-providers"></a><span data-ttu-id="f3dc5-415">Özel durumlar ve tür sağlayıcıları'ndan tanılama</span><span class="sxs-lookup"><span data-stu-id="f3dc5-415">Exceptions and Diagnostics from Type Providers</span></span>

<span data-ttu-id="f3dc5-416">Sağlanan türlerinden tüm üyeleri tüm kullanımları, özel durumlar.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-416">All uses of all members from provided types may throw exceptions.</span></span> <span data-ttu-id="f3dc5-417">Bir tür sağlayıcısı bir özel durum oluşturursa tüm durumlarda, belirli bir tür sağlayıcısı için konak derleyici hata öznitelikleri.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-417">In all cases, if a type provider throws an exception, the host compiler attributes the error to a specific type provider.</span></span>

- <span data-ttu-id="f3dc5-418">Tür sağlayıcısı özel durumlar, hiçbir zaman içinde iç derleyici hatalarının neden olur.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-418">Type provider exceptions should never result in internal compiler errors.</span></span>

- <span data-ttu-id="f3dc5-419">Tür sağlayıcıları uyarıları bildiremezsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-419">Type providers can't report warnings.</span></span>

- <span data-ttu-id="f3dc5-420">Bir tür Sağlayıcı F # derleyicisi, bir F # geliştirme ortamı veya F # Etkileşimli içinde barındırıldığında, tüm özel durumlar söz konusu sağlayıcısından yakalanır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-420">When a type provider is hosted in the F# compiler, an F# development environment, or F# Interactive, all exceptions from that provider are caught.</span></span> <span data-ttu-id="f3dc5-421">İleti özelliği her zaman hata metnini ve yığın izlemesi yok görünür.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-421">The Message property is always the error text, and no stack trace appears.</span></span> <span data-ttu-id="f3dc5-422">Bir özel durum oluşturmak için kullanacaksanız, aşağıdaki örneklerde oluşturabilecek: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-422">If you’re going to throw an exception, you can throw the following examples: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span></span>

#### <a name="providing-generated-types"></a><span data-ttu-id="f3dc5-423">Oluşturulan türleri sağlama</span><span class="sxs-lookup"><span data-stu-id="f3dc5-423">Providing Generated Types</span></span>

<span data-ttu-id="f3dc5-424">Şu ana kadar bu belgede silinen türler sağlamak üzere nasıl açıklandığı.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-424">So far, this document has explained how to provide erased types.</span></span> <span data-ttu-id="f3dc5-425">Tür sağlayıcısı mekanizması F # gerçek .NET türü tanımları kullanıcıların programı olarak eklenen oluşturulan türler sağlamak üzere kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-425">You can also use the type provider mechanism in F# to provide generated types, which are added as real .NET type definitions into the users' program.</span></span> <span data-ttu-id="f3dc5-426">Oluşturulan türleri bir tür tanımı kullanarak sağlanan başvurmanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-426">You must refer to generated provided types by using a type definition.</span></span>
...
<span data-ttu-id="f3dc5-427">F # 3.0 yayının parçası ProvidedTypes 0.2 yardımcı kod, yalnızca oluşturulan türleri sağlamak için destek sınırlıdır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-427">The ProvidedTypes-0.2 helper code that is part of the F# 3.0 release has only limited support for providing generated types.</span></span> <span data-ttu-id="f3dc5-428">Aşağıdaki deyimleri için oluşturulan tür tanımı true olması gerekir:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-428">The following statements must be true for a generated type definition:</span></span>

- <span data-ttu-id="f3dc5-429">`isErased` ayarlanmalıdır `false`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-429">`isErased` must be set to `false`.</span></span>

- <span data-ttu-id="f3dc5-430">Oluşturulan tür eklenmelidir için yeni oluşturulan `ProvidedAssembly()`, oluşturulan kod parçaları için bir kapsayıcıyı temsil eder.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-430">The generated type must be added to a newly constructed `ProvidedAssembly()`, which represents a container for generated code fragments.</span></span>

- <span data-ttu-id="f3dc5-431">Sağlayıcının bir gerçek destekleyen .NET .dll dosyası ile eşleşen bir .dll dosyası diskte sahip derlemeye olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-431">The provider must have an assembly that has an actual backing .NET .dll file with a matching .dll file on disk.</span></span>

## <a name="rules-and-limitations"></a><span data-ttu-id="f3dc5-432">Kurallar ve sınırlamalar</span><span class="sxs-lookup"><span data-stu-id="f3dc5-432">Rules and Limitations</span></span>

<span data-ttu-id="f3dc5-433">Tür sağlayıcıları yazdığınızda, aşağıdaki kurallar ve sınırlamalar göz önünde bulundurun.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-433">When you write type providers, keep the following rules and limitations in mind.</span></span>

### <a name="provided-types-must-be-reachable"></a><span data-ttu-id="f3dc5-434">Sağlanan türler erişilebilir olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-434">Provided types must be reachable</span></span>

<span data-ttu-id="f3dc5-435">Tüm türler iç içe olmayan türlerinden erişilebilmeli sağlanır.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-435">All provided types should be reachable from the non-nested types.</span></span> <span data-ttu-id="f3dc5-436">İç içe türleri çağrısında verilen `TypeProviderForNamespaces` oluşturucu veya çağrı `AddNamespace`.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-436">The non-nested types are given in the call to the `TypeProviderForNamespaces` constructor or a call to `AddNamespace`.</span></span> <span data-ttu-id="f3dc5-437">Örneğin, bir tür sağlayıcısı sağlıyorsa `StaticClass.P : T`, T iç içe türü veya altında iç içe geçmiş bir tane olduğundan emin olmalısınız.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-437">For example, if the provider provides a type `StaticClass.P : T`, you must ensure that T is either a non-nested type or nested under one.</span></span>

<span data-ttu-id="f3dc5-438">Örneğin, bazı sağlayıcılar gibi statik bir sınıf sahip `DataTypes` bunlar içeren `T1, T2, T3, ...` türleri.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-438">For example, some providers have a static class such as `DataTypes` that contain these `T1, T2, T3, ...` types.</span></span> <span data-ttu-id="f3dc5-439">Aksi takdirde, derlemesi T türünde bir başvuru bulundu, ancak türü bu derlemede bulunamadı hatası diyor.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-439">Otherwise, the error says that a reference to type T in assembly A was found, but the type couldn't be found in that assembly.</span></span> <span data-ttu-id="f3dc5-440">Bu hata yeniden görüntülenirse, tüm alt türleri sağlayıcı türlerinden erişilebildiğini doğrulayın.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-440">If this error appears, verify that all your subtypes can be reached from the provider types.</span></span> <span data-ttu-id="f3dc5-441">Not: Bu `T1, T2, T3...` türleri denir *üzerinde halindeyken* türleri.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-441">Note: These `T1, T2, T3...` types are referred to as the *on-the-fly* types.</span></span> <span data-ttu-id="f3dc5-442">Erişilebilir bir ad alanı veya üst türü koymayı unutmayın.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-442">Remember to put them in an accessible namespace or a parent type.</span></span>

### <a name="limitations-of-the-type-provider-mechanism"></a><span data-ttu-id="f3dc5-443">Tür sağlayıcısı mekanizması sınırlamaları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-443">Limitations of the Type Provider Mechanism</span></span>

<span data-ttu-id="f3dc5-444">F # tür sağlayıcısı mekanizması aşağıdaki sınırlamalara sahiptir:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-444">The type provider mechanism in F# has the following limitations:</span></span>

- <span data-ttu-id="f3dc5-445">Altyapının F # tür sağlayıcıları için genel türleri veya genel yöntemler sağlanan sağlanan desteklemiyor.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-445">The underlying infrastructure for type providers in F# doesn't support provided generic types or provided generic methods.</span></span>

- <span data-ttu-id="f3dc5-446">Mekanizması statik parametreler ile iç içe geçmiş türlerini desteklemez.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-446">The mechanism doesn't support nested types with static parameters.</span></span>

## <a name="development-tips"></a><span data-ttu-id="f3dc5-447">Geliştirme İpuçları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-447">Development Tips</span></span>

<span data-ttu-id="f3dc5-448">Aşağıdaki ipuçları yardımcı geliştirme sürecinde bulabilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-448">You might find the following tips helpful during the development process:</span></span>

### <a name="run-two-instances-of-visual-studio"></a><span data-ttu-id="f3dc5-449">Visual Studio iki örneğini çalıştırma</span><span class="sxs-lookup"><span data-stu-id="f3dc5-449">Run two instances of Visual Studio</span></span>

<span data-ttu-id="f3dc5-450">Tür sağlayıcısını bir örneğinde geliştirip test IDE yeniden oluşturulan tür sağlayıcısını engelleyen bir .dll dosyası üzerinde bir kilit etkinleştirilir çünkü sağlayıcı diğer test edebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-450">You can develop the type provider in one instance and test the provider in the other because the test IDE will take a lock on the .dll file that prevents the type provider from being rebuilt.</span></span> <span data-ttu-id="f3dc5-451">Bu nedenle, Visual Studio'nun ikinci örneğini ağdaki ilk örnek sağlayıcısı oluşturulur ve ardından sağlayıcısı oluşturulduktan sonra ikinci örneğini yeniden açmanız gerekir kapatmanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-451">Thus, you must close the second instance of Visual Studio while the provider is built in the first instance, and then you must reopen the second instance after the provider is built.</span></span>

### <a name="debug-type-providers-by-using-invocations-of-fscexe"></a><span data-ttu-id="f3dc5-452">Tür sağlayıcıları fsc.exe çağrıları kullanarak hata ayıklama</span><span class="sxs-lookup"><span data-stu-id="f3dc5-452">Debug type providers by using invocations of fsc.exe</span></span>

<span data-ttu-id="f3dc5-453">Tür sağlayıcıları aşağıdaki araçları kullanarak çağırabilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="f3dc5-453">You can invoke type providers by using the following tools:</span></span>

- <span data-ttu-id="f3dc5-454">fsc.exe (F # komut satırı derleyicisi)</span><span class="sxs-lookup"><span data-stu-id="f3dc5-454">fsc.exe (The F# command line compiler)</span></span>

- <span data-ttu-id="f3dc5-455">fsi.exe (F # Etkileşimli derleyici)</span><span class="sxs-lookup"><span data-stu-id="f3dc5-455">fsi.exe (The F# Interactive compiler)</span></span>

- <span data-ttu-id="f3dc5-456">Devenv.exe (Visual Studio)</span><span class="sxs-lookup"><span data-stu-id="f3dc5-456">devenv.exe (Visual Studio)</span></span>

<span data-ttu-id="f3dc5-457">Tür sağlayıcıları test betiği (örneğin, script.fsx) üzerinde fsc.exe kullanarak, en kolay genellikle ayıklayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-457">You can often debug type providers most easily by using fsc.exe on a test script file (for example, script.fsx).</span></span> <span data-ttu-id="f3dc5-458">Bir hata ayıklayıcı bir komut isteminden başlatabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-458">You can launch a debugger from a command prompt.</span></span>

```
  devenv /debugexe fsc.exe script.fsx
```

  <span data-ttu-id="f3dc5-459">Yazdırma için stdout'u günlüğe kaydetme kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-459">You can use print-to-stdout logging.</span></span>

## <a name="see-also"></a><span data-ttu-id="f3dc5-460">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="f3dc5-460">See also</span></span>

- [<span data-ttu-id="f3dc5-461">Tür Sağlayıcıları</span><span class="sxs-lookup"><span data-stu-id="f3dc5-461">Type Providers</span></span>](index.md)
- [<span data-ttu-id="f3dc5-462">Tür sağlayıcısını SDK'sı</span><span class="sxs-lookup"><span data-stu-id="f3dc5-462">The Type Provider SDK</span></span>](https://github.com/fsprojects/FSharp.TypeProviders.SDK)
