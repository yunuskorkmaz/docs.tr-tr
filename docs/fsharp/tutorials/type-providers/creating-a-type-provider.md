...
- <span data-ttu-id="e43db-112">[FSharp. Data. TypeProviders](https://fsprojects.github.io/FSharp.Data.TypeProviders/) , yalnızca SQL, Entity Framework, OData ve wsdl veri hizmetlerine erişim için .NET Framework programlamayla birlikte kullanılmak üzere daha eski bir tür sağlayıcıları kümesidir.</span><span class="sxs-lookup"><span data-stu-id="e43db-112">[FSharp.Data.TypeProviders](https://fsprojects.github.io/FSharp.Data.TypeProviders/) is an older set of type providers for use only with .NET Framework programming for accessing SQL, Entity Framework, OData and WSDL data services.</span></span>
...
- <span data-ttu-id="e43db-124">Bilgi kaynağınız için bir şemanız mı var?</span><span class="sxs-lookup"><span data-stu-id="e43db-124">Do you have a schema for your information source?</span></span> <span data-ttu-id="e43db-125">Bu durumda, F# ve .net tür sistemine ne eşleme yapılır?</span><span class="sxs-lookup"><span data-stu-id="e43db-125">If so, what’s the mapping into the F# and .NET type system?</span></span>
...
- <span data-ttu-id="e43db-215">Yalnızca veri ve yöntemleri içeren bir bilgi alanı için sağlayıcı yazarken.</span><span class="sxs-lookup"><span data-stu-id="e43db-215">When you are writing a provider for an information space that contains only data and methods.</span></span>
...
### <a name="key-lessons"></a><span data-ttu-id="e43db-310">Temel dersler</span><span class="sxs-lookup"><span data-stu-id="e43db-310">Key Lessons</span></span>
...
- <span data-ttu-id="e43db-329">Üst bilgi adları, birim-daha küçüktür veya "ad (birim)" biçiminde ve virgül içermez.</span><span class="sxs-lookup"><span data-stu-id="e43db-329">Header names are either unit-less or have the form "Name (unit)" and don't contain commas.</span></span>
...
<span data-ttu-id="e43db-343">Aşağıdaki kod, uygulamanın çekirdeğini gösterir.</span><span class="sxs-lookup"><span data-stu-id="e43db-343">The following code shows the core of the implementation.</span></span>
...
- <span data-ttu-id="e43db-345">Aşırı yüklenmiş oluşturucular, özgün dosyanın veya özdeş bir şemaya sahip olan birinin okunmasını sağlar.</span><span class="sxs-lookup"><span data-stu-id="e43db-345">Overloaded constructors allow either the original file or one that has an identical schema to be read.</span></span> <span data-ttu-id="e43db-346">Bu model, yerel veya uzak veri kaynakları için bir tür sağlayıcısı yazdığınızda yaygındır ve bu model yerel bir dosyanın uzak veriler için şablon olarak kullanılmasına izin verir.</span><span class="sxs-lookup"><span data-stu-id="e43db-346">This pattern is common when you write a type provider for local or remote data sources, and this pattern allows a local file to be used as the template for remote data.</span></span>

- <span data-ttu-id="e43db-347">Göreli dosya adlarını çözümlemek için tür sağlayıcısı oluşturucusuna geçirilen [TypeProviderConfig](https://msdn.microsoft.com/library/1cda7b9a-3d07-475d-9315-d65e1c97eb44) değerini kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-347">You can use the [TypeProviderConfig](https://msdn.microsoft.com/library/1cda7b9a-3d07-475d-9315-d65e1c97eb44) value that’s passed in to the type provider constructor to resolve relative file names.</span></span>

- <span data-ttu-id="e43db-348">Yöntemi, `AddDefinitionLocation` belirtilen özelliklerin konumunu tanımlamak için kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-348">You can use the `AddDefinitionLocation` method to define the location of the provided properties.</span></span> <span data-ttu-id="e43db-349">Bu nedenle, sağlanmış bir özellik üzerindekullanıyorsanız,CSVdosyasıVisualStudio'daaçılır.`Go To Definition`</span><span class="sxs-lookup"><span data-stu-id="e43db-349">Therefore, if you use `Go To Definition` on a provided property, the CSV file will open in Visual Studio.</span></span>

- <span data-ttu-id="e43db-350">Türü, `ProvidedMeasureBuilder` sı birimleri aramak ve ilgili `float<_>` türleri oluşturmak için kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-350">You can use the `ProvidedMeasureBuilder` type to look up the SI units and to generate the relevant `float<_>` types.</span></span>

### <a name="key-lessons"></a><span data-ttu-id="e43db-351">Temel dersler</span><span class="sxs-lookup"><span data-stu-id="e43db-351">Key Lessons</span></span>

<span data-ttu-id="e43db-352">Bu bölümde, veri kaynağının içinde bulunan basit bir şemaya sahip bir yerel veri kaynağı için bir tür sağlayıcısı oluşturma konusu açıklanmaktadır.</span><span class="sxs-lookup"><span data-stu-id="e43db-352">This section explained how to create a type provider for a local data source with a simple schema that's contained in the data source itself.</span></span>

## <a name="going-further"></a><span data-ttu-id="e43db-353">Daha fazla</span><span class="sxs-lookup"><span data-stu-id="e43db-353">Going Further</span></span>

<span data-ttu-id="e43db-354">Aşağıdaki bölümler, daha fazla çalışma için öneriler içerir.</span><span class="sxs-lookup"><span data-stu-id="e43db-354">The following sections include suggestions for further study.</span></span>

### <a name="a-look-at-the-compiled-code-for-erased-types"></a><span data-ttu-id="e43db-355">Silinen türler için derlenmiş koda göz atın</span><span class="sxs-lookup"><span data-stu-id="e43db-355">A Look at the Compiled Code for Erased Types</span></span>

<span data-ttu-id="e43db-356">Tür sağlayıcısının kullanımına yayılan koda karşılık gelen bir fikir vermek için, bu konunun önceki kısımlarında kullanılan öğesini kullanarak `HelloWorldTypeProvider` aşağıdaki işleve bakın.</span><span class="sxs-lookup"><span data-stu-id="e43db-356">To give you some idea of how the use of the type provider corresponds to the code that's emitted, look at the following function by using the `HelloWorldTypeProvider` that's used earlier in this topic.</span></span>
...
<span data-ttu-id="e43db-357">Ildadsm. exe kullanılarak derlenen ortaya çıkan kodun bir görüntüsü aşağıda verilmiştir:</span><span class="sxs-lookup"><span data-stu-id="e43db-357">Here’s an image of the resulting code decompiled by using ildasm.exe:</span></span>
...
<span data-ttu-id="e43db-358">Örnekte gösterildiği gibi, türünün `Type1` `InstanceProperty` ve özelliğinin tüm bahsetmeler silinmiş ve yalnızca ilgili çalışma zamanı türlerinde işlemler bırakılıyor.</span><span class="sxs-lookup"><span data-stu-id="e43db-358">As the example shows, all mentions of the type `Type1` and the `InstanceProperty` property have been erased, leaving only operations on the runtime types involved.</span></span>

### <a name="design-and-naming-conventions-for-type-providers"></a><span data-ttu-id="e43db-359">Tür sağlayıcıları için tasarım ve adlandırma kuralları</span><span class="sxs-lookup"><span data-stu-id="e43db-359">Design and Naming Conventions for Type Providers</span></span>

<span data-ttu-id="e43db-360">Tür sağlayıcıları yazarken aşağıdaki kuralları gözlemleyin.</span><span class="sxs-lookup"><span data-stu-id="e43db-360">Observe the following conventions when authoring type providers.</span></span>

<span data-ttu-id="e43db-361">**Bağlantı protokolleri Için sağlayıcılar** Genel olarak, veri ve hizmet bağlantı protokollerinin (OData veya SQL bağlantıları gibi) çoğu için sağlayıcı dll 'lerinin adları `TypeProvider` veya `TypeProviders`ile bitmelidir.</span><span class="sxs-lookup"><span data-stu-id="e43db-361">**Providers for Connectivity Protocols** In general, names of most provider DLLs for data and service connectivity protocols, such as OData or SQL connections, should end in `TypeProvider` or `TypeProviders`.</span></span> <span data-ttu-id="e43db-362">Örneğin, aşağıdaki dizeye benzer bir DLL adı kullanın:</span><span class="sxs-lookup"><span data-stu-id="e43db-362">For example, use a DLL name that resembles the following string:</span></span>

```
  Fabrikam.Management.BasicTypeProviders.dll
```

<span data-ttu-id="e43db-363">Belirttiğiniz türlerin karşılık gelen ad alanının üyesi olduğundan emin olun ve uygulanan bağlantı protokolünü belirtin:</span><span class="sxs-lookup"><span data-stu-id="e43db-363">Ensure that your provided types are members of the corresponding namespace, and indicate the connectivity protocol that you implemented:</span></span>
...
<span data-ttu-id="e43db-364">**Genel kodlama Için yardımcı program sağlayıcıları**.</span><span class="sxs-lookup"><span data-stu-id="e43db-364">**Utility Providers for General Coding**.</span></span>  <span data-ttu-id="e43db-365">Normal ifadeler gibi bir yardımcı program türü sağlayıcı için, aşağıdaki örnekte gösterildiği gibi tür sağlayıcısı bir temel kitaplığın parçası olabilir:</span><span class="sxs-lookup"><span data-stu-id="e43db-365">For a utility type provider such as that for regular expressions, the type provider may be part of a base library, as the following example shows:</span></span>

```fsharp
#r "Fabrikam.Core.Text.Utilities.dll"
```

<span data-ttu-id="e43db-366">Bu durumda, sunulan tür normal .NET tasarım kurallarına göre uygun bir noktada görünür:</span><span class="sxs-lookup"><span data-stu-id="e43db-366">In this case, the provided type would appear at an appropriate point according to normal .NET design conventions:</span></span>
...
<span data-ttu-id="e43db-367">**Tek veri kaynakları**.</span><span class="sxs-lookup"><span data-stu-id="e43db-367">**Singleton Data Sources**.</span></span> <span data-ttu-id="e43db-368">Bazı tür sağlayıcıları tek bir ayrılmış veri kaynağına bağlanır ve yalnızca verileri sağlar.</span><span class="sxs-lookup"><span data-stu-id="e43db-368">Some type providers connect to a single dedicated data source and provide only data.</span></span> <span data-ttu-id="e43db-369">Bu durumda, `TypeProvider` son eki bırakmalısınız ve .net adlandırma için normal kuralları kullanmanız gerekir:</span><span class="sxs-lookup"><span data-stu-id="e43db-369">In this case, you should drop the `TypeProvider` suffix and use normal conventions for .NET naming:</span></span>
...
<span data-ttu-id="e43db-370">Daha fazla bilgi için, bu `GetConnection` konunun ilerleyen kısımlarında açıklanan tasarım kuralına bakın.</span><span class="sxs-lookup"><span data-stu-id="e43db-370">For more information, see the `GetConnection` design convention that's described later in this topic.</span></span>

### <a name="design-patterns-for-type-providers"></a><span data-ttu-id="e43db-371">Tür sağlayıcıları için tasarım desenleri</span><span class="sxs-lookup"><span data-stu-id="e43db-371">Design Patterns for Type Providers</span></span>

<span data-ttu-id="e43db-372">Aşağıdaki bölümlerde, tür sağlayıcıları yazarken kullanabileceğiniz tasarım desenleri açıklanır.</span><span class="sxs-lookup"><span data-stu-id="e43db-372">The following sections describe design patterns you can use when authoring type providers.</span></span>

#### <a name="the-getconnection-design-pattern"></a><span data-ttu-id="e43db-373">GetConnection tasarım deseninin</span><span class="sxs-lookup"><span data-stu-id="e43db-373">The GetConnection Design Pattern</span></span>

<span data-ttu-id="e43db-374">Aşağıdaki örnekte gösterildiği gibi, FSharp. Data. `GetConnection` TypeProviders. dll içindeki tür sağlayıcıları tarafından kullanılan kalıbı kullanmak için çoğu tür sağlayıcı yazılmalıdır:</span><span class="sxs-lookup"><span data-stu-id="e43db-374">Most type providers should be written to use the `GetConnection` pattern that's used by the type providers in FSharp.Data.TypeProviders.dll, as the following example shows:</span></span>
...
#### <a name="type-providers-backed-by-remote-data-and-services"></a><span data-ttu-id="e43db-375">Uzak veri ve hizmetler tarafından desteklenen tür sağlayıcıları</span><span class="sxs-lookup"><span data-stu-id="e43db-375">Type Providers Backed By Remote Data and Services</span></span>

<span data-ttu-id="e43db-376">Uzak veri ve hizmetler tarafından desteklenen bir tür sağlayıcısı oluşturmadan önce, bağlı programlamaya ait bir dizi sorunu düşünmeniz gerekir.</span><span class="sxs-lookup"><span data-stu-id="e43db-376">Before you create a type provider that's backed by remote data and services, you must consider a range of issues that are inherent in connected programming.</span></span> <span data-ttu-id="e43db-377">Bu sorunlar aşağıdaki konuları içerir:</span><span class="sxs-lookup"><span data-stu-id="e43db-377">These issues include the following considerations:</span></span>

- <span data-ttu-id="e43db-378">Şema eşleme</span><span class="sxs-lookup"><span data-stu-id="e43db-378">schema mapping</span></span>

- <span data-ttu-id="e43db-379">şema değişikliği durumunda liilte ve geçersiz kılma</span><span class="sxs-lookup"><span data-stu-id="e43db-379">liveness and invalidation in the presence of schema change</span></span>

- <span data-ttu-id="e43db-380">şema önbelleğe alma</span><span class="sxs-lookup"><span data-stu-id="e43db-380">schema caching</span></span>

- <span data-ttu-id="e43db-381">veri erişim işlemlerinin zaman uyumsuz uygulamaları</span><span class="sxs-lookup"><span data-stu-id="e43db-381">asynchronous implementations of data access operations</span></span>

- <span data-ttu-id="e43db-382">LINQ sorguları dahil destekleyici sorgular</span><span class="sxs-lookup"><span data-stu-id="e43db-382">supporting queries, including LINQ queries</span></span>

- <span data-ttu-id="e43db-383">kimlik bilgileri ve kimlik doğrulama</span><span class="sxs-lookup"><span data-stu-id="e43db-383">credentials and authentication</span></span>

<span data-ttu-id="e43db-384">Bu konu, bu sorunları daha ayrıntılı bir şekilde araştırmaz.</span><span class="sxs-lookup"><span data-stu-id="e43db-384">This topic doesn't explore these issues further.</span></span>

### <a name="additional-authoring-techniques"></a><span data-ttu-id="e43db-385">Ek yazma teknikleri</span><span class="sxs-lookup"><span data-stu-id="e43db-385">Additional Authoring Techniques</span></span>

<span data-ttu-id="e43db-386">Kendi tür sağlayıcılarınızı yazdığınızda, aşağıdaki ek teknikleri kullanmak isteyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-386">When you write your own type providers, you might want to use the following additional techniques.</span></span>

### <a name="creating-types-and-members-on-demand"></a><span data-ttu-id="e43db-387">Istek üzerine türler ve Üyeler oluşturma</span><span class="sxs-lookup"><span data-stu-id="e43db-387">Creating Types and Members On-Demand</span></span>

<span data-ttu-id="e43db-388">ProvidedType API 'SI, AddMember 'ın gecikmeli sürümlerini içerir.</span><span class="sxs-lookup"><span data-stu-id="e43db-388">The ProvidedType API has delayed versions of AddMember.</span></span>
...
<span data-ttu-id="e43db-389">Bu sürümler, türlerin isteğe bağlı alanlarını oluşturmak için kullanılır.</span><span class="sxs-lookup"><span data-stu-id="e43db-389">These versions are used to create on-demand spaces of types.</span></span>

### <a name="providing-array-types-and-generic-type-instantiations"></a><span data-ttu-id="e43db-390">Dizi türleri ve genel tür örneklemeleri sağlama</span><span class="sxs-lookup"><span data-stu-id="e43db-390">Providing Array types and Generic Type Instantiations</span></span>

<span data-ttu-id="e43db-391">`MakeArrayType`,, `MakeGenericType` Vedahil`MakePointerType`olmak üzere herhangibirörneğindenormal,,ve'yikullanarak,belirtilenüyeleri(imzalarıdizitürleri,ByReftürlerivegeneltürlerinörneklemeleridahil)yaparsınız.`ProvidedTypeDefinitions` <xref:System.Type></span><span class="sxs-lookup"><span data-stu-id="e43db-391">You make provided members (whose signatures include array types, byref types, and instantiations of generic types) by using the normal `MakeArrayType`, `MakePointerType`, and `MakeGenericType` on any instance of <xref:System.Type>, including `ProvidedTypeDefinitions`.</span></span>

> [!NOTE]
> <span data-ttu-id="e43db-392">Bazı durumlarda, içinde `ProvidedTypeBuilder.MakeGenericType`yardımcı kullanmanız gerekebilir.</span><span class="sxs-lookup"><span data-stu-id="e43db-392">In some cases you may have to use the helper in `ProvidedTypeBuilder.MakeGenericType`.</span></span>  <span data-ttu-id="e43db-393">Daha fazla bilgi için bkz. [tür sağlayıcısı SDK belgeleri](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) .</span><span class="sxs-lookup"><span data-stu-id="e43db-393">See the [Type Provider SDK documentation](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) for more details.</span></span>

### <a name="providing-unit-of-measure-annotations"></a><span data-ttu-id="e43db-394">Ölçü birimi ek açıklamaları sağlama</span><span class="sxs-lookup"><span data-stu-id="e43db-394">Providing Unit of Measure Annotations</span></span>

<span data-ttu-id="e43db-395">ProvidedTypes API 'SI, ölçü ek açıklamaları sağlamak için yardımcılar sağlar.</span><span class="sxs-lookup"><span data-stu-id="e43db-395">The ProvidedTypes API provides helpers for providing measure annotations.</span></span> <span data-ttu-id="e43db-396">Örneğin, türü `float<kg>`sağlamak için aşağıdaki kodu kullanın:</span><span class="sxs-lookup"><span data-stu-id="e43db-396">For example, to provide the type `float<kg>`, use the following code:</span></span>
...
```
  ...
  <span data-ttu-id="e43db-397">Türü `Nullable<decimal<kg/m^2>>`sağlamak için aşağıdaki kodu kullanın:</span><span class="sxs-lookup"><span data-stu-id="e43db-397">To provide the type `Nullable<decimal<kg/m^2>>`, use the following code:</span></span>
...
### <a name="accessing-project-local-or-script-local-resources"></a><span data-ttu-id="e43db-398">Proje-yerel veya betiğe yerel kaynaklara erişme</span><span class="sxs-lookup"><span data-stu-id="e43db-398">Accessing Project-Local or Script-Local Resources</span></span>

<span data-ttu-id="e43db-399">Bir tür sağlayıcının her örneğine, oluşturma sırasında bir `TypeProviderConfig` değer verilebilir.</span><span class="sxs-lookup"><span data-stu-id="e43db-399">Each instance of a type provider can be given a `TypeProviderConfig` value during construction.</span></span> <span data-ttu-id="e43db-400">Bu değer, sağlayıcının "çözüm klasörünü" içerir (yani, derleme için proje klasörü veya bir betiği içeren dizin), başvurulan derlemelerin listesi ve diğer bilgiler.</span><span class="sxs-lookup"><span data-stu-id="e43db-400">This value contains the "resolution folder" for the provider (that is, the project folder for the compilation or the directory that contains a script), the list of referenced assemblies, and other information.</span></span>

### <a name="invalidation"></a><span data-ttu-id="e43db-401">Geçersiz kılma</span><span class="sxs-lookup"><span data-stu-id="e43db-401">Invalidation</span></span>

<span data-ttu-id="e43db-402">Sağlayıcılar, F# dil hizmetine şema varsayımlarında değişiklik olduğunu bildirmek için doğrulama sinyalleri oluşturabilir.</span><span class="sxs-lookup"><span data-stu-id="e43db-402">Providers can raise invalidation signals to notify the F# language service that the schema assumptions may have changed.</span></span> <span data-ttu-id="e43db-403">Invalidation gerçekleştiğinde, sağlayıcı Visual Studio 'da barındırılıyorsa bir typecheck, yinelendi olur.</span><span class="sxs-lookup"><span data-stu-id="e43db-403">When invalidation occurs, a typecheck is redone if the provider is being hosted in Visual Studio.</span></span> <span data-ttu-id="e43db-404">Bu sinyal, sağlayıcı F# etkileşimli veya F# derleyici (FSC. exe) tarafından barındırılıyorsa yok sayılır.</span><span class="sxs-lookup"><span data-stu-id="e43db-404">This signal will be ignored when the provider is hosted in F# Interactive or by the F# Compiler (fsc.exe).</span></span>

### <a name="caching-schema-information"></a><span data-ttu-id="e43db-405">Şema bilgilerini önbelleğe alma</span><span class="sxs-lookup"><span data-stu-id="e43db-405">Caching Schema Information</span></span>

<span data-ttu-id="e43db-406">Sağlayıcılar genellikle şema bilgilerine erişimi önbelleğe almalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-406">Providers must often cache access to schema information.</span></span> <span data-ttu-id="e43db-407">Önbelleğe alınan veriler, statik bir parametre veya Kullanıcı verileri olarak verilen bir dosya adı kullanılarak depolanmalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-407">The cached data should be stored by using a file name that's given as a static parameter or as user data.</span></span> <span data-ttu-id="e43db-408">Şema önbelleğe alma `LocalSchemaFile` örneği, `FSharp.Data.TypeProviders` derlemedeki tür sağlayıcılarındaki parametredir.</span><span class="sxs-lookup"><span data-stu-id="e43db-408">An example of schema caching is the `LocalSchemaFile` parameter in the type providers in the `FSharp.Data.TypeProviders` assembly.</span></span> <span data-ttu-id="e43db-409">Bu statik parametre, bu sağlayıcıları uygulamada, tür sağlayıcısını ağ üzerinden veri kaynağına erişmek yerine, belirtilen yerel dosyadaki şema bilgilerini kullanacak şekilde yönlendirir.</span><span class="sxs-lookup"><span data-stu-id="e43db-409">In the implementation of these providers, this static parameter directs the type provider to use the schema information in the specified local file instead of accessing the data source over the network.</span></span> <span data-ttu-id="e43db-410">Önbelleğe alınmış şema bilgilerini kullanmak için statik parametresini `ForceUpdate` olarak `false`da ayarlamanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="e43db-410">To use cached schema information, you must also set the static parameter `ForceUpdate` to `false`.</span></span> <span data-ttu-id="e43db-411">Çevrimiçi ve çevrimdışı veri erişimini etkinleştirmek için benzer bir teknik kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-411">You could use a similar technique to enable online and offline data access.</span></span>

### <a name="backing-assembly"></a><span data-ttu-id="e43db-412">Derleme yedekleniyor</span><span class="sxs-lookup"><span data-stu-id="e43db-412">Backing Assembly</span></span>

<span data-ttu-id="e43db-413">Bir `.dll` veya`.exe` dosyası derlerken, oluşturulan türler için yedekleme. dll dosyası, sonuçta elde edilen derlemeye statik olarak bağlanır.</span><span class="sxs-lookup"><span data-stu-id="e43db-413">When you compile a `.dll` or `.exe` file, the backing .dll file for generated types is statically linked into the resulting assembly.</span></span> <span data-ttu-id="e43db-414">Bu bağlantı, ara dil (IL) türü tanımlarını ve yönetilen herhangi bir kaynağı, yedekleme derlemesinden son derlemeye kopyalayarak oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="e43db-414">This link is created by copying the Intermediate Language (IL) type definitions and any managed resources from the backing assembly into the final assembly.</span></span> <span data-ttu-id="e43db-415">Etkileşimli 'yı kullandığınızda F# , yedekleme. dll dosyası kopyalanmaz ve bunun yerine doğrudan F# etkileşimli işleme yüklenir.</span><span class="sxs-lookup"><span data-stu-id="e43db-415">When you use F# Interactive, the backing .dll file isn't copied and is instead loaded directly into the F# Interactive process.</span></span>

### <a name="exceptions-and-diagnostics-from-type-providers"></a><span data-ttu-id="e43db-416">Tür sağlayıcılarından özel durumlar ve Tanılamalar</span><span class="sxs-lookup"><span data-stu-id="e43db-416">Exceptions and Diagnostics from Type Providers</span></span>

<span data-ttu-id="e43db-417">Belirtilen türlerden tüm üyelerin tüm kullanımları özel durumlar oluşturabilir.</span><span class="sxs-lookup"><span data-stu-id="e43db-417">All uses of all members from provided types may throw exceptions.</span></span> <span data-ttu-id="e43db-418">Her durumda, bir tür sağlayıcı bir özel durum oluşturursa, ana bilgisayar derleyicisi hatayı belirli bir tür sağlayıcısına göre öznitelikler.</span><span class="sxs-lookup"><span data-stu-id="e43db-418">In all cases, if a type provider throws an exception, the host compiler attributes the error to a specific type provider.</span></span>

- <span data-ttu-id="e43db-419">Tür sağlayıcısı özel durumları asla iç derleyici hatalarına neden olmamalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-419">Type provider exceptions should never result in internal compiler errors.</span></span>

- <span data-ttu-id="e43db-420">Tür sağlayıcıları uyarıları bildirememektedir.</span><span class="sxs-lookup"><span data-stu-id="e43db-420">Type providers can't report warnings.</span></span>

- <span data-ttu-id="e43db-421">Bir tür sağlayıcısı F# derleyicide, bir F# geliştirme ortamında veya F# etkileşimli olarak barındırılıyorsa, söz konusu sağlayıcıdan gelen tüm özel durumlar yakalanır.</span><span class="sxs-lookup"><span data-stu-id="e43db-421">When a type provider is hosted in the F# compiler, an F# development environment, or F# Interactive, all exceptions from that provider are caught.</span></span> <span data-ttu-id="e43db-422">Ileti özelliği her zaman hata metindir ve yığın izlemesi görünmez.</span><span class="sxs-lookup"><span data-stu-id="e43db-422">The Message property is always the error text, and no stack trace appears.</span></span> <span data-ttu-id="e43db-423">Bir özel durum oluşturacaksanız aşağıdaki örnekleri atabilirsiniz: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span><span class="sxs-lookup"><span data-stu-id="e43db-423">If you’re going to throw an exception, you can throw the following examples: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span></span>

#### <a name="providing-generated-types"></a><span data-ttu-id="e43db-424">Oluşturulan türleri sağlama</span><span class="sxs-lookup"><span data-stu-id="e43db-424">Providing Generated Types</span></span>

<span data-ttu-id="e43db-425">Şimdiye kadar bu belgede, silinen türlerin nasıl sağlanması açıklanmıştı.</span><span class="sxs-lookup"><span data-stu-id="e43db-425">So far, this document has explained how to provide erased types.</span></span> <span data-ttu-id="e43db-426">Ayrıca, ' de F# tür sağlayıcısı mekanizmasını kullanarak, kullanıcıların programına gerçek .net tür tanımları olarak eklenen üretilmiş türleri sağlayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-426">You can also use the type provider mechanism in F# to provide generated types, which are added as real .NET type definitions into the users' program.</span></span> <span data-ttu-id="e43db-427">Bir tür tanımı kullanarak oluşturulan sağlanmış türlere başvurmanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="e43db-427">You must refer to generated provided types by using a type definition.</span></span>
...
<span data-ttu-id="e43db-428">F# 3,0 sürümünün parçası olan ProvidedTypes-0,2 yardımcı kodu yalnızca oluşturulan türleri sağlamak için sınırlı desteğe sahiptir.</span><span class="sxs-lookup"><span data-stu-id="e43db-428">The ProvidedTypes-0.2 helper code that is part of the F# 3.0 release has only limited support for providing generated types.</span></span> <span data-ttu-id="e43db-429">Oluşturulan bir tür tanımı için aşağıdaki deyimler doğru olmalıdır:</span><span class="sxs-lookup"><span data-stu-id="e43db-429">The following statements must be true for a generated type definition:</span></span>

- <span data-ttu-id="e43db-430">`isErased`olarak `false`ayarlanmalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-430">`isErased` must be set to `false`.</span></span>

- <span data-ttu-id="e43db-431">Oluşturulan tür, oluşturulan kod parçaları için bir kapsayıcıyı temsil `ProvidedAssembly()`eden yeni oluşturulmuş bir şekilde eklenmiş olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-431">The generated type must be added to a newly constructed `ProvidedAssembly()`, which represents a container for generated code fragments.</span></span>

- <span data-ttu-id="e43db-432">Sağlayıcıda, diskte eşleşen bir. dll dosyası olan gerçek bir .NET. dll dosyasına sahip bir derleme olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-432">The provider must have an assembly that has an actual backing .NET .dll file with a matching .dll file on disk.</span></span>

## <a name="rules-and-limitations"></a><span data-ttu-id="e43db-433">Kurallar ve sınırlamalar</span><span class="sxs-lookup"><span data-stu-id="e43db-433">Rules and Limitations</span></span>

<span data-ttu-id="e43db-434">Tür sağlayıcılarını yazdığınızda aşağıdaki kuralları ve sınırlamaları aklınızda bulundurun.</span><span class="sxs-lookup"><span data-stu-id="e43db-434">When you write type providers, keep the following rules and limitations in mind.</span></span>

### <a name="provided-types-must-be-reachable"></a><span data-ttu-id="e43db-435">Belirtilen türlere ulaşılabilir olmalıdır</span><span class="sxs-lookup"><span data-stu-id="e43db-435">Provided types must be reachable</span></span>

<span data-ttu-id="e43db-436">Tüm sağlanmış türler, iç içe olmayan türlerden ulaşılabilir olmalıdır.</span><span class="sxs-lookup"><span data-stu-id="e43db-436">All provided types should be reachable from the non-nested types.</span></span> <span data-ttu-id="e43db-437">İç içe olmayan türler, `TypeProviderForNamespaces` oluşturucuya yapılan çağrıda veya öğesine `AddNamespace`yapılan çağrıda verilir.</span><span class="sxs-lookup"><span data-stu-id="e43db-437">The non-nested types are given in the call to the `TypeProviderForNamespaces` constructor or a call to `AddNamespace`.</span></span> <span data-ttu-id="e43db-438">Örneğin, sağlayıcı bir tür `StaticClass.P : T`sağlıyorsa T 'nin, iç içe olmayan bir tür ya da bir iç içe geçmiş olduğundan emin olmanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="e43db-438">For example, if the provider provides a type `StaticClass.P : T`, you must ensure that T is either a non-nested type or nested under one.</span></span>

<span data-ttu-id="e43db-439">Örneğin, bazı sağlayıcılardan bu `DataTypes` `T1, T2, T3, ...` türleri içeren gibi bir statik sınıf vardır.</span><span class="sxs-lookup"><span data-stu-id="e43db-439">For example, some providers have a static class such as `DataTypes` that contain these `T1, T2, T3, ...` types.</span></span> <span data-ttu-id="e43db-440">Aksi takdirde hata, derleme A 'da T türüne başvurunun bulunduğunu söyler, ancak tür Bu derlemede bulunamadı.</span><span class="sxs-lookup"><span data-stu-id="e43db-440">Otherwise, the error says that a reference to type T in assembly A was found, but the type couldn't be found in that assembly.</span></span> <span data-ttu-id="e43db-441">Bu hata görünürse, tüm alt türlerine sağlayıcı türlerinden ulaşılamadığından emin olun.</span><span class="sxs-lookup"><span data-stu-id="e43db-441">If this error appears, verify that all your subtypes can be reached from the provider types.</span></span> <span data-ttu-id="e43db-442">Not: Bu `T1, T2, T3...` türler *, hareket halindeyken* türler olarak adlandırılır.</span><span class="sxs-lookup"><span data-stu-id="e43db-442">Note: These `T1, T2, T3...` types are referred to as the *on-the-fly* types.</span></span> <span data-ttu-id="e43db-443">Bunları erişilebilir bir ad alanına veya üst türe getirmeyi unutmayın.</span><span class="sxs-lookup"><span data-stu-id="e43db-443">Remember to put them in an accessible namespace or a parent type.</span></span>

### <a name="limitations-of-the-type-provider-mechanism"></a><span data-ttu-id="e43db-444">Tür sağlayıcısı mekanizmasıyla ilgili sınırlamalar</span><span class="sxs-lookup"><span data-stu-id="e43db-444">Limitations of the Type Provider Mechanism</span></span>

<span data-ttu-id="e43db-445">İçindeki F# tür sağlayıcı mekanizması aşağıdaki sınırlamalara sahiptir:</span><span class="sxs-lookup"><span data-stu-id="e43db-445">The type provider mechanism in F# has the following limitations:</span></span>

- <span data-ttu-id="e43db-446">İçindeki F# tür sağlayıcılarının temeldeki altyapısı, sağlanmış genel türleri veya sağlanmış genel yöntemleri desteklemez.</span><span class="sxs-lookup"><span data-stu-id="e43db-446">The underlying infrastructure for type providers in F# doesn't support provided generic types or provided generic methods.</span></span>

- <span data-ttu-id="e43db-447">Mekanizması statik parametrelerle iç içe geçmiş türleri desteklemez.</span><span class="sxs-lookup"><span data-stu-id="e43db-447">The mechanism doesn't support nested types with static parameters.</span></span>

## <a name="development-tips"></a><span data-ttu-id="e43db-448">Geliştirme İpuçları</span><span class="sxs-lookup"><span data-stu-id="e43db-448">Development Tips</span></span>

<span data-ttu-id="e43db-449">Geliştirme sürecinde aşağıdaki ipuçlarını yararlı bulabilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="e43db-449">You might find the following tips helpful during the development process:</span></span>

### <a name="run-two-instances-of-visual-studio"></a><span data-ttu-id="e43db-450">Visual Studio 'nun iki örneğini çalıştırın</span><span class="sxs-lookup"><span data-stu-id="e43db-450">Run two instances of Visual Studio</span></span>

<span data-ttu-id="e43db-451">Test IDE, tür sağlayıcının yeniden oluşturulmasını önleyen. dll dosyasında bir kilit alacak olduğundan, tür sağlayıcısını bir örnek içinde geliştirebilir ve sağlayıcıyı başka bir örnekte test edebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-451">You can develop the type provider in one instance and test the provider in the other because the test IDE will take a lock on the .dll file that prevents the type provider from being rebuilt.</span></span> <span data-ttu-id="e43db-452">Bu nedenle, sağlayıcı ilk örnekte derlenirken Visual Studio 'nun ikinci örneğini kapatmanız gerekir ve ardından sağlayıcı oluşturulduktan sonra ikinci örneği yeniden açmanız gerekir.</span><span class="sxs-lookup"><span data-stu-id="e43db-452">Thus, you must close the second instance of Visual Studio while the provider is built in the first instance, and then you must reopen the second instance after the provider is built.</span></span>

### <a name="debug-type-providers-by-using-invocations-of-fscexe"></a><span data-ttu-id="e43db-453">FSC. exe ' nin etkinleştirmeleri kullanılarak tür sağlayıcılarının hatalarını ayıklama</span><span class="sxs-lookup"><span data-stu-id="e43db-453">Debug type providers by using invocations of fsc.exe</span></span>

<span data-ttu-id="e43db-454">Aşağıdaki araçları kullanarak tür sağlayıcılarını çağırabilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="e43db-454">You can invoke type providers by using the following tools:</span></span>

- <span data-ttu-id="e43db-455">FSC. exe ( F# komut satırı derleyicisi)</span><span class="sxs-lookup"><span data-stu-id="e43db-455">fsc.exe (The F# command line compiler)</span></span>

- <span data-ttu-id="e43db-456">fsi. exe ( F# etkileşimli derleyici)</span><span class="sxs-lookup"><span data-stu-id="e43db-456">fsi.exe (The F# Interactive compiler)</span></span>

- <span data-ttu-id="e43db-457">Devenv. exe (Visual Studio)</span><span class="sxs-lookup"><span data-stu-id="e43db-457">devenv.exe (Visual Studio)</span></span>

<span data-ttu-id="e43db-458">Bir test betik dosyasında (örneğin, Script. FSX) FSC. exe ' yi kullanarak genellikle tür sağlayıcılarının hatalarını kolayca ayıklayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-458">You can often debug type providers most easily by using fsc.exe on a test script file (for example, script.fsx).</span></span> <span data-ttu-id="e43db-459">Bir komut isteminden bir hata ayıklayıcı başlatabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-459">You can launch a debugger from a command prompt.</span></span>

```
  devenv /debugexe fsc.exe script.fsx
```

  <span data-ttu-id="e43db-460">Print-stdout günlük kaydını kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="e43db-460">You can use print-to-stdout logging.</span></span>

## <a name="see-also"></a><span data-ttu-id="e43db-461">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="e43db-461">See also</span></span>

- [<span data-ttu-id="e43db-462">Tür Sağlayıcıları</span><span class="sxs-lookup"><span data-stu-id="e43db-462">Type Providers</span></span>](index.md)
- [<span data-ttu-id="e43db-463">Tür sağlayıcısı SDK 'Sı</span><span class="sxs-lookup"><span data-stu-id="e43db-463">The Type Provider SDK</span></span>](https://github.com/fsprojects/FSharp.TypeProviders.SDK)
