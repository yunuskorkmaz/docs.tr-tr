...
F#verilmiştir daha az yazarak ile aynı sonucu elde etmek için daha kısa bir söz dizimi.
...
Değer bir dilde birinci sınıf bir durum varsa, bir işleve bağımsız değişken olarak geçirebilirsiniz. Örneğin, tamsayılar ve dizelerin bağımsız değişken olarak geçirmek için yaygındır. Aşağıdaki kod tamsayılar ve dizelerin bağımsız değişken olarak geçirilen gösterir F#.
...
Aşağıdaki örnekte birinci sınıf işlevler durumunu F# bir işlevi bildirmek için `compose`, iki işlev bağımsız değişkenleri bir bileşimini döndürür.
...
> [!NOTE]
> F#iki işleçleri sağlar `<<` ve `>>`, İşlevler oluşturun. Örneğin, `let squareAndDouble2 = doubleIt << squareIt` eşdeğerdir `let squareAndDouble = compose doubleIt squareIt` önceki örnekte.
...
Önceki bölümde yer alan örnekler birçoğu daha kısaca örtük avantajlarından yararlanarak yazılabilir *currying* içinde F# işlev bildirimleri. Currying her biri tek bir parametreye sahip bir dizi katıştırılmış İşlevler, birden fazla parametresi olan bir işlev dönüştüren bir işlemdir. İçinde F#, birden fazla parametreye sahip işlevler kendiliğinden curried. Örneğin, `compose` üç parametrelerle aşağıdaki kısa stili gösterildiği gibi önceki bölümden yazılabilir.
...
İşlevler önceki bölümlerde örnekler göstermektedir F# ilk sınıf değerleri olan ölçütlerini karşılayan F#:
...
Hakkında daha fazla bilgi için F#, bkz: [ F# dil başvurusu](../language-reference/index.md).
