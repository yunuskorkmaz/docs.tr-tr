...
Ayrıca, veritabanları, önbellek ve ileti aracıları gibi altyapı varlıklarının, Orchestrator 'dan boşaltılabilmesi ve Azure SQL veritabanı, Azure Cosmos DB, Azure redin, Azure Service Bus veya şirket içinde herhangi bir HA kümeleme çözümü gibi altyapıda yüksek kullanılabilir sistemlere dağıtılması gerekir.
...
HTTP isteği, mikro hizmet veritabanına ve gerekli ek eylemlere erişen bu tür C# kodunu çalıştırmaya sona acaktır.
...
`docker-compose.override.yml`Katalog mikro hizmetinin dosyasına bir örnek aşağıda verilmiştir:
...
Daha sonra, Katalog mikro hizmetine doğrudan erişebilir ve bu durumda "dış" bağlantı noktasından doğrudan bu bağlantı noktası üzerinden erişim için Swagger Kullanıcı arabirimi aracılığıyla yöntemlerini görebilirsiniz `http://localhost:5101/swagger` :
...
Bu noktada, Visual Studio 'da C# kodunda bir kesme noktası ayarlayabilir, mikro hizmeti Swagger Kullanıcı arabiriminde kullanıma sunulan yöntemlerle test edebilirsiniz ve son olarak komutla her şeyi temizleyebilirsiniz `docker-compose down` .
...
Ocelot yalnızca ASP.NET Core çalışmak üzere tasarlanmıştır. Paketin en son sürümü hedefler `.NETCoreApp 3.1` ve bu nedenle .NET Framework uygulamalar için uygun değildir.
...
Örneğin, sepet mikro hizmeti yapılandırması, yukarıdaki configuration.jsüzerinde bulunan ReRoutes birine odaklanalım.
...
Ayrıca, aşağıdaki Docker-Compose. override. yıml dosyasında görebileceğiniz gibi, bu API ağ geçidi kapsayıcıları arasındaki tek fark, her bir hizmet kapsayıcısı için farklı olan ve bir Docker birimi aracılığıyla çalışma zamanında belirtilen Ocelot yapılandırma dosyasıdır.
...
Bu önceki kod nedeniyle ve aşağıdaki Visual Studio Gezgini 'nde gösterildiği gibi, her bir iş/BFF API ağ geçidini tanımlamak için gereken tek dosya yalnızca bir configuration.jsdosyası olur, çünkü dört API ağ geçidi aynı Docker görüntüsüne dayalıdır.
...
Örneğin, `http://localhost:5202/api/v1/c/catalog/items/2/` webshoppingapigw API Gateway tarafından sunulan yukarı AKıŞ URL 'sini ziyaret ederken, `http://catalog-api/api/v1/2` Docker ana bilgisayarı Içindeki Iç aşağı akış URL 'sinden aşağıdaki tarayıcıda olduğu gibi aynı sonucu elde edersiniz.
...
API ağ geçidi düzeyinde herhangi bir hizmette kimlik doğrulama ile güvenli hale getirmenin yolu, üzerinde configuration.jsilgili ayarlarındaki AuthenticationProviderKey ' i ayarlamadır.
...
"Sepet" gibi Validaudide, `AddJwtBearer()` aşağıdaki kodda olduğu gibi başlangıç sınıfının ConfigureServices () ile birlikte her bir mikro hizmette tanımlanan hedef kitlelerle bağıntılı.
...
EShopOnContainers kaynak kodu dosyalarında, özgün "configuration.json" dosyaları klasör içinde bulunabilir `k8s/ocelot/` . Her BFF/APIGateway için bir dosya vardır.
