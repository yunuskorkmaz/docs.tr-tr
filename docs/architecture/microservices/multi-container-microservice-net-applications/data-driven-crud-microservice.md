...
API projesi, tüm gerekli paketlere yönelik başvuruları içeren Microsoft. AspNetCore. app NuGet paketine yönelik başvurular içerir. Diğer bazı paketleri de içerebilir.
...
Ek uygulamalarına sahip olabilirsiniz `DbContext` . Örneğin, örnek katalog. API mikro hizmetinde, `DbContext` `CatalogContextSeed` veritabanına ilk kez erişmeye çalıştığında örnek verileri otomatik olarak dolduran ikinci bir adlandırılmış ad vardır. Bu yöntem, tanıtım verileri ve otomatikleştirilmiş test senaryoları için yararlıdır.

İçinde `DbContext` , `OnModelCreating` nesne/veritabanı varlık eşlemelerini ve diğer [EF genişletilebilirlik noktalarını](https://devblogs.microsoft.com/dotnet/implementing-seeding-custom-conventions-and-interceptors-in-ef-core-1-0/)özelleştirmek için yöntemini kullanırsınız.
...
Web API projesinde ayarlanmakta olan önemli bir yapılandırma, hizmetin IOC kapsayıcısına DbContext sınıfı kaydolmalıdır. Bu şekilde, `Startup` `services.AddDbContext<CatalogContext>()` `ConfigureServices()` aşağıdaki **Basitleştirilmiş** örnekte gösterildiği gibi yöntemi içindeki yöntemini çağırarak sınıfında.
...
Aşağıdaki örnekte gösterildiği gibi ASP.NET Core ayarlarını kullanarak settings.jsdosyasına bir ConnectionString özelliği ekleyebilirsiniz:
...
Dosyadaki settings.jsConnectionString özelliği veya diğer herhangi bir özellik için varsayılan değerlere sahip olabilir. Ancak, Docker kullanılırken, bu özellikler Docker-Compose. override. yıml dosyasında belirttiğiniz ortam değişkenlerinin değerleri tarafından geçersiz kılınır.

Docker-Compose. yml veya Docker-Compose. override. yml dosyalarından, bu ortam değişkenlerini, Docker 'ın sizin için işletim sistemi ortam değişkenleri olarak ayarlayacağından, aşağıdaki Docker-Compose. override. yml dosyasında (bağlantı dizesi ve diğer satırlar bu örneğe sarılmaz, ancak kendi dosyanızda sarılmamasından) başlatabilirsiniz.
...
Çözüm düzeyindeki Docker-Compose. yıml dosyaları, proje veya mikro hizmet düzeyindeki yapılandırma dosyalarından yalnızca daha esnek değildir, ancak Docker-Compose dosyaları Azure DevOps Services Docker dağıtım görevleri gibi dağıtım araçlarınızla ayarlanan değerlerle ayarlanmış ortam değişkenlerini geçersiz kılarsınız, daha da güvenli hale geldi.
...
Şu anda, swashbuckle, ASP.NET Core uygulamalar için üst düzey metapackage [swashbuckle. AspNetCore](https://www.nuget.org/packages/Swashbuckle.AspNetCore) altında beş iç NuGet paketinden oluşur.
...
Daha önce, gibi bir URL için swashbuckle tarafından oluşturulan oluşturulan kullanıcı arabirimini gördünüz `http://<your-root-url>/swagger` . Şekil 6-9 ' de, herhangi bir API yöntemini nasıl test kullanabileceğinizi da görebilirsiniz.
...
Swagger Kullanıcı arabirimi API 'SI ayrıntısı yanıtın bir örneğini gösterir ve geliştirici keşfi için harika olan gerçek API 'yi yürütmek için kullanılabilir. Şekil 6-10, `http://<your-root-url>/swagger/v1/swagger.json` [Postman](https://www.getpostman.com/)kullanarak Istek yaptığınızda eShopOnContainers mikro HIZMETINDEN oluşturulan Swagger JSON meta verilerini gösterir (araçların altında kullanıldığı Özellikler).
