...
Ocelot, bir NuGet paketi olarak kullanılabilir. NET Standard 2,0 ' i hedefler ve hem .NET Core 2.0 + hem de .NET Framework 4.6.1 + çalışma zamanları ile uyumlu hale getirir. Ocelot, HTTP 'yi konuştuk ve .NET Core 'un desteklediği platformlarda çalışan her şeyi tümleştirir: Linux, macOS ve Windows. Ocelot genişletilebilir ve Docker kapsayıcıları, Azure Kubernetes Hizmetleri veya diğer genel bulutlar dahil olmak üzere birçok modern platformu destekler.  Ocelot, [Tüketil](https://www.consul.io), [Graphql](https://graphql.org)ve Netflix 'in [Eureka](https://github.com/Netflix/eureka)gibi açık kaynaklı paketlerle tümleştirilir. 
