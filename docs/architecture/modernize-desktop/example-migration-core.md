...
.NET Core için [SDK stili proje](../../core/project-sdk/msbuild-props.md) , .NET Framework proje biçiminden daha basittir. Daha önce bahsedilen girdilerden ayrı `PackageReference` olarak, çok daha fazlasını yapmanız gerekmez. Yeni proje biçimi, [belirli uzantılara sahip dosyaları](../../core/project-sdk/overview.md#default-includes-and-excludes), örneğin `.cs` ve `.xaml` dosyaları proje dosyasına açıkça dahil etmek zorunda kalmadan, varsayılan olarak içerir.
