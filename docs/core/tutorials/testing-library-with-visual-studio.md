...
---
# <a name="tutorial-test-a-net-standard-library-with-net-core-in-visual-studio"></a>Öğretici: Visual Studio 'da .NET Core ile .NET Standard kitaplığı test etme
...
Kitaplık yönteminiz dizeleri yaptığından, ayrıca [boş bir dizeyi ( `String.Empty` )](xref:System.String.Empty)başarılı bir şekilde (), karakteri olmayan ve 0 olan geçerli bir dize <xref:System.String.Length> ve başlatılmamış bir dize de işlemesini sağlamak istersiniz `null` . `StartsWithUpper`Bir örnek üzerinde uzantı yöntemi olarak çağrılırsa <xref:System.String> , bir `null` dize geçirilebilir. Ancak, bunu doğrudan statik bir yöntem olarak çağırabilir ve tek bir <xref:System.String> bağımsız değişken geçirebilirsiniz.
