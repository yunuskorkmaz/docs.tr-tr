...
Proje dosyanızı açtığınızda, iki veya daha fazla `<ItemGroup>` düğümü görürsünüz. Düğümlerden birinin zaten `<PackageReference>` öğeleri olduğunu fark edeceksiniz. Yeni bağımlılığı bu düğüme ekleyebilir veya yeni bir tane oluşturabilirsiniz; Sonuç aynı olacağı için size ait.
