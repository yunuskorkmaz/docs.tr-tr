...
Kodunuzun .NET Core veya .NET Standard bağlantı noktası olması için bağımlılıklarınızı anlamanız gerekir. Dış bağımlılıklar, `.dll` projenizde başvurduğunuz, ancak sizin oluşturmayan NuGet paketlerdir veya dosyalardır.
...
Projenizi biçimine geçirdikten sonra `PackageReference` , paketlerinizin .NET Core ile uyumlu olduğunu doğrulayın.
...
Bu değerler, .NET Core ile uyumlu [.NET Standard](../../standard/net-standard.md), .NET Core ve geleneksel taşınabilir sınıf KITAPLıĞı (PCL) profillerinin sürümleriyle eşleyen [hedef çerçeve takma adları (tfms)](../../standard/frameworks.md) .

> [!IMPORTANT]
> Bir paketin desteklediği TFMs 'Leri ararken, `netcoreapp*` uyumlu iken yalnızca .NET Core projeleri için olduğunu ve .NET Standard projelerine yönelik olduğunu unutmayın.
> Yalnızca hedefleri olan `netcoreapp*` ve olmayan bir kitaplık `netstandard*` yalnızca diğer .NET Core uygulamaları tarafından tüketilebilir.
