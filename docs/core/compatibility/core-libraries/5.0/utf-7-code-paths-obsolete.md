...
- Şunları desteketmeniz gerekirse `Encoding.GetEncoding("utf-7", ...)` :
  ...
  Bir uyumluluk anahtarı aracılığıyla bunun desteğini yeniden etkinleştirebilirsiniz. Bu uyumluluk anahtarı, aşağıdaki örneklerde gösterildiği gibi uygulamanın *. csproj* dosyasında veya bir [çalışma zamanı yapılandırma dosyasında](../../../run-time-config/index.md)belirtilebilir.
