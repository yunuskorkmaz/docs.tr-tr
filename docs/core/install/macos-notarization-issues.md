...
.NET Core (çalışma zamanı ve SDK) yükleyicileri 3,1, 3,0 ve 2,1 sürümleri, 18 Şubat 2020 tarihinden itibaren giderilmiş. Önceki yayınlanmış sürümler yok. Önce yükleyiciyi indirerek ve sonra `sudo installer` komutunu kullanarak .NET Core 'un bilinmeyen bir sürümünü el ile yükleyebilirsiniz. Daha fazla bilgi için bkz. [macOS Için indirme ve el ile yükleme](sdk.md?pivots=os-macos#download-and-manually-install).
...
Varsayılan olarak, .NET Core SDK 3,0 ve üzeri sürümleri, projeniz derlendiği, yayımladığında veya çalıştırıldığında yerel bir makine-O yürütülebilir dosyası ( **appHost**olarak bilinir) oluşturur. Bu yürütülebilir dosya, uygulamanızı çalıştırmak için kullanışlı bir yoldur. Aksi takdirde, `dotnet <filename.dll>`çalıştırılarak uygulamanız başlatılmalıdır. AppHost etkinleştirildiğinde, `dotnet run` komutu appHost bağlamında çağrılır. Daha fazla bilgi için bkz. [apphost bağlamı](#context-of-the-apphost).

.NET Core SDK 3,0 ve üzeri sürümleri ile başlayarak, appHost yürütülebilir dosyası varsayılan olarak oluşturulmaz. AppHost oluşturmayı, proje dosyasında [`UseAppHost`](../project-sdk/msbuild-props.md#useapphost) Boolean ayarıyla açabilirsiniz. Ayrıca, çalıştırdığınız belirli `dotnet` komutu için komut satırındaki `-p:UseAppHost` parametresiyle appHost öğesini değiştirebilirsiniz:
...
Sertifika paylaşımı gerekliyse, macOS `security` yardımcı programıyla yapılandırma seçenekleri sağlar.
