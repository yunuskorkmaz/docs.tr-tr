...
3. `Const`Bu sabitleri uygulamanız için kullanılabilir hale getirmek için sınıfınıza veya modülünüzü eşdeğer deyimler ekleyin. Örneğin:  
...
8. `DllImport`Özniteliği boş işleve uygulayın. İlk parametre, aradığınız işlevi içeren DLL 'nin adı ve konumudur. Windows sistem dizinlerinde bulunan dosyaların yolunu belirtmeniz gerekmez. İkinci parametre, Windows API 'sindeki işlevin adını belirten adlandırılmış bir bağımsız değişkendir. Bu örnekte, `DllImport` özniteliği `MoveFile` `MoveFileW` KERNEL32.DLL ' de ' e iletime çağrısı yapılmasını zorlar. `MoveFileW`Yöntemi yolu yolundan bir dosya kopyalar `src` `dst` .  
