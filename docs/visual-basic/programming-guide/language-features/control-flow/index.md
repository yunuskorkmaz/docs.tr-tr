...
Sol düzenlenmemiş bir program baştan gelen kendi deyimleri ile devam eder. Bazı çok basit programları ile için tek yönlü Bu akış yazılabilir. Ancak, güç çoğunu ve herhangi bir programlama dili yardımcı programı denetim ifadeleri ve döngüler yürütme sırasını değiştirme olanağı gelir.  
 ...
 Denetim yapıları programınızın yürütme akışını olanak sağlar. Denetim yapıları'ni kullanarak, yazabilirsiniz [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] kodunun kararlarını verir veya eylemleri yineler. Diğer denetim yapıları bir kaynağın elden garanti veya bir dizi deyimi aynı nesne başvurusu üzerinde çalıştırmak sağlar.  
