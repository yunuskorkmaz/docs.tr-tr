...
<xref:System.Enum.GetValues%2A> Ve <xref:System.Enum.GetNames%2A> yöntemler dizeler ve numaralandırma üyeleri ile ilişkili değerler belirlemenize olanak sağlar.  
  
### <a name="to-determine-the-string-associated-with-an-enumeration"></a>Bir numaralandırma ile ilişkili dizeyi belirleme  
  
-   Kullanım <xref:System.Enum.GetNames%2A> numaralandırma üyelerle ilişkilendirilen dizeleri alma yöntemi. Bu örnek, bir numaralandırma bildirir `flavorEnum`, ardından kullanan <xref:System.Enum.GetNames%2A> her üyesiyle ilişkili dizelerini yöntemi.  
