...
Bir özellik değeri depolamak veya değeri alınırken bir özellik yordamı çağırma. Bir özelliği bir değişkene erişme aynı şekilde erişin.  
 ...
 Özelliğin `Set` yordamı bir değer depolar ve kendi `Get` yordamı değerini alır. Ancak, siz açıkça bu yordamları adıyla çağırmayın. Yalnızca siz depolamak veya bir değişkenin değerini almak bir atama ifadesi veya bir ifade özelliğini kullanın. [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]özelliğin yordamlara çağrılar.  
