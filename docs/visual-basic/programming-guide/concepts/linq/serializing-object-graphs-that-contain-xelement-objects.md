---
title: XElement nesneler (Visual Basic) içeren nesne grafiklerinin seri hale getirme
ms.date: 07/20/2015
ms.assetid: c0cc5c92-5ca3-44b1-98dd-371601df721b
ms.openlocfilehash: 35e05f4201920401dbbf68fc810c77b92c2850f5
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33645765"
---
# <a name="serializing-object-graphs-that-contain-xelement-objects-visual-basic"></a>XElement nesneler (Visual Basic) içeren nesne grafiklerinin seri hale getirme
Bu konu, başvuru türünde nesneler içeren nesne grafiklerinin seri hale getirme yeteneği tanıtır <xref:System.Xml.Linq.XElement>. Tesis için seri hale getirme, bu tür <xref:System.Xml.Linq.XElement> uygulayan <xref:System.Xml.Serialization.IXmlSerializable> arabirimi.  
  
 Yalnızca Not <xref:System.Xml.Linq.XElement> sınıfı serileştirme uygular.  
  
## <a name="in-this-section"></a>Bu Bölümde  
  
|Konu|Açıklama|  
|-----------|-----------------|  
|[Nasıl yapılır: XmlSerializer (Visual Basic) kullanarak serileştirme](../../../../visual-basic/programming-guide/concepts/linq/how-to-serialize-using-xmlserializer.md)|Kullanarak serileştirmek gösterilmiştir <xref:System.Xml.Serialization.XmlSerializer>.|  
|[Nasıl yapılır: DataContractSerializer (Visual Basic) kullanarak serileştirme](../../../../visual-basic/programming-guide/concepts/linq/how-to-serialize-using-datacontractserializer.md)|Kullanarak serileştirmek gösterilmiştir <xref:System.Runtime.Serialization.DataContractSerializer>.|  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Gelişmiş LINQ-XML programlama (Visual Basic)](../../../../visual-basic/programming-guide/concepts/linq/advanced-linq-to-xml-programming.md)
