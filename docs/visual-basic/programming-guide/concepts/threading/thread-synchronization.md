...
## <a name="the-lock-and-synclock-keywords"></a>SyncLock anahtar sözcükleri ve kilitleme  
 Visual Basic `SyncLock` deyimi bir kod bloğu kesintisiz tamamlanması için çalıştığından emin olmak için kullanılabilir başka iş parçacıklarının engellemelerinden. Bu işlem, kod bloğunun süresi için karşılıklı dışlama kilidini belirli nesne elde ederek gerçekleştirilir.  
  
 A `SyncLock` deyimi bir nesne bir bağımsız değişken olarak verilir ve bir kerede yalnızca bir iş parçacığı tarafından yürütülecek bir kod bloğu tarafından izlenir. Örneğin:  
...
```  
 ...
 Kullanarak `SyncLock` anahtar sözcüğü, genellikle tercih edilen yerine <xref:System.Threading.Monitor> hem doğrudan, çünkü sınıf `SyncLock` daha kısa olduğundan ve çünkü `SyncLock` bile korunan kod oluşturur, temel alınan İzleyici yayımlanan oluşturmasını sağlar bir özel durum. Bu ile gerçekleştirilir `Finally` anahtar sözcüğü bir özel durum olup bağımsız olarak, ilişkili kod bloğunu yürütür.  
