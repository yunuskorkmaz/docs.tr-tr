...
Genel bir tür olarak işaretlenmiş `<CLSCompliant(True)>`, ancak bir kısıtlama türü parametrelerinden biri olarak işaretlenmiş bir tür belirttiğinden `<CLSCompliant(False)>`işaretlenmemiş veya uyumlu olmayan bir tür olduğundan geçerli değil.  
 ...
 Türü ile uyumlu olması [dil bağımsızlığı ve dilden bağımsız bileşenler](../../../docs/standard/language-independence-and-language-independent-components.md) (CLS), onu yalnızca CLS uyumlu türleri kullanmalıdır. Bu da genel bir tür, tür parametrelerindeki kısıtlamalar geçerlidir.  
