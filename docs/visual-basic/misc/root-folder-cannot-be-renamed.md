---
title: Kök klasör adı değiştirilemez
ms.date: 07/20/2015
ms.assetid: 325eb73c-d5fe-4b65-8926-5dabff007277
ms.openlocfilehash: 19cc5e59672736f36d83e0b8a7d9040665ed1065
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="root-folder-cannot-be-renamed"></a>Kök klasör adı değiştirilemez
Bir kök dizini yeniden adlandırmak için girişimde bulunuldu. Bu yanlış ayrıştırılmış dosya yolu sonucu olabilir.  
  
## <a name="to-correct-this-error"></a>Bu hatayı düzeltmek için  
  
-   Yolun doğru olduğundan emin olmak için kontrol edin.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [My.Computer.FileSystem.RenameDirectory]xref:Microsoft.VisualBasic.MyServices.FileSystemProxy.RenameDirectory)  
 [My.Computer.FileSystem.RenameFile]xref:Microsoft.VisualBasic.MyServices.FileSystemProxy.RenameFile)  
 [Dosya ve Dizin Oluşturma, Silme ve Taşıma](../../visual-basic/developing-apps/programming/drives-directories-files/creating-deleting-and-moving-files-and-directories.md)  
 [Nasıl yapılır: Visual Basic'te dosyayı yeniden adlandırma](../developing-apps/programming/drives-directories-files/how-to-rename-a-file.md)  

