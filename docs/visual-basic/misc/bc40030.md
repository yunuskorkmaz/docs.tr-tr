...
Bir sınıf veya arabirim olarak işaretlenmiş `<CLSCompliant(True)>` zaman onu iç içe geçmiş olarak işaretlenmiş bir türdeki `<CLSCompliant(False)>` veya değil olarak işaretlenmiş.  
 ...
 Bir sınıf veya uyumlu olması için arabirimi için [dil bağımsızlığı ve dilden bağımsız bileşenler](https://msdn.microsoft.com/library/12a7a7h3) (CLS), tüm kapsayıcı hiyerarşisi olmalıdır uyumlu. Bu yer alan her tür uyumlu olması gerektiği anlamına gelir.  
