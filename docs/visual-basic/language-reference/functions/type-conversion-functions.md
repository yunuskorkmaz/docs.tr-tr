...
|İşlev adı|Dönüş veri türü|`expression` bağımsız değişkeni aralığı|
|-------------------|----------------------|-------------------------------------|
...
|`CStr`|[String Veri Türü](../../../visual-basic/language-reference/data-types/string-data-type.md)|`CStr` için döndürür `expression` bağımsız değişkenine bağlıdır. Bkz. [CStr işlevi Için dönüş değerleri](../../../visual-basic/language-reference/functions/return-values-for-the-cstr-function.md).|
...
Visual Basic 15,8 ' den itibaren, aşağıdaki yöntemlerle döndürülen <xref:System.Single> veya <xref:System.Double> değeri tamsayı dönüştürme işlevlerinden birine (`CByte`, `CShort`, `CInt`, `CLng`, `CSByte`, `CUShort`, `CUInt`, `CULng`) geçirdiğinizde, kayan noktalı tamsayı dönüştürme performansı en iyi duruma getirilir:
...
Aşağıdaki örnek, sayısal bir değeri `Object`dönüştürmek için `CObj` işlevini kullanır. `Object` değişkeni, kendisine atanan `Double` değerine işaret eden yalnızca dört baytlık bir işaretçi içerir.
...
`CStr`tarafından döndürülen değerler hakkında daha fazla ayrıntı için bkz. [CStr işlevi Için dönüş değerleri](../../../visual-basic/language-reference/functions/return-values-for-the-cstr-function.md).
