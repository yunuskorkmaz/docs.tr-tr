...
`Widening`  
`Narrowing`belirtmediğiniz takdirde bir dönüştürme işleci için gereklidir. Bu işleç yordamının bir [genişletme](../../../visual-basic/language-reference/modifiers/widening.md) dönüşümünü tanımladığını gösterir. Bu Yardım sayfasında "genişletme ve daraltma dönüşümleri" başlığına bakın.

`Narrowing`  
`Widening`belirtmediğiniz takdirde bir dönüştürme işleci için gereklidir. Bu işleç yordamının bir [daraltma](../../../visual-basic/language-reference/modifiers/narrowing.md) dönüşümünü tanımladığını gösterir. Bu Yardım sayfasında "genişletme ve daraltma dönüşümleri" başlığına bakın.
...
|`operandtype`|`Option Strict` `On`olmadığı müddetçe isteğe bağlıdır. Bu işlenenin veri türü.|

`type`  
`Option Strict` `On`olmadığı müddetçe isteğe bağlıdır. İşleç yordamının döndürdüğü değerin veri türü.
...
Dönüş değerini tutmak için işleç sembolünü veya tanımlayıcıyı kullanamazsınız. `Return` ifadesini kullanmanız ve bir değer belirtmesi gerekir. Herhangi bir sayıda `Return` deyimi yordamda herhangi bir yerde görünebilir.
...
`CType`tanımladığınızda, `Widening` veya `Narrowing`belirtmeniz gerekir.
...
- `IsTrue` ve `IsFalse` işleçlerini tanımlarsanız, her ikisi de `Boolean` türünü döndürmelidir.

- `<<` ve `>>` işleçlerini tanımlarsanız, her ikisi de `operand2``operandtype` için `Integer` türünü belirtmelidir.
...
`AndAlso` işlecini doğrudan bir `Operator` ifadesiyle tanımlayamazsınız. Ancak, aşağıdaki koşulları karşıladıysanız `AndAlso` kullanabilirsiniz:

- `AndAlso`için kullanmak istediğiniz aynı işlenen türlerinde `And` tanımladınız.

- `And` tanımınız, tanımladığınız sınıf veya yapı ile aynı türü döndürür.

- `And`tanımladığınız sınıf veya yapıda `IsFalse` işlecini tanımladınız.
...
`Widening`bir dönüştürme yordamı bildirirseniz, yordam kodunuzun herhangi bir başarısızlık üretmemelidir. Bu, aşağıdakiler anlamına gelir:
