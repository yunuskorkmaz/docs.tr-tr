---
title: "Tümceler (Visual Basic)"
ms.date: 07/20/2015
ms.prod: .net
ms.reviewer: 
ms.suite: 
ms.technology: devlang-visual-basic
ms.topic: article
helpviewer_keywords: clauses [Visual Basic]
ms.assetid: 5e89f5f3-d129-4ed1-8857-30cd71cff95e
caps.latest.revision: "8"
author: dotnet-bot
ms.author: dotnetcontent
ms.openlocfilehash: 1a4166cc480541f913dd47952782a6015c5348a2
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 10/18/2017
---
# <a name="clauses-visual-basic"></a>Tümceler (Visual Basic)
Bu bölümdeki konular, Visual Basic çalışma zamanı yan tümceleri belge.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Diğer ad](../../../visual-basic/language-reference/statements/alias-clause.md)  
  
 [Olarak](../../../visual-basic/language-reference/statements/as-clause.md)  
  
 [İşleme](../../../visual-basic/language-reference/statements/handles-clause.md)  
  
 [Uygular](../../../visual-basic/language-reference/statements/implements-clause.md)  
  
 [İçinde](../../../visual-basic/language-reference/statements/in-clause.md)  
  
 [İçine](../../../visual-basic/language-reference/statements/into-clause.md)  
  
 [,](../../../visual-basic/language-reference/statements/of-clause.md)  
  
## <a name="related-sections"></a>İlgili Bölümler  
 [Visual Basic Dil Başvurusu](../../../visual-basic/language-reference/index.md)  
  
 [Visual Basic](../../../visual-basic/index.md)
