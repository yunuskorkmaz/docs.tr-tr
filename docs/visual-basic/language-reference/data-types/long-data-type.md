...
@No__t_1 veri türüne sığamayacak kadar büyük tamsayı sayılar içermesi için `Long` veri türünü kullanın.
...
- **Kan.** @No__t_0 veri türü `Decimal`, `Single` veya `Double` için. Bu, bir <xref:System.OverflowException?displayProperty=nameWithType> hatasıyla karşılaşmadan `Long` bu türlerden birine dönüştürebileceğiniz anlamına gelir.
