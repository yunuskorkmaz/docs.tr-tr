---
title: Visual Basic ile Uygulama Geliştirme
ms.date: 07/20/2015
helpviewer_keywords:
- Visual Basic, developing applications
- applications [Visual Basic]
ms.assetid: 1e1c0c81-6d95-4167-a98b-44b1efb6d25f
ms.openlocfilehash: b4434d6f66ea8329903c92fe664d9704bce19625
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
---
# <a name="developing-applications-with-visual-basic"></a>Visual Basic ile Uygulama Geliştirme
Bu bölümde, Visual Basic dil için kavramsal belge ele alır.  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Visual Basic'te Programlama](../../visual-basic/developing-apps/programming/index.md)  
 Çeşitli programlama konuları kapsar.  
  
 [My Özelliğiyle Geliştirme](../../visual-basic/developing-apps/development-with-my/index.md)  
 Adlı yeni bir özellik anlatılmaktadır `My`, bir uygulama ve onun çalışma zamanı ortamı ile ilgili nesne örneklerini bilgilerini ve varsayılan erişim sağlar.  
  
 [Visual Basic uygulamalarındaki verilere erişme](../../visual-basic/developing-apps/accessing-data.md)  
 Visual Basic veri erişimi hakkında Yardım içerir.  
  
 [Oluşturma ve Visual Basic'te bileşenleri kullanma](../../visual-basic/developing-apps/creating-and-using-components.md)  
 Terim tanımlar *bileşen* ve ne zaman ve nasıl ele almaktadır bileşenleri oluşturmak için.  
  
 [Yazdırma ve Raporlama](../../visual-basic/developing-apps/printing/printing-and-reporting.md)  
 Genel bakışlar ve yazdırma ve Raporlama ile ilgili ilgili belgelere bağlantılar sağlar.  
  
 [Windows Forms Uygulaması Temelleri](../../visual-basic/developing-apps/windows-forms/windows-forms-application-basics.md)  
 Visual Studio kullanarak Windows Forms uygulamaları oluşturma hakkında bilgi sağlar.  
  
 [Projeleri özelleştirme ve genişletme My](../../visual-basic/developing-apps/customizing-extending-my/customizing-projects-and-extending-my.md)  
 Ek sağlamak üzere proje şablonları nasıl özelleştirebileceğiniz açıklanmaktadır `My` nesneleri.  
  
## <a name="related-sections"></a>İlgili Bölümler  
 [Visual Basic programlama kılavuzu](../../visual-basic/programming-guide/index.md)  
 Visual Basic ile programlama temel öğelerini anlatılmaktadır.  
  
 [Visual Basic Dil Başvurusu](../../visual-basic/language-reference/index.md)  
 Visual Basic dil için başvuru belgelerini içerir.
