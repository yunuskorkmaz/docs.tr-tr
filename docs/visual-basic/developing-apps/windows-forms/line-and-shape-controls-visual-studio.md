---
title: Çizgi ve Şekil Denetimleri (Visual Studio)
ms.date: 07/20/2015
helpviewer_keywords:
- RectangleShape control [Visual Basic], topic list
- OvalShape control [Visual Basic], topic list
- Shape control [Visual Basic]
- Line control [Visual Basic], topic list
ms.assetid: 29ee3a5f-920f-4677-85bc-bd7425ab57e8
ms.openlocfilehash: 6d052af15257f00f43ad21b08765ab3c882b31bf
ms.sourcegitcommit: a885cc8c3e444ca6471348893d5373c6e9e49a47
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 09/06/2018
ms.locfileid: "43871773"
---
# <a name="line-and-shape-controls-visual-studio"></a>Çizgi ve Şekil Denetimleri (Visual Studio)
Visual Basic Power Packs çizgi ve Şekil denetimleri yatay, dikey ve çapraz satırları, dikdörtgenler, kare, elips, daire ve dikdörtgenler ve kareler yuvarlatılmış köşeler bir form veya kapsayıcı ile çizme olanak tanıyan Grafik denetimlerdir.  
  
 PowerPack denetimleri artık Visual Studio'ya dahil, ancak bunları indirebilirsiniz [İndirme Merkezi](https://www.microsoft.com/en-us/download/details.aspx?id=25169).  
  
## <a name="in-this-section"></a>Bu Bölümde  
 [Çizgi ve Şekil Denetimlerine Giriş](../../../visual-basic/developing-apps/windows-forms/introduction-to-the-line-and-shape-controls-visual-studio.md)  
 Tanıtır ve çizgi ve Şekil denetimleri açıklar ve nesne modelini açıklar.  
  
 [Nasıl Yapılır: LineShape Denetimiyle Çizgi Çizme](../../../visual-basic/developing-apps/windows-forms/how-to-draw-lines-with-the-lineshape-control-visual-studio.md)  
 Nasıl kullanılacağını gösteren <xref:Microsoft.VisualBasic.PowerPacks.LineShape> denetimi tasarım zamanında ve çalışma zamanında çizgi çizmek için.  
  
 [Nasıl Yapılır: OvalShape ve RectangleShape Denetimleriyle Şekil Çizme](../../../visual-basic/developing-apps/windows-forms/how-to-draw-shapes-with-the-ovalshape-and-rectangleshape-controls.md)  
 Nasıl kullanılacağını gösteren <xref:Microsoft.VisualBasic.PowerPacks.OvalShape> ve <xref:Microsoft.VisualBasic.PowerPacks.RectangleShape> denetimleri tasarım zamanında ve çalışma zamanında şekiller çizmek için.  
  
 [Nasıl Yapılır: Şekiller Arasında Sekmeyle Gitmeyi Etkinleştirme](../../../visual-basic/developing-apps/windows-forms/how-to-enable-tabbing-between-shapes-visual-studio.md)  
 Klavyeyi kullanarak şekiller arasında taşımak kullanıcıları etkinleştirmek nasıl gösterir.  
  
## <a name="reference"></a>Başvuru  
 <xref:Microsoft.VisualBasic.PowerPacks>
