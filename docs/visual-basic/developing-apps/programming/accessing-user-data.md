---
title: Kullanıcı Verilerine Erişme (Visual Basic)
ms.date: 07/20/2015
helpviewer_keywords:
- domain names [Visual Basic], retrieving
- data [Visual Basic], accessing user data
- My.User object [Visual Basic], tasks
- user data [Visual Basic], domain
- user names [Visual Basic], retrieving
- user data [Visual Basic], accessing
- login names [Visual Basic]
- examples [Visual Basic], accessing user data
ms.assetid: 32492a15-ee59-4a63-a1f1-9b24cc13140a
ms.openlocfilehash: 097006caf56072d5a6e9f2945f5969eed249849e
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33582850"
---
# <a name="accessing-user-data-visual-basic"></a>Kullanıcı Verilerine Erişme (Visual Basic)
Bu bölüm ile konuları içerir `My.User` nesne ve ile gerçekleştirebileceğiniz görevler.  
  
 `My.User` Nesnesi sağlar oturum açmış kullanıcı hakkındaki bilgilere erişimi uygulayan bir nesne döndürerek <xref:System.Security.Principal.IPrincipal> arabirimi.  
  
## <a name="tasks"></a>Görevler  
  
|Bitiş|Bkz. |  
|--------|---------|  
|Kullanıcının oturum açma adını Al|<xref:Microsoft.VisualBasic.ApplicationServices.User.Name%2A>|  
|Uygulama Windows kimlik doğrulaması kullanıyorsa, kullanıcının etki alanı adını Al|<xref:Microsoft.VisualBasic.ApplicationServices.User.CurrentPrincipal>|  
|Kullanıcının rolünü belirleme|<xref:Microsoft.VisualBasic.ApplicationServices.User.IsInRole%2A>|  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 <xref:Microsoft.VisualBasic.ApplicationServices.User>
