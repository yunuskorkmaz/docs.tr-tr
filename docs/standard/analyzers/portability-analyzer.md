...
Projenizi, .NET Core gibi yeni platformu hedefleyecek şekilde dönüştürdükten sonra, <xref:System.PlatformNotSupportedException> özel durumları ve diğer uyumluluk sorunlarını oluşturan API 'Leri belirlemek için Roslyn tabanlı [API Çözümleyicisi aracını](api-analyzer.md) kullanabilirsiniz.
...
- Daha fazla yardım almak için `ApiPort.exe -?` çalıştırın
