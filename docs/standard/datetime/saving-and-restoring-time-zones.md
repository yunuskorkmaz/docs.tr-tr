...
- Nesnenin <xref:System.TimeZoneInfo.ToSerializedString%2A> yöntemini çağırarak bir <xref:System.TimeZoneInfo> nesnesini seri hale getirebilirsiniz. Yöntem hiçbir parametre alır ve saat dilimi bilgilerini içeren bir dize döndürür.

- Seri hale getirilmiş bir dizeden, bu dizeyi `static` (Visual Basic`Shared`) <xref:System.TimeZoneInfo.FromSerializedString%2A?displayProperty=nameWithType> yöntemine geçirerek bir <xref:System.TimeZoneInfo> nesnesinin serisini kaldırabilirsiniz.

## <a name="serialization-and-deserialization-scenarios"></a>Serileştirme ve seri kaldırma senaryoları

Bir <xref:System.TimeZoneInfo> nesnesini bir dizeye kaydetme (veya serileştirme) ve daha sonra kullanmak için geri yükleme (veya serisini kaldırma) özelliği, hem yardımcı programını hem de <xref:System.TimeZoneInfo> sınıfının esnekliğini artırır. Bu bölümde, serileştirme ve seri durumundan çıkarma konusunda en yararlı olan bazı durumlar incelenir.
...
Aşağıdaki örnekte, düzeltme kuralları olmayan bir özel <xref:System.TimeZoneInfo> sınıfı, Birleşik Devletler yaz saatinin giriş zamanından önce 1883 ile 1917 arasında ABD Doğu Standart saat dilimini temsil edecek şekilde tanımlanmıştır. Özel saat dilimi, genel kapsama sahip bir değişkende serileştirilir. `ConvertUtcTime`saat dilimi dönüştürme yöntemi, dönüştürmek için Eşgüdümlü Evrensel Saat (UTC) zamanlarını geçti. Tarih ve saat 1917 veya daha önceki bir sürümde gerçekleşirse, özel Doğu Standart saat dilimi serileştirilmiş bir dizeden geri yüklenir ve kayıt defterinden alınan saat dilimini değiştirir.
...
Kayıt defteri dinamik bir yapı olduğundan, içeriği yanlışlıkla veya bilinçli değişikliğine tabidir. Bu, kayıt defterinde tanımlanması gereken ve bir uygulamanın başarıyla yürütülmesi için gerekli olan bir zaman diliminin eksik olması anlamına gelir. Saat dilimi serileştirme ve seri durumundan çıkarma desteği olmadan, uygulamayı sonlandırarak sonuç <xref:System.TimeZoneNotFoundException> idare etmeniz gerekir. Ancak, saat dilimi serileştirme ve serisini kaldırma kullanarak, istenen saat dilimini serileştirilmiş bir dizeden geri yükleyerek beklenmedik bir <xref:System.TimeZoneNotFoundException> işleyebilirsiniz ve uygulama çalışmaya devam eder.
...
Seri hale getirilmiş saat dilimi verilerini depolamak için gömülü bir kaynak kullanan bir örnek için bkz. [nasıl yapılır: bir katıştırılmış kaynağa saat dilimlerini kaydetme](../../../docs/standard/datetime/save-time-zones-to-an-embedded-resource.md) ve [nasıl yapılır: zaman dilimlerini gömülü bir kaynaktan geri yükleme](../../../docs/standard/datetime/restore-time-zones-from-an-embedded-resource.md).
