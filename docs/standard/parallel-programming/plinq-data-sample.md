...
---
# <a name="plinq-data-sample"></a><span data-ttu-id="c9313-102">PLINQ Veri Örneği</span><span class="sxs-lookup"><span data-stu-id="c9313-102">PLINQ Data Sample</span></span>
<span data-ttu-id="c9313-103">Bu örnek, .csv formatında örnek veriler ve müşteri, ürün, sipariş ve sipariş ayrıntılarının bellek içi koleksiyonlarına dönüştüren yöntemler içerir.</span><span class="sxs-lookup"><span data-stu-id="c9313-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="c9313-104">PLINQ ile daha fazla deneme yapmak için, bu konudaki koda diğer bazı `Main` konulardan kod örneklerini yapıştırabilir ve yöntemden çağırabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c9313-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="c9313-105">Bu verileri kendi PLINQ sorgularınızla da kullanabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="c9313-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="c9313-106">Veriler Northwind veritabanının bir alt kümesini temsil eder.</span><span class="sxs-lookup"><span data-stu-id="c9313-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="c9313-107">Elli (50) müşteri kaydı dahildir, ancak tüm alanlar dahil değildir.</span><span class="sxs-lookup"><span data-stu-id="c9313-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="c9313-108">Siparişler'den gelen satırların bir alt kümesi ve her Müşteri için karşılık gelen Order_Detail verileri dahildir.</span><span class="sxs-lookup"><span data-stu-id="c9313-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="c9313-109">Tüm Ürünler dahildir.</span><span class="sxs-lookup"><span data-stu-id="c9313-109">All Products are included.</span></span>  
  
> [!NOTE]
> <span data-ttu-id="c9313-110">Veri kümesi, PLINQ'nin yalnızca temel `where` ve `select` yan tümceleri içeren sorgular için LINQ'dan Nesnelere linq'ten daha hızlı olduğunu gösterecek kadar büyük değildir.</span><span class="sxs-lookup"><span data-stu-id="c9313-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="c9313-111">Bunun gibi küçük veri kümelerinin hız artışlarını gözlemlemek için, veri kümesindeki her öğede hesaplama açısından pahalı işlemler içeren sorgular kullanın.</span><span class="sxs-lookup"><span data-stu-id="c9313-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="c9313-112">Bu örneği ayarlamak için</span><span class="sxs-lookup"><span data-stu-id="c9313-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="c9313-113">Visual Basic veya Visual C# konsol uygulama projesi oluşturun.</span><span class="sxs-lookup"><span data-stu-id="c9313-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="c9313-114">Bu adımları izleyen kodu kullanarak Module1.vb veya Program.cs içeriğini değiştirin.</span><span class="sxs-lookup"><span data-stu-id="c9313-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="c9313-115">**Proje** menüsünde **Yeni Öğe Ekle'yi**tıklatın.</span><span class="sxs-lookup"><span data-stu-id="c9313-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="c9313-116">**Metin Dosyası'nı** seçin ve ardından **Tamam'ı**tıklatın.</span><span class="sxs-lookup"><span data-stu-id="c9313-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="c9313-117">Bu konuyla ilgili verileri kopyalayın ve yeni metin dosyasına yapıştırın.</span><span class="sxs-lookup"><span data-stu-id="c9313-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="c9313-118">**Dosya** menüsünde **Kaydet,** Plinqdata.csv dosyasını adlandır'ı ve ardından kaynak kod dosyalarınızı içeren klasöre kaydedin'i tıklatın.</span><span class="sxs-lookup"><span data-stu-id="c9313-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="c9313-119">Projenin doğru oluşturduğunu ve çalıştığını doğrulamak için F5 tuşuna basın.</span><span class="sxs-lookup"><span data-stu-id="c9313-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="c9313-120">Aşağıdaki çıktı konsol penceresinde görüntülenmelidir.</span><span class="sxs-lookup"><span data-stu-id="c9313-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="c9313-121">Veriler</span><span class="sxs-lookup"><span data-stu-id="c9313-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="c9313-122">Ayrıca bkz.</span><span class="sxs-lookup"><span data-stu-id="c9313-122">See also</span></span>

- [<span data-ttu-id="c9313-123">Paralel LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="c9313-123">Parallel LINQ (PLINQ)</span></span>](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)
