...
## <a name="creating-a-console-application"></a>Bir konsol uygulaması oluşturma  
 İçinde [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)], oluşturma bir [!INCLUDE[csprcs](../../../includes/csprcs-md.md)] veya [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)] konsol uygulama projesi. System.Threading.Tasks.Dataflow.dll bir başvuru ekleyin.  
  
 Alternatif olarak, bir dosya oluşturun ve adlandırın `ReverseWords.cs` (`ReverseWords.vb` için [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)]), ve ardından Projeyi derlemek için Visual Studio komut istemi penceresinde aşağıdaki komutu çalıştırın.  
...
|`filterWordList`|<xref:System.Threading.Tasks.Dataflow.TransformBlock%602>|Kaldırır kısa sözcükler Word'den dizi, siparişler elde edilen sözcükler alfabetik olarak ve yinelenenleri kaldırma.|  
...
## <a name="forming-the-pipeline"></a>Ardışık Düzen oluşturuluyor  
 ...
 Çağırdığınızda <xref:System.Threading.Tasks.Dataflow.DataflowBlock.LinkTo%2A> yöntemi bir kaynak veri akışı bloğu bir hedef veri akışı blok kaynak veri akışı bloğu bağlanmak için hedef blok veri veri olarak kullanılabilir hale geldiğinde yayar.  
  
 [!code-csharp[TPLDataflow_Palindromes#4](../../../samples/snippets/csharp/VS_Snippets_Misc/tpldataflow_palindromes/cs/dataflowpalindromes.cs#4)]
 [!code-vb[TPLDataflow_Palindromes#4](../../../samples/snippets/visualbasic/VS_Snippets_Misc/tpldataflow_palindromes/vb/dataflowpalindromes.vb#4)]  
  
## <a name="creating-the-completion-tasks"></a>Tamamlama görevler oluşturma  
 Tüm verileri işledikten sonra son bir eylemi gerçekleştirmek her veri akışı bloğu etkinleştirmek için aşağıdaki kodu ekleyin.  
 ...
 Ardışık düzeninden tamamlama yaymak için her tamamlama görevi sonraki veri akışı bloğu tamamlandı durumuna ayarlar. Örneğin, ardışık düzen head tamamlandı durumuna ayarladığınızda, kalan arabelleğe alınan iletileri işleyen ve ardından ikinci üye ardışık tamamlandı durumuna ayarlar, tamamlama görevini çalıştırır. İkinci üye ardışık sırayla kalan arabelleğe alınan iletileri işleyen ve ardışık düzen üçüncü üyesi tamamlandı durumuna ayarlar, tamamlama görevini çalıştırır. Bu işlem, tüm üyeleri ardışık düzen tamamlanana kadar devam eder. Bu örnekte `delegate` anahtar sözcüğü (`Function` içinde [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)]) devamlılık görevlerini tanımlamak için.  
...
## <a name="completing-pipeline-activity"></a>Ardışık Düzen etkinlik Tamamlanıyor  
 Ardışık Düzen head tamamlandı olarak işaretlemek için aşağıdaki kodu ekleyin. Tüm arabelleğe alınan iletileri işledikten sonra ardışık head devamlılık görevi çalıştırır. Bu devamlılık görevi tamamlandı durumuna ardışık düzeninden yayar.  
...
## <a name="waiting-for-the-pipeline-to-finish"></a>Ardışık düzeni için bitiş bekleniyor  
 Ardışık bitmesini beklemek için aşağıdaki kodu ekleyin. Bu örnekte, ardışık düzeninden tamamlama yaymak için devamlılık görevlerini kullandığından, ardışık düzen kuyruğu tamamlandığında ve genel işlemi tamamlandı.  
