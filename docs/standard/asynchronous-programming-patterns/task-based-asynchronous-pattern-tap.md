...
## <a name="target-environment"></a>Hedef ortam  
 ...
 TAP metodunu çağıran, elde edilen görevde zaman uyumlu olarak bekleyen TAP yönteminin tamamlanmasını beklemeyi engelleyebilir veya zaman uyumsuz işlem tamamlandığında ek (devamlılık) kodu çalıştırabilir. Devam kodunun yaratıcısı kodun nerede çalışacağına dair denetime sahiptir. Devamlılığın üzerinde yerleşik olarak bulunan dil desteğini kullanarak (örneğin, <xref:System.Threading.Tasks.Task.ContinueWith%2A>) veya <xref:System.Threading.Tasks.Task> sınıftaki yöntemler aracılığıyla devamlılık kodu oluşturabilirsiniz (örneğin, `await` C#, Visual Basic içinde `Await` F#).`AwaitValue`  
...
```  
 ...
 İptali veya ilerlemeyi isteğe bağlı hale getirmek için birden çok aşırı yük sunmaya karar verirseniz, iptal veya ilerlemeyi desteklemeyen aşırı yüklemeler, iptal için <xref:System.Threading.CancellationToken.None%2A> geçirilmiş ya da bunları destekleyen aşırı yüklemeye ilerleme için `null` gibi davranır.  
