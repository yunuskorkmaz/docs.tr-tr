...
## <a name="naming-asynchronous-methods"></a>Zaman uyumsuz yöntemleri adlandırma  
 ...
 Tanımlayan bir *MethodName* `Async` yöntemi:  
...
-   Birden çok çağrılarını kabul eder.  
 ...
 İsteğe bağlı olarak tanımlayan bir *MethodName* `Async` aşırı yüklemesi, aynı *MethodName*`Async`, ancak ek bir nesne değerli parametre olarak adlandırılan `userState`. Yönteminize yönelik birden çok eşzamanlı çağrılarını; bu durumda yönetmek için hazırlanmış bunu `userState` değeri yöntemi çağrılarını ayırt etmek için geri tüm olay işleyicileri için teslim. Bunu yapmak tercih edebilirsiniz sonraki alma için kullanıcı durumunu depolamak için yalnızca bir yer olarak.  
  
 Her ayrı için *MethodName* `Async` yöntemi imza:  
...
-   Birden çok bekleyen işlemler iptal desteği zaman uyumsuz işlemleri kullanabilir? Diğer bir deyişle, mu *MethodName* `Async` yöntemi Al bir `userState` parametresi ve herhangi bitmesini beklemeden birden çok çağrılarına izin?  
...
|Sınıf içinde birden çok zaman uyumsuz işlemler|`void CancelAsync(object userState);`|`void CancelAsync();`|  
 ...
 Bu yöntemleri genellikle hemen döndürür ve işlem olabilir ya da değil gerçekte iptal edebilirsiniz. Olay işleyicisi içinde *MethodName* `Completed` olayı *MethodName* `CompletedEventArgs` nesnesini içeren bir `Cancelled` istemcileri belirlemek için kullanabileceğiniz alan olup olmadığını İptal oluştu.  
...
## <a name="optionally-support-the-isbusy-property"></a>İsteğe bağlı olarak IsBusy özelliği desteği  
 Sınıfınızda birden çok eşzamanlı çağrılarını desteklemiyorsa gösterme göz önünde bulundurun bir `IsBusy` özelliği. Bu belirlemek geliştiriciler sağlar olup bir *MethodName* `Async` yöntemi, gelen bir özel durum yakalama olmadan çalıştığı *MethodName* `Async` yöntemi.  
...
-   Bu yükseltme *MethodName* `ProgressChanged` raporuna artımlı bir sonuç olduğunda olay.  
...
-   `out`parametreleri *MethodName* parçası olmamalıdır *MethodName*`Async`. Bunun yerine, bir parçası olmalıdır *MethodName* `CompletedEventArgs` , parametre olarak eşdeğer olarak aynı ada sahip *MethodName* (daha uygun bir ad olmadıkça).  
  
-   `ref`parametreleri *MethodName* parçası olarak görünmesi gereken *MethodName*`Async`ve bir parçası olarak *MethodName* `CompletedEventArgs` aynı ada sahip kendi parametre olarak eşdeğer *MethodName* (daha uygun bir ad olmadıkça).  
