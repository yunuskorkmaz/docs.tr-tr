...
CBC şifresi, düz metin bloğunu şifrelemek için bir başlatma vektörü (IV) kullanarak ECB şifrelemeleri ile ilişkili sorunları ortadan kaldırmak. Düz metin olan her bir blok, şifrelenmeden önceki bir bit düzeyinde özel veya (`XOR`) işlemi önceki şifreli olmayan bloğa geçer. Bu nedenle, her bir şifreli blok, önceki tüm bloklara bağımlıdır. Bu sistem kullanıldığında, yetkisiz bir kullanıcı tarafından bilinen ortak ileti üstbilgileri, bir anahtara ters mühendislik uygulamak için kullanılamaz.
...
- <xref:System.Security.Cryptography.ManifestSignatureInformation> sınıfı, <xref:System.Security.Cryptography.ManifestSignatureInformation.VerifySignature%2A> yöntemi aşırı yüklerini kullandığınızda bir bildirim imzasıyla ilgili bilgileri alır.

- Hangi bildirimlerin doğrulanacak olduğunu belirlemek için <xref:System.Security.ManifestKinds> numaralandırmayı kullanabilirsiniz. Doğrulamanın sonucu <xref:System.Security.Cryptography.SignatureVerificationResult> sabit listesi değerlerinden biridir.

- <xref:System.Security.Cryptography.ManifestSignatureInformationCollection> sınıfı, doğrulanmış imzaların <xref:System.Security.Cryptography.ManifestSignatureInformation> nesnelerinin salt okunurdur bir koleksiyonunu sağlar.
...
- <xref:System.Security.Cryptography.X509Certificates.AuthenticodeSignatureInformation> bir bildirime yönelik Authenticode imza bilgilerini temsil eder.

- <xref:System.Security.Cryptography.X509Certificates.TimestampInformation>, Authenticode imzasında zaman damgasıyla ilgili bilgiler içerir.
...
|Başlık|Açıklama|
|-----------|-----------------|
|[Şifreleme Modeli](../../../docs/standard/security/cryptography-model.md)|Şifrelemenin temel sınıf kitaplığında nasıl uygulandığını açıklar.|
...
|[Şifreleme Sınıflarını Yapılandırma](../../../docs/framework/configure-apps/configure-cryptography-classes.md)|Algoritma adlarını şifreleme sınıflarına eşlemeyi ve nesne tanımlayıcılarını şifreleme algoritmayla eşlemeyi açıklar.|
