...
|`x`|Kaçışsız boşluk işaretini yoksayın ve x-Mode yorumlara izin verin.|  
 ...
 `(?imnsx-imnsx)` yapısı tarafından tanımlanan normal ifade seçeneklerindeki değişiklikler, kapsayan grubun sonuna kadar yürürlükte kalır.  
  
> [!NOTE]
> `(?imnsx-imnsx:`alt *ifade*`)` gruplandırma yapısı, bir alt ifade için aynı işlevselliği sağlar. Daha fazla bilgi için bkz. [yapıları gruplandırma](../../../docs/standard/base-types/grouping-constructs-in-regular-expressions.md).  
  
 Aşağıdaki örnek, büyük/küçük harf duyarlı ve açık yakalamaları etkinleştirmek için `i`, `n`ve `x` seçeneklerini kullanır ve normal bir ifadenin ortasında yer alan normal ifade deseninin beyaz uzayını yok sayar.  
...
|`(d\w+)`|Büyük veya küçük harf "d" ile sonra bir veya daha fazla sözcük karakteri eşleştirin. `n` (açık yakalama) seçeneği etkinleştirildiğinden bu grup yakalanmaz.|  
|`\b`|Bir sözcük sınırıyla eşleş.|  
  
## <a name="inline-comment"></a>Satır içi açıklama  
 `(?#` *comment*`)` yapısı, bir normal ifadeye satır içi bir açıklama eklemenize olanak sağlar. Normal ifade altyapısı, açıklama <xref:System.Text.RegularExpressions.Regex.ToString%2A?displayProperty=nameWithType> yöntemi tarafından döndürülen dizeye dahil edilse de, bu açıklamanın herhangi bir parçasını model eşleme içinde kullanmaz. Açıklama ilk kapanış parantezinde sona erer.  
  
 Aşağıdaki örnek, önceki bölümdeki örnekteki ilk normal ifade deseninin yinelenir. Karşılaştırmayı, büyük/küçük harfe duyarlı olup olmadığını belirtmek için normal ifadeye iki satır içi açıklama ekler. `\b((?# case-sensitive comparison)D\w+)\s(?ixn)((?#case-insensitive comparison)d\w+)\b`normal ifade deseninin aşağıdaki şekilde tanımlanması gerekir.  
