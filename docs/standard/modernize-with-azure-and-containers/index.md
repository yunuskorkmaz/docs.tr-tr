...
description: "Kaldırın ve kapsayıcılar ve Azure cloud mevcut uygulamalarına shift öğrenin"
...
Ardından, geçirmek, birçok var olan .NET Framework uygulamaları için ideal senaryo olarak çok az şey yapmanızı büyük avantajlarından yararlanabilmek için bir bulut DevOps hazır uygulamaya geçişi yapılamadı. Bu yaklaşım Ayrıca, bulut iyileştirme için olası bir sonraki adım olarak ayarlar. Şekil 1-6 bir örneği gösterir.
...
Bu kılavuzun odağı özeldir. Yükseltme ve varolan .NET uygulamalarınızın shift mimarisinin yeniden düzenlenmesinden olmadan ve kod değişikliğine elde etmek için uygulayabileceğiniz yolu gösteriyoruz. Sonuç olarak, uygulamanızın bulut DevOps kullanıma hazır olmak nasıl gösteriyoruz.

Bu kılavuz bir mikro mimarileri gelişmesi nasıl gibi bulut yerel uygulamalar ile çalışmaya nasıl göstermez. Uygulamalarınızı yeniden düzenlenmesine veya üzerinde mikro dayalı yeni uygulamalar oluşturmak için e-kitap bakın [.NET mikro: mimarisi kapsayıcılı .NET uygulamaları için](https://aka.ms/microservicesebook).
...
Geliştiriciler ve .NET Framework sevkiyat ve uygulamaları serbest Gelişmiş çeviklik dayalı mevcut ASP.NET uygulamaları modernize istediğiniz çözüm mimarları için bu kılavuzu yazıldı.
