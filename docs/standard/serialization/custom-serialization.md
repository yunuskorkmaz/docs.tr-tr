...
```  
 ...
 Bir sınıfa eklendiğinde <xref:System.Runtime.Serialization.ISerializable> , hem **GetObjectData** hem de özel oluşturucuyu uygulamanız gerektiğini belirten bir stres olması önemlidir. **GetObjectData** eksik olduğunda derleyici sizi uyarır. Ancak, bir oluşturucunun uygulanmasını zorlamak imkansız olduğundan, Oluşturucu yoksa bir uyarı sağlanmaz ve bir sınıfın Oluşturucusu olmadan serisini kaldırma girişimi yapıldığında bir özel durum oluşturulur.  
