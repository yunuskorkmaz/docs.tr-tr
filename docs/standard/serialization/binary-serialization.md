...
İkili serileştirme bir nesne içindeki özel üyelerin değiştirilmesini ve bu nedenle durumunun değiştirilmesini sağlar. Bu nedenle, ortak API yüzeyi üzerinde çalışan <xref:System.Text.Json?displayProperty=fullName>gibi diğer serileştirme çerçeveleri önerilir.
...
> | <xref:System.Data.DataTable?displayProperty=nameWithType> | `RemotingFormat` `SerializationFormat.Binary`olarak ayarlarsanız yalnızca .NET Core 2,1 ve sonraki sürümlerle değiş tokuş edilebilir. |
