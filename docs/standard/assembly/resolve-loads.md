...
### <a name="the-correct-way-to-handle-assemblyresolve"></a>AssemblyResolve işlemenin doğru yolu

<xref:System.AppDomain.AssemblyResolve>Olay işleyiciden derlemeler çözümlenirken, <xref:System.StackOverflowException> işleyicinin <xref:System.Reflection.Assembly.Load%2A?displayProperty=nameWithType> veya yöntem çağrılarını kullanması durumunda, bir sonuç olarak oluşturulur <xref:System.AppDomain.Load%2A?displayProperty=nameWithType> . Bunun yerine, <xref:System.Reflection.Assembly.LoadFile%2A> <xref:System.Reflection.Assembly.LoadFrom%2A> olayı yükseltmediğinden veya yöntemlerini kullanın `AssemblyResolve` .

`MyAssembly.dll`Yürütülen derlemenin yakınında, bilinen bir konumda bulunan, derlemenin yolu kullanılarak çözülebildiği hakkında düşünün `Assembly.LoadFile` .
