...
- Bir yığın yuvasında veya bir işlemci kaydındaki (Kullanıcı kodu tarafından açıkça oluşturulan veya örtük olarak tam zamanında (JıT) derleyici), statik bir değişken veya güçlü bir tanımlayıcı olan toplanabilir `AssemblyLoadContext` dışında tutulan düzenli başvurular ( sabitleme) GC tanıtıcısı ve şu şekilde geçişli işaret:
...
Bu komut, GC yığınındaki `LoaderAllocator` içeren bir tür adına sahip tüm nesneleri döker. Aşağıda bir örnek verilmiştir:
