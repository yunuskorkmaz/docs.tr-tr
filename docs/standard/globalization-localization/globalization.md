...
Bu makalede bazı dikkate almanız gereken ana sorunları ve, dizeleri, tarih ve saat değerleri ve genelleştirilmiş bir uygulamada sayısal değerleri işlerken uygulayabileceğiniz en iyi uygulamalar açıklanmaktadır.
...
Varsayılan olarak, .NET, Unicode dizelerini kullanır. Bir Unicode dizesi sıfır, bir veya daha fazla oluşur <xref:System.Char> her biri bir UTF-16 kod birimini temsil eden nesneleri. Dünyada kullanılan kümesindeki her karakteri neredeyse her karakter için bir Unicode gösterimi vardır.
...
### <a name="use-resource-files"></a>Kaynak dosyaları kullan
...
### <a name="search-and-compare-strings"></a>Arama ve karşılaştırma dizeleri
...
### <a name="test-strings-for-equality"></a>Dizeleri eşitlik için test etme

İki dizeyi eşitlik yerine sıralama düzeninde karşılaştırmasına belirlemek daha kullanmak için test etmek isterseniz <xref:System.String.Equals%2A?displayProperty=nameWithType> yöntemi gibi dize karşılaştırma yöntemi yerine <xref:System.String.Compare%2A?displayProperty=nameWithType> veya <xref:System.Globalization.CompareInfo.Compare%2A?displayProperty=nameWithType>.
...
.NET, kültüre duyarlı sıralamaları dize veriler üzerinde gerçekleştirmek için tablolar kullanır. Dize normalleştirme ve sıralama ağırlıkları verilerini içeren, bu tabloların içeriği, belirli bir .NET sürümü tarafından uygulanan Unicode standart sürümü tarafından belirlenir. Aşağıdaki tabloda, .NET Core ve .NET Framework'ün belirtilen sürümleri tarafından uygulanan Unicode sürümleri listelenmiştir. Bu liste desteklenen Unicode sürümleri karakter karşılaştırma ve sıralama yalnızca geçerli olduğunu unutmayın. kategoriye göre Unicode karakter sınıflandırması için uygulanmaz. Daha fazla bilgi için bkz: "Dizeleri ve Unicode standardı" bölümünde <xref:System.String> makalesi.
...
|Windows 7'yi .NET framework 4.5 ve sonraki sürümleri|Unicode 5.0|
|.NET framework 4.5 ve daha sonra Windows 8 ve sonraki işletim sistemleri|Unicode 6.3.0|
|.NET core (tüm sürümler)|Temel işletim sistemi tarafından desteklenen Unicode standart sürümü bağlıdır.|

Tüm .NET Core sürümlerinde .NET Framework 4.5 ile başlayarak, dize karşılaştırması ve sıralaması işletim sistemine bağlıdır üzerinde. .NET Framework 4.5 ve sonraki Windows 7'de çalışan verileri Unicode 5.0 uygulayan kendi tablolarından alır. .NET Framework 4.5 ve sonraki Windows 8 ve sonraki sürümlerde çalışan verilerini Unicode 6.3 uygulayan işletim sistemi tablolarından alır. .NET Core üzerinde desteklenen Unicode sürümü temel işletim sistemine bağlıdır. Kültüre duyarlı sıralanmış verileri sıralarsanız, kullanabileceğiniz <xref:System.Globalization.SortVersion> serileştirilmiş verilerinizi ne zaman .NET ile işletim sisteminin sıralama düzeniyle tutarlı olması yeniden sırılanacığını saptamak için. Bir örnek için bkz <xref:System.Globalization.SortVersion> sınıf konusuna.
...
### <a name="display-dates-and-times"></a>Görüntü tarihler ve saatler
...
### <a name="persist-dates-and-times"></a>Tarihler ve saatler Sürdür
...
### <a name="perform-date-and-time-arithmetic"></a>Tarih ve saat aritmetiği gerçekleştirir
...
### <a name="use-culture-sensitive-names-for-date-elements"></a>Veri öğeleri için kültüre duyarlı adlar kullanma
...
## <a name="numeric-values"></a>Sayısal değerleri
...
> [!NOTE]
> Ayrıştırma ve biçimlendirme işlemleri, .NET yalnızca temel Latin karakterleri 0-9 tanır (U + 0030-U + 0039) sayısal basamak olarak.
...
### <a name="persist-numeric-values"></a>Kalıcı sayısal değerler
...
. NET'te, <xref:System.Globalization.CultureInfo> sınıfı belirli bir kültür veya bölgeyi temsil eder. Bazı özellikleri bir kültürün bazı yönleri hakkında belirli bilgi sağlayan nesneleri döndürür:
