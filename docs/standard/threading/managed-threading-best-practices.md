...
## <a name="static-members-and-static-constructors"></a>Statik üyeler ve statik oluşturucular  
 Sınıf Oluşturucusu (içinde C#`static` oluşturucu, @no__t-Visual Basic 2) çalışmayı tamamlayana kadar bir sınıf başlatılamaz. Başlatılmamış bir türdeki kodun yürütülmesini engellemek için, ortak dil çalışma zamanı, sınıf oluşturucusunun çalışmayı bitirene kadar diğer iş parçacıklarından gelen `static` üye (Visual Basic `Shared` üyeleri) olan tüm çağrıları engeller.  
  
 Örneğin, bir sınıf Oluşturucusu yeni bir iş parçacığı başlatır ve iş parçacığı yordamı sınıfının `static` üyesini çağırırsa, sınıf oluşturucusu tamamlanana kadar yeni iş parçacığı engeller.  
  
 Bu, `static` oluşturucusuna sahip olan her tür için geçerlidir.  
...
- Birden çok iş parçacığının etkinliklerini eşleştirmek için <xref:System.Threading.Thread.Suspend%2A?displayProperty=nameWithType> ve <xref:System.Threading.Thread.Resume%2A?displayProperty=nameWithType> kullanmayın. @No__t-0, <xref:System.Threading.ManualResetEvent>, <xref:System.Threading.AutoResetEvent> ve <xref:System.Threading.Monitor> kullanın.  
...
- Türleri kilit nesneleri olarak kullanmayın. Diğer bir deyişle, Visual Basic `lock(typeof(X))` C# veya `SyncLock(GetType(X))` gibi koddan veya <xref:System.Threading.Monitor.Enter%2A?displayProperty=nameWithType> ' in <xref:System.Type> nesneleriyle birlikte kullanılmasına engel olur. Belirli bir tür için, uygulama etki alanı başına <xref:System.Type?displayProperty=nameWithType> ' ın yalnızca bir örneği vardır. Bir kilidi aldığınız tür herkese açık ise, kendi dışında bir kod, kilitlenmeleri için bir kilit alabilir. Ek sorunlar için bkz. [Güvenilirlik En Iyi uygulamaları](../../../docs/framework/performance/reliability-best-practices.md).  
  
- Örneğin, Visual Basic içinde `lock(this)` C# veya `SyncLock(Me)` gibi örnekleri kilitlerken dikkatli olun. Uygulamanızdaki diğer kod, türü dışında, nesne üzerinde bir kilit alırsa, kilitlenmeler meydana gelebilir.  
...
- @No__t-1 ifadesini kullanmak yerine basit durum değişiklikleri için <xref:System.Threading.Interlocked> sınıfının yöntemlerini kullanmayı düşünün (`SyncLock` Visual Basic). @No__t-0, iyi bir genel amaçlı araçtır, ancak <xref:System.Threading.Interlocked> sınıfı atomik olması gereken güncelleştirmeler için daha iyi performans sağlar. Bir çekişme yoksa, dahili olarak tek bir kilit öneki yürütür. Kod İncelemeleri bölümünde aşağıdaki örneklerde gösterildiği gibi kodu izleyin. İlk örnekte, bir durum değişkeni artırılır:  
    ...
    ```  
     ...
     @No__t-1 ifadesinin yerine <xref:System.Threading.Interlocked.Increment%2A> yöntemini kullanarak performansı geliştirebilirsiniz:  
    ...
    > .NET Framework 2,0 ve sonrasında, 1 ' den büyük atomik artışlarla <xref:System.Threading.Interlocked.Add%2A> yöntemini kullanın.  
     ...
     İkinci örnekte, bir başvuru türü değişkeni yalnızca null bir başvuru ise güncelleştirilir (Visual Basic `Nothing`).  
    ...
    ```  
     ...
     Performans, aşağıdaki gibi <xref:System.Threading.Interlocked.CompareExchange%2A> yöntemi kullanılarak artırılabilir:  
