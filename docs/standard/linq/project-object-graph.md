...
---
# <a name="how-to-project-an-object-graph-linq-to-xml"></a>Bir nesne grafiğini proje (LINQ to XML)

XML 'den bir nesne grafiğini proje veya doldurmak için C# veya Visual Basic kullanabilirsiniz.

## <a name="example-populate-an-object-graph-from-xml"></a>Örnek: bir nesne grafiğini XML 'den doldurma

Bu örnek `Address` , `PurchaseOrder` XML BELGESI örnek XML dosyasındaki, ve sınıfları ile bir nesne grafiğini doldurur `PurchaseOrderItem` [: tipik satın alma siparişi](sample-xml-file-typical-purchase-order.md).
