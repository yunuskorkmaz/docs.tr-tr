...
## <a name="enable-xslt-script"></a>XSLT betiğini etkinleştir  
 Katıştırılmış betikler için destek, <xref:System.Xml.Xsl.XslCompiledTransform> sınıfında isteğe bağlı bir XSLT ayarıdır. Komut dosyası desteği varsayılan olarak devre dışıdır. Betik desteğini etkinleştirmek için, <xref:System.Xml.Xsl.XsltSettings.EnableScript%2A> özelliği `true` olarak ayarlanmış bir <xref:System.Xml.Xsl.XsltSettings> nesnesi oluşturun ve nesneyi <xref:System.Xml.Xsl.XslCompiledTransform.Load%2A> metoduna geçirin.  
...
|`Node Set`|<xref:System.Xml.XPath.XPathNodeIterator>|  
 ...
 CLR sayısal türleri <xref:System.Double>dönüştürülür. <xref:System.DateTime> türü <xref:System.String>dönüştürülür. <xref:System.Xml.XPath.IXPathNavigable> türler <xref:System.Xml.XPath.XPathNavigator>dönüştürülür. **XPathNavigator []** <xref:System.Xml.XPath.XPathNodeIterator>dönüştürüldü.  
...
### <a name="importing-namespaces-and-assemblies"></a>Ad alanları ve derlemeler içeri aktarılıyor  
 <xref:System.Xml.Xsl.XslCompiledTransform> sınıfı, varsayılan olarak `msxsl:script` öğesi tarafından desteklenen derleme ve ad alanları kümesini önceden tanımlar. Ancak, `msxsl:script` bloğunda derlemeyi ve ad alanını içeri aktararak önceden tanımlanmış listede olmayan bir ad alanına ait sınıfları ve üyeleri kullanabilirsiniz.  
...
```  
 ...
 `name` özniteliği derlemenin adını içerir ve `href` özniteliği derlemenin yolunu içerir. Derleme adı "System. Data, Version = 2.0.3600.0, Culture = neutral, PublicKeyToken = b77a5c561934e089" gibi bir tam ad veya "System. Web" gibi kısa bir ad olabilir.  
