...
[World Wide Web Konsorsiyumu (W3C) XML şeması önerisi](https://www.w3.org/XML/Schema) , şema öncesi doğrulama ve şema sonrası derleme için açığa çıkarılması gereken bilgi kümesini (Infoset) açıklar. XML şeması nesne modeli (SOM), bu pozlamayı <xref:System.Xml.Schema.XmlSchemaSet> <xref:System.Xml.Schema.XmlSchemaSet.Compile%2A> yönteminden önce ve sonra görüntüler.  
  
 Şema düzenlemesi sırasında şema öncesi doğrulama bilgi kümesi oluşturulur. Şema sonrası derleme bilgi kümesi, şemanın derlenmesi sırasında <xref:System.Xml.Schema.XmlSchemaSet> <xref:System.Xml.Schema.XmlSchemaSet.Compile%2A> yöntemi çağrıldıktan sonra oluşturulur ve özellikler olarak gösterilir.  
...
|<xref:System.Xml.Schema.XmlSchemaComplexType>|<xref:System.Xml.Schema.XmlSchemaComplexType.AnyAttribute%2A>|  
 ...
 Örneğin, <xref:System.Xml.Schema.XmlSchemaElement> ve <xref:System.Xml.Schema.XmlSchemaComplexType> sınıflarının her ikisi de `BlockResolved` ve `FinalResolved` özelliklere sahiptir. Bu özellikler, şema derlendikten ve doğrulandıktan sonra `Block` ve `Final` özelliklerinin değerlerini tutmak için kullanılır. `BlockResolved` ve `FinalResolved`, şema sonrası derleme bilgi kümesinin parçası olan salt yazılır özelliklerdir.  
  
 Aşağıdaki örnek, şema doğrulandıktan sonra <xref:System.Xml.Schema.XmlSchemaElement> sınıfının <xref:System.Xml.Schema.XmlSchemaElement.ElementSchemaType%2A> özelliğini gösterir. Doğrulamadan önce, özelliği `null` bir başvuru içerir ve <xref:System.Xml.Schema.XmlSchemaElement.SchemaTypeName%2A> söz konusu türün adına ayarlanır. Doğrulamadan sonra, <xref:System.Xml.Schema.XmlSchemaElement.SchemaTypeName%2A> geçerli bir türe çözülür ve Type nesnesi <xref:System.Xml.Schema.XmlSchemaElement.ElementSchemaType%2A> özelliği aracılığıyla kullanılabilir.  
