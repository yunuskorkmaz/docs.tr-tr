...
> [!NOTE]
> Tüm `complexType` bildirimleri anonim türler olarak algılanır. Çıkarılan tek genel öğe kök öğesidir; diğer tüm öğeler yereldir.  
...
### <a name="simple-typed-element"></a>Basit tür belirtilmiş öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğe, basit tür öğesi için gösterilen şemayı gösterir.  
...
### <a name="empty-element"></a>Boş öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğe, boş öğe için gösterilen şemayı gösterir.  
...
### <a name="empty-element-with-attributes"></a>Öznitelikleri olan boş öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, özniteliklerin bulunduğu boş öğe için gösterilen şemayı gösterir.  
...
### <a name="element-with-attributes-and-simple-content"></a>Öznitelikleri ve basit Içeriği olan öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, öznitelikleri ve basit içeriği olan bir öğe için gösterilen şemayı gösterir.  
...
### <a name="element-with-a-sequence-of-child-elements"></a>Alt öğeleri dizisi olan öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, alt öğeleri dizisi olan bir öğe için gösterilen şemayı gösterir.  
...
### <a name="element-with-a-sequence-of-child-elements-and-attributes"></a>Alt öğeler ve öznitelikler dizisi olan öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, alt öğe ve özniteliklerin sırası olan bir öğe için gösterilen şemayı gösterir.  
...
### <a name="element-with-a-sequence-and-choices-of-child-elements"></a>Alt öğelerin sırası ve seçimlerine sahip öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, bir öğe için gösterilen şemayı bir dizi ve tercih edilen alt öğeleri gösterir.  
  
> [!NOTE]
> `xs:choice` öğesinin `maxOccurs` özniteliği, çıkarılan şemada `"unbounded"` olarak ayarlanmıştır.  
...
### <a name="element-with-a-sequence-and-choice-of-child-elements-and-attributes"></a>Alt öğe ve özniteliklerin sırası ve seçimi içeren öğe  
 Aşağıdaki tabloda <xref:System.Xml.Schema.XmlSchemaInference.InferSchema%2A> metoduna XML girişi ve oluşturulan XML şeması gösterilmektedir. Kalın öğeler, alt öğe ve özniteliklerin sırası ve seçimi içeren bir öğe için gösterilen şemayı gösterir.  
...
### <a name="attribute-processing"></a>Öznitelik Işleme  
 Düğüm içinde yeni bir özniteliğe her karşılaşıldığında, `use="required"`ile düğümün çıkartılan tanımına eklenir. Örnekte aynı düğüm bir sonraki sefer olduğunda, çıkarım işlemi geçerli örneğin özniteliklerini zaten çıkartılan olanlarla karşılaştıracaktır. Örnekte zaten bir tane varsa, öznitelik tanımına `use="optional"` eklenir. Yeni öznitelikler `use="optional"`var olan bildirimlere eklenir.  
  
### <a name="occurrence-constraints"></a>Oluşum kısıtlamaları  
 Şema çıkarımı işlemi sırasında, bir şemanın çıkartılan bileşenleri için, `minOccurs` ve `maxOccurs` öznitelikleri oluşturulur, bu değerler `"0"` veya `"1"` ve `"1"` ya da `"unbounded"`. `"1"` ve `"unbounded"` değerleri yalnızca `"0"` ve `"1"` XML belgesini doğrulayamıyorsa kullanılır (örneğin, `MinOccurs="0"` bir öğeyi doğru bir şekilde açıklamıyorsa, `minOccurs="1"` kullanılır).  
  
### <a name="mixed-content"></a>Karışık Içerik  
 Bir öğe karışık içerik içeriyorsa (örneğin, öğeler ile metin ile birlikte), `mixed="true"` özniteliği, gösterilen karmaşık tür tanımı için oluşturulur.  
...
|İşlem yönergesi|LIP.|  
|Yorum|LIP.|  
|Varlık başvurusu|<xref:System.Xml.Schema.XmlSchemaInference> sınıfı varlık başvurularını işlemez. Bir XML belgesi varlık başvuruları içeriyorsa, varlıkları genişleten bir okuyucu kullanmanız gerekir. Örneğin, bir <xref:System.Xml.XmlTextReader> <xref:System.Xml.XmlTextReader.EntityHandling%2A> özelliği bir parametre olarak <xref:System.Xml.EntityHandling.ExpandEntities> olarak geçirebilirsiniz. Varlık başvurularına karşılaşılırsa ve okuyucu varlıkları genişletmezse bir özel durum oluşturur.|  
|CDATA|XML belgesindeki tüm `<![CDATA[ … ]]` bölümleri, `xs:string`olarak algılanır.|  
|Belge türü|LIP.|  
|{1&gt;Ad alanları&lt;1}|LIP.|  
