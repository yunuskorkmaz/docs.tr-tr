...
> [!NOTE]
> `xs:choice` Öğesinin `maxOccurs` özniteliği, gösterilen şemada olarak `"unbounded"` ayarlanır.  
...
### <a name="occurrence-constraints"></a>Oluşum kısıtlamaları  
 Şema `minOccurs` çıkarımı işlemi sırasında, ve `maxOccurs` öznitelikleri, bir şemanın Çıkarsanan bileşenleri için, veya `"0"` `"1"` ve `"1"` ya `"unbounded"`da değerleri oluşturulur. Değerler `"1"` ve `"unbounded"` yalnızca değerler `"0"` ve `"1"` XML belgesini doğrulayamıyorsa kullanılır (örneğin, `MinOccurs="0"` bir öğeyi doğru bir şekilde açıklamıyorsa, `minOccurs="1"` kullanılır).  
  
### <a name="mixed-content"></a>Karışık Içerik  
 Bir öğe, karışık içerik içeriyorsa (örneğin, öğelerle birlikte metin oluşturma), bu `mixed="true"` öznitelik, gösterilen karmaşık tür tanımı için oluşturulur.  
