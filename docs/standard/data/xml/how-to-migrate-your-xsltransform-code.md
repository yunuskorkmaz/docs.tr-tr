...
- <xref:System.Xml.Xsl.XslCompiledTransform> Sınıfını kullanan kod.  
    ...
    > [!NOTE]
    > <xref:System.Xml.Xsl.XslCompiledTransform> SıNıFıN, XSLT dönüştürme sonuçlarını <xref:System.Xml.XmlReader> nesne olarak döndüren bir yöntemi yoktur. Ancak, bir XML dosyasına çıktı verebilir ve XML dosyasını başka bir nesneye yükleyebilirsiniz.  
