...
```  
 ...
 Sonraki örnekte, fiyatın belirtilen miktardan daha büyük olduğu tüm kitap düğümlerini seçmek için <xref:System.Xml.XmlNode.SelectNodes%2A> yöntemi kullanılmaktadır. Seçili listedeki her bir kitabın fiyatı, daha sonra program aracılığıyla yüzde on oranında azaltılır. Son olarak, güncelleştirilmiş dosya konsola yazılır. Bu konunun sonunda belirtilen kitaplığı. xml dosyası, giriş dosyası olarak kullanılır.  
...
```  
 ...
 Tüm <xref:System.Xml.XmlNodeList> nesneleri temeldeki belgeyle eşitlenir. Bu nedenle, düğüm listesini yineleyebilir ve bir düğümün değerini değiştirirseniz bu düğüm, geldiği belgede de güncelleştirilir. Önceki örnekte, bir düğüm seçili <xref:System.Xml.XmlNodeList> değiştirildiğinde, temel alınan belgenin da değiştirildiğini görürsünüz.  
...
## <a name="namespaces-in-xpath-expressions"></a>XPath Ifadelerinde ad alanları  
 XPath ifadeleri ad alanı içerebilir. Ad alanı çözümlemesi <xref:System.Xml.XmlNamespaceManager>kullanılarak desteklenir. XPath ifadesi bir ön ek içeriyorsa, önek ve ad alanı URI çiftinin <xref:System.Xml.XmlNamespaceManager>eklenmelidir ve <xref:System.Xml.XmlNamespaceManager> <xref:System.Xml.XmlNode.SelectNodes%28System.String%2CSystem.Xml.XmlNamespaceManager%29> ya da <xref:System.Xml.XmlNode.SelectSingleNode%28System.String%2CSystem.Xml.XmlNamespaceManager%29> yöntemine geçirilir. Yukarıdaki kod örneklerinin, kitaplığı. xml belgesinin ad alanını çözümlemek için <xref:System.Xml.XmlNamespaceManager> kullandığına dikkat edin.  
  
> [!NOTE]
> XPath ifadesi bir ön ek içermiyorsa, ad alanı Tekdüzen Kaynak tanımlayıcısı 'nın (URI) boş ad alanı olduğu varsayılır. XML 'niz varsayılan bir ad alanı içeriyorsa, <xref:System.Xml.XmlNamespaceManager>bir ön ek ve ad alanı URI 'SI eklemeniz gerekir; Aksi takdirde hiçbir düğüm seçilmeyecektir.  
