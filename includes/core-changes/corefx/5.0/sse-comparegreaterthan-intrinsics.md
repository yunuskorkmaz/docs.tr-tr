...
#### <a name="change-description"></a><span data-ttu-id="30c32-103">Açıklamayı Değiştir</span><span class="sxs-lookup"><span data-stu-id="30c32-103">Change description</span></span>

<span data-ttu-id="30c32-104">Daha önce, `NaN` listelenmiş <xref:System.Runtime.Intrinsics.X86.Sse> ve yöntemlere yapılan girişler <xref:System.Runtime.Intrinsics.X86.Sse2> yanlış bir sonuç döndürdü.</span><span class="sxs-lookup"><span data-stu-id="30c32-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="30c32-105">Sonuç, sınıfında karşılık gelen yöntem tarafından oluşturulan sonuçtan de farklıydı <xref:System.Runtime.Intrinsics.X86.Avx> .</span><span class="sxs-lookup"><span data-stu-id="30c32-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="30c32-106">.NET 5,0 ' den başlayarak, bu yöntemler girişleri doğru şekilde işler `NaN` ve sınıftaki karşılık gelen yöntemlerle aynı sonuçları döndürür <xref:System.Runtime.Intrinsics.X86.Avx> .</span><span class="sxs-lookup"><span data-stu-id="30c32-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="30c32-107">Streaming SIMD Extensions (SSE) ve Streaming SIMD Extensions 2 (SSE2) endüstri standardı mimarileri (IAS), bu karşılaştırma yöntemleri için doğrudan donanım desteği sağlamamakta ve bu nedenle yazılımda uygulanırlar.</span><span class="sxs-lookup"><span data-stu-id="30c32-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="30c32-108">Daha önce, Yöntemler hatalı şekilde uygulanmıştır ve girdileri yanlış bir şekilde işledi `NaN` .</span><span class="sxs-lookup"><span data-stu-id="30c32-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="30c32-109">Yerel bilgisayardan gelen kod için hatalı davranış hataları ortaya çıkarabilir.</span><span class="sxs-lookup"><span data-stu-id="30c32-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="30c32-110">256 bitlik bir kod yolu için yöntemler, sınıftaki eşdeğer yöntemlere farklı sonuçlar da üretebilir <xref:System.Runtime.Intrinsics.X86.Avx> .</span><span class="sxs-lookup"><span data-stu-id="30c32-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="30c32-111">Yöntemlerin daha önceden nasıl yanlış olduğunu gösteren bir örnek olarak, `CompareNotGreaterThan(x,y)` `CompareLessThanOrEqual(x,y)` normal tamsayılar için olarak öğesini uygulayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="30c32-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="30c32-112">Ancak, `NaN` girişler için bu mantık yanlış sonucu hesaplar.</span><span class="sxs-lookup"><span data-stu-id="30c32-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="30c32-113">Bunun yerine, `CompareNotLessThan(y,x)` sayılar doğru şekilde karşılaştırılmaktadır *ve* `NaN` girişler dikkate alır.</span><span class="sxs-lookup"><span data-stu-id="30c32-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="30c32-114">Sunulan sürüm</span><span class="sxs-lookup"><span data-stu-id="30c32-114">Version introduced</span></span>

<span data-ttu-id="30c32-115">5,0 Preview 4</span><span class="sxs-lookup"><span data-stu-id="30c32-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="30c32-116">Önerilen eylem</span><span class="sxs-lookup"><span data-stu-id="30c32-116">Recommended action</span></span>

- <span data-ttu-id="30c32-117">Önceki davranışın bir hata olması durumunda hiçbir değişiklik yapılması gerekmez.</span><span class="sxs-lookup"><span data-stu-id="30c32-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="30c32-118">Önceki davranış isteniyorsa, ilgili çağrıyı aşağıdaki gibi değiştirerek bu davranışı koruyabilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="30c32-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="30c32-119">Kategori</span><span class="sxs-lookup"><span data-stu-id="30c32-119">Category</span></span>

<span data-ttu-id="30c32-120">Core .NET kitaplıkları</span><span class="sxs-lookup"><span data-stu-id="30c32-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="30c32-121">Etkilenen API’ler</span><span class="sxs-lookup"><span data-stu-id="30c32-121">Affected APIs</span></span>
